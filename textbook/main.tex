...
% ============================================================
% CLEVEREF NAMES (all envs above share the `defn' counter via the
% \newtheorem optional arg, which defeats cleveref's auto-naming;
% declare names explicitly, else \cref emits "reference format for
% label type ... undefined" warnings + a spurious "undefined
% references" summary line, even though refs render correctly).
% ============================================================
...
\crefname{notation}{Notation}{Notations}            \Crefname{notation}{Notation}{Notations}
\crefname{convention}{Convention}{Conventions}      \Crefname{convention}{Convention}{Conventions}
...
\crefname{warning}{Warning}{Warnings}               \Crefname{warning}{Warning}{Warnings}
...
% Graph relations. The local \_ redefinition inserts a zero-width breakpoint
% after each underscore so long snake_case edge/node labels wrap in tight captions.
\newcommand{\rel}[1]{\texttt{\small\def\_{\textunderscore\discretionary{}{}{}}#1}}
\newcommand{\nde}[1]{\textsf{\small\def\_{\textunderscore\discretionary{}{}{}}#1}}
\newcommand{\edge}[3]{\nde{#1}\ensuremath{\;\xrightarrow{\text{\rel{#2}}}\;}\nde{#3}}

% Growth function
\newcommand{\growth}{\ensuremath{\Pi}}

% Formal-companion references to the design-lab Lean 4 development
% Long qualified names are wrapped: a zero-width breakpoint is inserted after
% every '.' and '_' and at each camelCase boundary (lower-to-upper), so Lean
% declarations never overflow the text block.
\makeatletter
\begingroup\catcode`\_=12 \gdef\lean@us{_}\endgroup
\def\lean@end{\lean@end}
\newif\iflean@lc
\def\lean@brk#1{%
  \ifx#1\lean@end
  \else
    \iflean@lc \ifnum`#1>64 \ifnum`#1<91 \discretionary{}{}{}\fi\fi \fi
    #1%
    \lean@lcfalse
    \ifnum`#1>96 \ifnum`#1<123 \lean@lctrue \fi\fi
    \if#1.\discretionary{}{}{}\fi
    \if#1\lean@us\discretionary{}{}{}\fi
    \expandafter\lean@brk
  \fi}
\newcommand{\leanname}[1]{\texttt{\lean@lcfalse\expandafter\lean@brk\detokenize{#1}\lean@end}}
\newcommand{\formal}[1]{\footnote{Machine-checked as \leanname{#1} in the companion Lean~4 development.}}
% \code: same wrapping for inline code identifiers that are not Lean declarations
% (file names, JSON schema fields), so they break rather than overflow the margin.
\newcommand{\code}[1]{\texttt{\lean@lcfalse\expandafter\lean@brk\detokenize{#1}\lean@end}}
\makeatother
% Let TeX accept slightly looser lines so the breakpoints above are taken
% instead of overflowing the margin with an unbreakable identifier.
...
\input{chapters/ch01_objects}
\input{chapters/ch02_data}
\input{chapters/ch03_automata}
\input{chapters/ch04_agents}
\input{chapters/ch05_pac}
\input{chapters/ch06_online}
\input{chapters/ch07_gold}
\input{chapters/ch08_exact}
\input{chapters/ch09_universal}
\input{chapters/ch10_dimensions}
\input{chapters/ch11_compression}
\input{chapters/ch12_generalization}
\input{chapters/ch13_ordinals}
\input{chapters/ch14_separations}
\input{chapters/ch15_analogies}
\input{chapters/ch16_computational}
\input{chapters/ch17_extensions}
\input{chapters/ch18_frontiers}

% ============================================================
\appendix
% ============================================================

\input{appendices/appA_edge_inventory}
\input{appendices/appB_traversals}
\input{appendices/appC_validation}
