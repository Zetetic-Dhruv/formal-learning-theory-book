...
\nde{mistake\_bound} & \nde{littlestone\_dimension} & \textit{none: proved equivalence} & \cite{Littlestone1988} \\
...
\midrule
\bottomrule
\nde{computational\_hardness} & \nde{cryptographic\_hardness} & \cite{ApplebaumBarakXiao2008} \\
\nde{posterior\_consistency} & \nde{bayesian\_inference} & \cite{Schwartz1965} \\
\end{longtable}

\bigskip
\noindent\textbf{Machine-checked witnesses.}\quad
A growing subset of these edges carries a witness that is not merely cited but
verified in the companion Lean~4 development.  The \rel{characterizes} edge
$\nde{ds\_dimension}\to\nde{pac\_learning}$ aside (whose witness is the
Brukhim et al.\ construction, cited not formalized), several
\rel{does\_not\_imply} separations are machine-checked (thresholds for
$\nde{pac\_learning}\not\to\nde{mistake\_bounded}$, all finite subsets for
$\nde{ex\_learning}\not\to\nde{pac\_learning}$, and parities for
$\nde{vc\_dimension}\not\to\nde{sq\_dimension}$), as are the
\rel{strictly\_stronger} edges of \Cref{tab:strictly-stronger}.  In the main
text these rows carry \texttt{\textbackslash formal} footnotes naming their
verified counterparts.  The transformer instances (softmax attention as a
well-behaved VC class, and the certified generalization bound for an attention
block, \Cref{ch:generalization}) enter the inventory as verified witnesses
for the VC-dimension and generalization edges.
