...
al.\ universal-learning trichotomy.  Two suggestively named kernel declarations
are deliberately \emph{not} cited as companions, because their statements do
not prove what their names suggest: \leanname{rademacher_gen_bound}, which only
asserts the existence of a non-negative real rather than the Rademacher
generalization bound; and the kernel's \leanname{DSDim}, which is the
``all labelings realizable'' dimension (satisfying $\DSdim \le \Ndim$) rather
than the Daniely--Shalev-Shwartz DS dimension (which satisfies
$\Ndim \le \DSdim$).  Both are recorded as formalization gaps in
\Cref{ch:generalization} and \Cref{ch:extensions}.
\par\endgroup
