...
9 & Bib keys resolve & All values in node \code{bib_keys} arrays, \code{provenance.introduced_by}, \code{provenance.proved_by}, and edge \texttt{citation} fields that match the BibTeX key pattern \texttt{[A-Za-z]+[0-9]\{4\}[a-z]?} resolve against entries in the bibliography file. \\
...
13 & No \code{generalizes_unclassified} & No edges carry the legacy relation \code{generalizes_unclassified}.  All such edges must be reclassified as either \rel{restricts} (constraint removal) or \rel{extends\_grammar} (new primitives) before release. \\
\end{longtable}
...
\item \textbf{Missing keys} (FAIL).  BibTeX keys referenced in node \code{bib_keys}, \code{provenance.introduced_by}, \code{provenance.proved_by}, or edge \texttt{citation} fields that do not appear in \texttt{flt\_bibliography.bib}.  Each missing key is listed with all locations where it is referenced.
...
When all checks pass, \code{validate_graph.py} produces:
...
\noindent\textbf{What the checks certify.}  The thirteen checks establish
\emph{structural} integrity: valid JSON, unique and resolvable node
identifiers, schema-conformant relations and fields, matching counts, and
fully resolved citations.  They do \emph{not} certify the \emph{semantic}
correctness of an edge's target or of an analogy's obstruction label: a
well-formed but mis-targeted edge passes every check.  Semantic correctness is
maintained separately, by reconciling the graph against the proofs in the main
text, for instance, the corrections of \Cref{ch:extensions} to the
Natarajan/DS relationship and of \Cref{ch:computational} to the assumption
underlying computational hardness.
...
% ----------------------------------------------------------
\section{The Machine-Checked Layer}
\label{sec:machine-checked}

The validators above guard the \emph{graph}.  A second, independent layer
guards a subset of the \emph{theorems}: where a result has a companion in the
machine-checked Lean~4 development, the main text marks it with a
\texttt{\textbackslash formal} footnote naming the verified declaration.  Each
such declaration is \texttt{sorry}-free.  The companions fall in two
libraries: the learning-theory kernel (FLT) and the transformer library
(TLT).

\begingroup\sloppy\emergencystretch=2.5em
\smallskip
\noindent\emph{Learning-theory kernel (FLT), representative sample:}
\leanname{fundamental_theorem} (the fundamental theorem, \Cref{ch:pac});
\leanname{vcdim_bounds_rademacher_quantitative}, \leanname{finite_massart_lemma},
and \leanname{pac_bayes_finite} (\Cref{ch:generalization});
\leanname{fundamental_vc_compression} (\Cref{ch:compression});
\leanname{optimal_mistake_bound_eq_ldim} and the dimension-as-ordinal bridge
\leanname{withTopNatToOrdinal}, \leanname{vcdim_to_ordinal_vcdim}
(\Cref{ch:ordinals}); \leanname{vcdim_le_of_mistake_bounded},
\leanname{pac_not_implies_online}, and \leanname{ex_not_implies_pac}
(\Cref{ch:separations}); the multiclass and real-valued dimensions
\leanname{NatarajanDim}, \leanname{Pseudodimension}, \leanname{FatShatteringDim}
(\Cref{ch:extensions}); the minimax pair \leanname{finite_approx_minimax},
\leanname{mwu_approx_minimax} (\Cref{ch:analogies}); and
\leanname{vcdim_not_implies_hardness} (\Cref{ch:computational}).

\smallskip
\noindent\emph{Transformer library (TLT):}
\leanname{softAttention_wellBehaved} (softmax attention is a well-behaved VC
class, \Cref{ch:generalization}); \leanname{multiHeadArgmax_wellBehaved}
(\Cref{ch:compression}); and the certified generalization bounds
\leanname{attnHead_certified_generalization} for a single head and
\leanname{config_capacity_law}, \leanname{CertifiedGeneralization} for an
encoder stack (\Cref{ch:generalization}).

\smallskip
\noindent\textbf{Scope of the guarantee.}  The companion layer is partial by
design, and the main text is explicit about its boundary.  Many cited theorems
are \emph{not} machine-checked and are flagged as open formalization problems
where they appear, among them the exact $2^{O(d)}$ Moran--Yehudayoff
compression bound, the Brukhim et al.\ DS characterization of multiclass
learnability, the Kearns--Valiant cryptographic hardness, and the Bousquet et
al.\ universal-learning trichotomy.  The kernel declaration \leanname{rademacher_gen_bound} is deliberately
\emph{not} cited as a companion, because its statement does not prove what its
name suggests: it only asserts the existence of a non-negative real rather than
the Rademacher generalization bound.  A second gap is one of coverage rather
than naming: the kernel types the all-labelings shattering dimension
\leanname{FLT_Proofs.Complexity.Structures.StrongShatteringDim}, bounded above
by $\Ndim$, but not the Daniely--Shalev-Shwartz DS dimension, whose pseudo-cube
definition gives the reverse bound $\Ndim \le \DSdim$, so the multiclass
characterization is cited rather than verified.  Both are recorded as
formalization gaps in \Cref{ch:generalization} and \Cref{ch:extensions}.
\par\endgroup
