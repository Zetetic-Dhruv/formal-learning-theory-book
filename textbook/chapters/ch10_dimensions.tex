...
A learner facing a hypothesis class needs one piece of information before
anything else: does the class contain enough hypotheses to memorize arbitrary
finite configurations, or does it eventually run out of room?  If it can
memorize any finite set of any size, no finite sample can distinguish it from
the true concept, and learning is hopeless.  If it cannot, the sample
complexity is finite and learning is possible.  The VC dimension is the
precise measure of that boundary: it counts the largest set the class can
fully shatter, and the Sauer--Shelah lemma converts a finite VC dimension
into a polynomial bound on the class's complexity, which is the combinatorial
step the Fundamental Theorem turns into PAC guarantees.
...
the combinatorial backbone that supports the Fundamental Theorem.  Two steps
connect the geometry to learning: first, shattering pins down how much freedom
the class actually has; second, the Sauer--Shelah lemma shows that once that
freedom is finite, the class's entire complexity collapses to a polynomial.
...
\begin{tikzpicture}[scale=0.7,
  ...
  \end{scope}
\end{tikzpicture}
\caption[Three points shattered by halfplanes; no four are, so the VC dimension is three.]{All eight labelings of three non-collinear points are realized by a separating halfplane, so the points are shattered (\leanname{Shatters}) and $\VC \ge 3$: these are the eight witnesses that prove the lower bound. The lower panel closes the argument: any fourth point lies inside the convex hull of the other three (Radon), so the labeling that flips it against the rest has no halfplane separator, giving $\VC < 4$ and therefore $\VC = 3 = d+1$ for halfspaces in $d$ dimensions. The two panels together constitute a complete proof.}
...
growth function $\growth_\mathcal{H}(n)$ (\leanname{GrowthFunction}) rides the exponential ceiling
$2^n$ (dashed ray) as long as the class can still shatter a set of size $n$. At $n = d+1$, the first
size that cannot be shattered, the function peels off immediately into the Sauer--Shelah bound
$\sum_{i=0}^{d}\binom{n}{i}$ (\leanname{sauer_shelah_quantitative}), a polynomial of degree $d$,
and never returns to the exponential. There is no intermediate regime: the transition is a cliff.
This cliff is the combinatorial fact the Fundamental Theorem converts into PAC learnability: a
polynomial growth function gives uniform convergence; an exponential one prevents it.}
...
whose every root-to-leaf path is realized.  The distinction matters because
the online adversary acts sequentially, adapting to the learner's responses;
the VC game fixes all instances at once.  The Littlestone dimension is
therefore strictly larger, and knowing the gap is infinite (threshold
classifiers: $\VC = 1$, $\Ldim = \infty$) explains why online learnability
is a strictly stronger demand than PAC learnability.  We recall the definition for
reference.
...
$\Ldim = \infty$ (\Cref{ch:separations}).  The Littlestone dimension is larger
because it accounts for adaptive adversaries: the adversary in the online game
chooses instances sequentially, seeing the learner's responses, while the VC
dimension considers all labelings of a fixed set simultaneously.
...
be redefined.  The trouble is that real-valued functions can differ on a point
without disagreeing on any binary labeling, so the VC dimension misses
complexity that matters for regression and margin-based learning.
Two progressively refined notions capture what VC misses.  Pseudodimension
introduces thresholds that reduce the real-valued problem to a binary one;
fat-shattering adds a margin, making the notion scale-sensitive.  The margin
turns out to be essential: pseudodimension alone cannot characterize agnostic
real-valued learnability, but finite fat-shattering dimension at every scale
does, as the characterization theorem below shows.
...
elements?  This question, first posed in the 1980s, stayed open for thirty
years.  The field had a candidate, the Natarajan dimension, which
generalizes VC naturally and was supported by partial results for bounded
label sets.  It was wrong.  A class of Natarajan dimension one can be
unlearnable, and the gap between ``Natarajan dimension finite'' and
``PAC learnable'' is not a failure of proof strategy but a genuine
mathematical fact: the Natarajan dimension does not look at enough of
the labeling structure.  The resolution required a new dimension, the DS
dimension, defined through pseudo-cubes in the one-inclusion hypergraph,
and the proof arrived only in 2022.
...
The Natarajan dimension can be as small as one and the class still
unlearnable.  The witness uses $|Y| = \infty$: the Natarajan dimension
examines only pairs of opposing functions, so it constrains the binary slices
of the labeling space but not the total complexity once the label set grows
without bound.
...
\begin{historical}
\textbf{The one-inclusion hypergraph.}
...
PAC learner (the one-inclusion algorithm).  Brukhim et al.\ found that the
complexity of a multiclass class is measured by the high-dimensional
pseudo-cubes inside this hypergraph.  Pseudo-cubes are the multiclass analogue
of shattered sets: configurations whose labeling structure is rich enough to
prevent learning.
\end{historical}
...
\begin{tikzpicture}[
  ...
  \end{scope}
\end{tikzpicture}
\caption[The multiclass dimension ladder $\mathrm{StrongShattering} \le \Ndim \le \DSdim$.]{The
multiclass dimension ladder, with evidence grades encoded by line style. For binary classes
($|Y|=2$) the VC dimension characterizes PAC learnability (solid arrow: kernel-verified). The
natural generalization to $k \ge 3$ labels, the Natarajan dimension (\leanname{NatarajanDim}),
fails: a class with $\Ndim = 1$ over an infinite label set is not learnable, because the
Natarajan condition checks only pairs of opposing functions and misses the full labeling
complexity when $|Y| = \infty$ (Brukhim et al.\ 2022). The DS dimension, defined through
pseudo-cubes in the one-inclusion hypergraph, is the correct characterization (dashed arrow:
literature, not yet formalized). The diagram makes the correction visible: one arrow is
solid, one is dashed, and the red failure annotation marks exactly where the thirty-year
conjecture broke.}
...
\emph{strictly stronger} than PAC learnability; its
...
\label{op:ds-formal}
Formalize the DS dimension, defined through pseudo-cubes in the one-inclusion hypergraph (a default labeling perturbed by single-coordinate deviations), together with its order relation $\Ndim \le \DSdim$ and the Brukhim et al.\ proof that $\DSdim(\mathcal{H}) < \infty$ iff $\mathcal{H}$ is multiclass PAC learnable.
\end{openprob}
...
The figures above prove specific facts: that three non-collinear points are shattered by halfplanes,
that the growth function bends at $n = d+1$ and nowhere else, that Natarajan dimension one can
coexist with unlearnability.  Each exercise below asks you to take one of those specific proofs and
determine which step carries the load: remove it, and the argument fails.  That is the counterfactual
discipline.  Working through these is how the general claims become yours.

\begin{enumerate}[leftmargin=*,itemsep=3pt]
\item \textbf{Shattering is a full cube.} A separating halfplane in every cell of the $2^3$ grid says exactly $\mathcal{H}|_S = \{0,1\}^{|S|}$, which is the definition of shattering (\cref{def:shattering}, \leanname{Shatters}). Show that the eight halfplanes are the constructive content of ``$S$ is shattered'', so removing any one leaves the statement unproved.

\item \textbf{Radon caps the halfplane class at three.} Add a fourth point inside the convex hull of the other three and request the labeling that flips it against the rest. Show that this cell can never carry a separating line, so $\VC = 3$ for halfplanes in $\R^2$ (\Cref{ex:halfplanes-shatter}).

\item \textbf{Finiteness is learnability.} The finiteness the grid measures is what the Fundamental Theorem converts into PAC learnability: $\mathcal{H}$ is PAC learnable iff $\VC(\mathcal{H}) < \infty$ (\cref{thm:fundamental}, \leanname{vc_characterization}). Conclude that the halfplane class is learnable, and identify which edge of the picture is the verified equivalence.

\item \textbf{The same grid at every dimension.} Compute the VC dimension as the witness shape changes: $\VC = 1$ for halflines and $2$ for intervals on $\R$, $\VC = 4$ for axis-aligned rectangles in $\R^2$, and $\VC = d+1$ for halfspaces in $\R^d$ (\cite{BlumerEhrenfeuchtHausslerWarmuth1989}). Each instantiates \leanname{VCDim} with a different largest fillable $d$.

\item \textbf{Static shattering versus the adaptive game.} Shattering fixes one set $S$ and asks for all labelings at once, so $\VC(\mathcal{H}) \le \Ldim(\mathcal{H})$ (\cref{prop:vc-leq-ldim-dim}, \leanname{BranchWiseLittlestoneDim_ge_VCDim}). Show the gap is infinite for thresholds, whose static grid fills only to $d=1$ while their mistake tree is unbounded.

\item \textbf{Where the growth function bends.} The growth function rides $2^n$ exactly while a size-$n$ set is shattered, then at $n=d+1$ peels into $\sum_{i=0}^{d}\binom{n}{i}$ (\cref{def:growth-function}, \cref{lem:sauer-shelah}, \leanname{sauer_shelah_quantitative}). Verify the drop is immediate and permanent, with no $n$ of intermediate growth, and relocate the bend by recomputing $\VC$ for the classes of exercise~4.

\item \textbf{The ray that never bends.} For a class shattering arbitrarily large sets ($\VC = \infty$), such as all finite and cofinite subsets of $\N$, show $\growth_\mathcal{H}(n) = 2^n$ for every $n$, so Sauer--Shelah does not apply and the class is not PAC learnable (\cref{thm:fundamental}, \leanname{vc_characterization}).

\item \textbf{Why the Natarajan dimension fails.} Exhibit the regime where the obvious multiclass lift breaks: there is a class with $\Ndim(\mathcal{H}) = 1$ over an infinite label set that is not PAC learnable (\cref{thm:natarajan-failure}, \cite{BrukhimCarmonDinurMoranYehudayoff2022}). Explain why $|Y| = \infty$ is required, given that $\Ndim$ reads only pairs of opposing functions.

\item \textbf{The correct characterization.} A class is multiclass PAC learnable iff its DS dimension is finite (\cref{thm:ds-char}, \cite{BrukhimCarmonDinurMoranYehudayoff2022}), with $\Ndim \le \DSdim$. Construct the pseudo-cube definition of $\DSdim$ explicitly, verify $\Ndim \le \DSdim$ on the class with $\Ndim = 1$ and $\DSdim = \infty$, and state what formalizing this dimension in Lean would require (\cref{op:ds-formal}).
\end{enumerate}
