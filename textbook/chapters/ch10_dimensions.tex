...
\begin{tikzpicture}[scale=0.7,
  ...
  \end{scope}
\end{tikzpicture}
\caption[Three points shattered by halfplanes; no four are, so the VC dimension is three.]{Each of the eight labelings of three non-collinear points is realized by a separating halfplane, so the points are shattered (\leanname{Shatters}) and $\VC \ge 3$. The lower panel shows that no four points can be shattered, since one always lies in the convex hull of the others (Radon), giving $\VC = 3 = d+1$ for halfspaces in $d$ dimensions.}
...
growth function $\growth_\mathcal{H}(n)$ (\leanname{GrowthFunction}) equals $2^n$ (the dashed ray)
exactly while the class shatters a set of that size, $n \le d = \VC(\mathcal{H})$. At $n = d+1$
it peels permanently into the Sauer--Shelah bound $\sum_{i=0}^{d}\binom{n}{i}$
(\leanname{sauer_shelah_quantitative}), a polynomial of degree~$\VC$; the drop is immediate.
This collapse is the combinatorial step the Fundamental Theorem turns into PAC learnability.}
...
years.  Its resolution demanded a new combinatorial dimension, because the
obvious candidate, the Natarajan dimension, turned out not to characterize
learnability.
...
multiclass dimension ladder. For binary classes ($|Y|=2$) the VC dimension characterizes PAC
learnability: a class is learnable iff its VC dimension is finite. The obvious generalization to
$k \ge 3$ labels, the Natarajan dimension (\leanname{NatarajanDim}), fails: a class of Natarajan
dimension one over an infinite label set is not learnable (Brukhim et al.\ 2022). The genuine
characterization is the DS dimension, defined through pseudo-cubes in the one-inclusion hypergraph.}
...
Each exercise takes the concrete halfplane, growth-function, or multiclass-ladder picture as a special case of a wider family and asks you to supply the general statement.
