...
The soft-attention concept class $\mathcal{A}_{d,n_K}$ is a well-behaved VC-measurable target: every concept in it is measurable, and the class satisfies the regularity condition under which \cref{thm:vc-char,thm:fundamental} were proved.\formal{TLT.Bridge.softAttention_wellBehaved} Consequently the equivalence web applies to it verbatim: $\mathcal{A}_{d,n_K}$ is PAC learnable if and only if $\VC(\mathcal{A}_{d,n_K}) < \infty$, with the sample complexity of \cref{thm:vc-char}.
\end{prop}
...
The Fundamental Theorem is representation-independent (\Cref{ch:objects}), yet \cref{thm:kearns-valiant} and the computational interlude show the proper/improper distinction is computationally sharp.  Exhibit an invariant of $\mathcal{H}$ that controls the proper-versus-improper sample or time gap, and characterize the classes for which proper PAC learning is no costlier than improper.
\end{openprob}
