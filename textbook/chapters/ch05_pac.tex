...
learning, Gold-style identification, universal learning) earns its meaning by
comparison: what it shares with what is proved here, and where it breaks away.

Before this chapter, the reader has no answer to the most basic question a
learner can ask: \emph{is there enough signal in this class for any algorithm
to succeed?}  The question has weight.  Without an answer, one cannot tell
whether a learning failure comes from the algorithm, the data, or from the
class being fundamentally unlearnable by any procedure.  This chapter provides
the answer, and its form is surprising: learnability is controlled by a single
combinatorial quantity, the VC dimension, which turns out to be the hub of nine
independently motivated conditions that all turn out to be the same condition.
The destination is the Fundamental Theorem of Statistical Learning.
...
\item \textbf{The Fundamental Theorem} (\Cref{sec:fundamental}): all nine
  equivalent conditions and the equivalence web.
...
PAC learnability is characterized by a single combinatorial condition:
...
\[
  ...
  = O\!\left(\frac{d \log(d/\varepsilon) + \log(1/\delta)}{\varepsilon}\right).
\]
\end{thm}

The proof proceeds in four stages, each a separate lemma.
...
conditions.  The Fundamental Theorem packages them all.
...
\end{scope}
\end{tikzpicture}
\caption[The equivalence web of the Fundamental Theorem: VC dimension as the hub]{Nine conditions
arrive from different directions, PAC learnability, uniform convergence, growth function collapse,
compression, Rademacher complexity, and more, and collapse into a single equivalence class.  Finite VC
dimension sits at the hub because it is what all of them are measuring (\leanname{fundamental_theorem}).
Solid spokes are verified in the formal development; dashed spokes are classical results in the cited
literature.  The single outside node, finite Littlestone dimension, is not an equivalence: it implies
PAC learnability (via finite VC dimension), but the converse fails.  Thresholds on $\R$ have $\VC=1$
yet $\Ldim=\infty$, which is the first crack where the PAC and online landscapes separate
(\Cref{ch:separations}).}
...
Where does the gap come from?  It is not an artifact of the proof technique:
no argument recovers $1/\varepsilon$ in the agnostic case.  The reason is
...
infinite and VC dimension replaces $\log|\mathcal{H}|$.  Realizability changes
the information structure, which is what lets the realizable rate outrun the
...
\begin{enumerate}[leftmargin=*,itemsep=3pt]
\item \textbf{VC dimension is the right measure.}  It sits at the center of a
  seven-way equivalence.
...
The equivalence web in \Cref{fig:equivalence-web} is not a finished object: its spokes can be sharpened,
its hub replaced by a tighter representing object, its outside nodes brought inside or shown to be
genuinely external.  Each exercise takes one of those moves seriously.  The web is the argument; these
are the next steps in it.

\begin{enumerate}[leftmargin=*,itemsep=3pt]
\item \textbf{The Rademacher face.} Add finite Rademacher complexity as a satellite of the hub, and show it is equivalent to PAC learnability: PAC learnability and uniform vanishing of the Rademacher complexity coincide (\leanname{fundamental_rademacher}).

\item \textbf{The compression face.} Add finite sample compression with side information as a satellite and show it joins the equivalence class. The deep direction, that finite VC dimension yields a compression scheme, is the Moran--Yehudayoff theorem (\leanname{fundamental_vc_compression}).

\item \textbf{The quantitative ring.} Replace the qualitative hub equivalence by the two-sided sample complexity bound, which pins both an upper bound from any PAC witness and a lower bound from VC dimension (\leanname{pac_sample_complexity_sandwich}). Show the spoke now carries a rate where before it carried only an equivalence.

\item \textbf{The agnostic price.} Move from realizable to agnostic PAC. The equivalence to realizable PAC is classical (\cite{ShalevShwartzBenDavid2014}); show that the sample complexity nonetheless carries the $\varepsilon^{2}$ price, whose exact constant is open (\cref{op:sample-constant}).

\item \textbf{The Littlestone strict edge.} Attempt to upgrade the outside node to an equivalence. Show it refuses to close: finite Littlestone dimension implies PAC learnability but the converse fails, witnessed by thresholds on $\R$ with $\VC=1$ and $\Ldim=\infty$ (\leanname{online_strictly_stronger_pac}).

\item \textbf{The compression size gap.} Refine the compression face from $2^{O(d)}$ to $O(d)$. The finiteness direction and a quadratic reverse bound $\VC \le 2(k+1)^{2}-1$ are known (\leanname{compression_bounds_vcdim}); the Littlestone--Warmuth $O(d)$ conjecture and the removal of side information are open (\cref{op:compression-od}).

\item \textbf{The multiclass web.} Redraw the figure for multiclass or real-valued hypotheses with the DS dimension as hub. No multiclass fundamental theorem is known (\cref{op:multiclass-fundamental}); determine which of the nine equivalences survive the change of dimension.

\item \textbf{One representing object.} Replace the several spokes by a single claim that the conditions are evaluations of one representable predicate with VC dimension as its representing object. The faces are assembled as one bundle (\leanname{fundamental_theorem}); state the universal property that would unify them, which is open.
\end{enumerate}
