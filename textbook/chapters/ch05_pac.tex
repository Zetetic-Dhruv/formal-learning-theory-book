...
requirement.\formal{FLT_Proofs.Criterion.PAC.PACLearnable}
\end{defn}
...
\item $\VC(\mathcal{H}) < \infty$.\formal{FLT_Proofs.Theorem.PAC.vc_characterization}
\end{enumerate}
...
\label{sec:uniform-convergence-pac}
...
is a PAC learner for $\mathcal{H}$.\formal{FLT_Proofs.Complexity.Generalization.uc_imp_pac}
\end{prop}
...
If $\VC(\mathcal{H}) = \infty$, then $\mathcal{H}$ is not PAC learnable.\formal{FLT_Proofs.Complexity.Generalization.vcdim_infinite_not_pac}
\end{thm}
...
The VC characterization (\Cref{thm:vc-char-pac}) is the core equivalence.  But the
...
\item \label{F:finvc} $\VC(\mathcal{H}) < \infty$.\formal{FLT_Proofs.Theorem.PAC.fundamental_theorem}
\item \label{F:fingrowth} $\growth_\mathcal{H}(m) < 2^m$ for some $m$.
\item \label{F:compression} $\mathcal{H}$ has a finite compression scheme (with side information).\formal{FLT_Proofs.Theorem.PAC.fundamental_vc_compression}
...
\begin{tikzpicture}[
    >={Stealth[length=2mm]},
    sat/.style={draw, semithick, rounded corners, minimum width=2.2cm, align=center,
      inner sep=2.5pt, font=\scriptsize, fill=defblue!8},
    hub/.style={draw, very thick, rounded corners, minimum width=2.4cm, align=center,
      inner sep=3pt, font=\small, fill=thmred!12},
    classical/.style={draw, semithick, densely dashed, rounded corners, minimum width=2.2cm,
      align=center, inner sep=2.5pt, font=\scriptsize, fill=white},
    strict/.style={draw=edgeorange!80!black, semithick, rounded corners, minimum width=2.2cm,
      align=center, inner sep=2.5pt, font=\scriptsize, fill=edgeorange!8},
    sp/.style={defblue, semithick},
    spcore/.style={thmred, very thick},
    spcomp/.style={sepgreen!55!black, semithick},
    spcl/.style={defblue!65, semithick, densely dashed},
    spstrict/.style={edgeorange!80!black, semithick, densely dashed},
]
% --- hub: the representing object ---
\node[hub] (vc) at (0,0) {$\VC(\mathcal{H})<\infty$~\ref{F:finvc}\\[-2pt]{\scriptsize the hub}};
% --- verified faces (solid spokes) ---
\node[sat] (pac) at (0,3.6) {PAC learnable~\ref{F:pac}\\{\tiny\leanname{vc_characterization}}};
\node[sat] (uc) at (2.55,2.55) {Uniform conv.~\ref{F:uc}\\{\tiny\leanname{uc_imp_pac}}};
\node[sat] (rad) at (3.6,0) {Rademacher\\{\tiny\leanname{fundamental_rademacher}}};
\node[sat, fill=sepgreen!10] (comp) at (2.55,-2.55) {Compression~\ref{F:compression}\\{\tiny\leanname{fundamental_vc_compression}}};
\node[sat] (growth) at (0,-3.6) {Growth $\growth(m)<2^m$~\ref{F:fingrowth}\\{\tiny in \leanname{fundamental_theorem}}};
% --- classical faces (dashed spokes, not kernel conjuncts) ---
\node[classical] (erm) at (-2.55,-2.55) {ERM is PAC~\ref{F:erm}\\{\tiny classical \cite{ShalevShwartzBenDavid2014}}};
\node[classical] (agpac) at (-3.6,0) {Agnostic PAC~\ref{F:agpac}\\{\tiny classical \cite{ShalevShwartzBenDavid2014}}};
\node[classical] (fin) at (-2.55,2.55) {Finiteness~\ref{F:finiteness}\\{\tiny classical \cite{BlumerEhrenfeuchtHausslerWarmuth1989}}};
% --- strict outside node ---
\node[strict] (ldim) at (5.5,-2.9) {$\Ldim<\infty$~\ref{F:online}\\{\scriptsize\itshape strict}\\{\tiny\leanname{online_strictly_stronger_pac}}};
% --- spokes ---
\draw[spcore] (vc) -- (pac);
\draw[sp] (vc) -- (uc);
\draw[sp] (vc) -- (rad);
...
\draw[spcl] (vc) -- (agpac);
\draw[spcl] (vc) -- (fin);
\draw[spstrict, ->] (ldim) -- node[font=\tiny, text=edgeorange!80!black, fill=white, inner sep=1pt, pos=0.58]{converse fails} (vc);
% --- the core spoke carries the quantitative two-sided bound ---
\node[font=\tiny, text=thmred!75!black, fill=white, inner sep=1pt] at (0,1.65) {\leanname{pac_sample_complexity_sandwich}};
% --- legend ---
\begin{scope}[shift={(-4.9,-4.35)}, font=\tiny]
  \draw[spcore] (0,0) -- (0.4,0); \node[anchor=west, inner sep=1pt] at (0.45,0) {core VC$\Leftrightarrow$PAC};
  \draw[sp] (2.7,0) -- (3.1,0); \node[anchor=west, inner sep=1pt] at (3.15,0) {verified equivalence};
  \draw[spcomp] (5.5,0) -- (5.9,0); \node[anchor=west, inner sep=1pt] at (5.95,0) {compression};
  \draw[spcl] (0,-0.34) -- (0.4,-0.34); \node[anchor=west, inner sep=1pt] at (0.45,-0.34) {classical, cited (not in kernel)};
  \draw[spstrict] (4.0,-0.34) -- (4.4,-0.34); \node[anchor=west, inner sep=1pt] at (4.45,-0.34) {strict, one-way};
\end{scope}
\end{tikzpicture}
\caption[The equivalence web of the Fundamental Theorem: VC dimension as the hub]{The Fundamental
Theorem as a confluence: several a priori unrelated descriptions of ``learnable'' collapse into one
equivalence class, with finite VC dimension as the central hub that represents the class. Each solid
spoke is an equivalence the verified kernel certifies: PAC learnability (\leanname{vc_characterization}),
the uniform convergence property, finite Rademacher complexity (\leanname{fundamental_rademacher}), the
polynomial growth bound, and finite sample compression with side information
(\leanname{fundamental_vc_compression}, Moran--Yehudayoff); the two-sided sample complexity bound on the
core spoke is \leanname{pac_sample_complexity_sandwich}, and the whole bundle is
\leanname{fundamental_theorem}. Dashed spokes are classical but outside the present kernel: agnostic PAC
learnability, ERM as a PAC learner, and the finiteness lifting
(\cite{BlumerEhrenfeuchtHausslerWarmuth1989,ShalevShwartzBenDavid2014}). The single outside node, finite
Littlestone dimension, is joined by one dashed strict edge: it implies finite VC dimension, hence PAC
learnability, but the converse fails (thresholds on $\R$ have $\VC=1$ and $\Ldim=\infty$;
\leanname{online_strictly_stronger_pac}), the one place where the equivalence breaks into the online
separation of \Cref{ch:separations}.}
...
\item $\ref{F:finvc} \Leftrightarrow \ref{F:fingrowth}$:  By the Sauer--Shelah
  lemma (\Cref{lem:sauer-shelah-pac}), $\VC(\mathcal{H}) = d$ implies
...
$d$ has a sample compression scheme of size $O(d)$.  What Moran and
Yehudayoff~\cite{MoranYehudayoff2016} established in 2016 is the
\emph{finiteness} direction: every class of finite VC dimension has a
compression scheme of size $2^{O(d)}$, which closes the equivalence
\ref{F:finvc} $\Leftrightarrow$ \ref{F:compression} of the Fundamental
Theorem.  The original $O(d)$ conjecture remains \emph{open}; the
exponential gap between $d$ and $2^{O(d)}$, and the question of whether
side information can be removed, are among the field's outstanding
structural questions (\cref{op:compression-od}).
\end{historical}
...
\subsection{A modern instance: attention is PAC learnable}
\label{sec:attention-pac}

The Fundamental Theorem reaches well beyond the geometric classes of \cref{ex:pac-rectangles}: its hypotheses are met by the attention mechanism at the centre of the transformer.  Fix an input dimension $d$ and a number of keys $n_K$.  The \emph{soft-attention concept class} $\mathcal{A}_{d,n_K}$ consists of the binary concepts on $\R^d$ obtained by softmax-scoring an input against $n_K$ keys and thresholding the routed output.

\begin{prop}[Soft attention meets the Fundamental Theorem's hypotheses]
\label{prop:attention-pac}
The soft-attention concept class $\mathcal{A}_{d,n_K}$ is a well-behaved VC-measurable target: every concept in it is measurable, and the class satisfies the regularity condition under which \cref{thm:vc-char-pac,thm:fundamental} were proved.\formal{TLT.Bridge.softAttention_wellBehaved} Consequently the equivalence web applies to it verbatim: $\mathcal{A}_{d,n_K}$ is PAC learnable if and only if $\VC(\mathcal{A}_{d,n_K}) < \infty$, with the sample complexity of \cref{thm:vc-char-pac}.
\end{prop}

\begin{proof}
The softmax routing map is continuous in the input, hence Borel; the score-comparison cells that select the attended key are measurable, so the joint map $(\text{parameters}, x) \mapsto$ concept value is measurable.\formal{TLT_Proofs.Routing.MeasurableScoreRouting.FiniteScoreRouterCode.route_measurable} This is exactly the Krapp--Wirth well-behaved VC-measurable target hypothesis carried silently by the measure-theoretic steps of \cref{sec:vc-char}.  With that hypothesis discharged, the chain Sauer--Shelah $\to$ uniform convergence $\to$ ERM applies to $\mathcal{A}_{d,n_K}$ unchanged, as does the converse (\cref{thm:vc-converse}); the equivalences of \cref{thm:fundamental} therefore hold for the attention class.
\end{proof}

What \cref{prop:attention-pac} does \emph{not} settle is whether the VC dimension is finite, and that is the substantive question.  For a fixed architecture it is finite: the concepts of $\mathcal{A}_{d,n_K}$ are cut out by a bounded number of smooth inequalities in finitely many parameters, and such parametric families have finite VC dimension by the parameter-counting bounds of Goldberg--Jerrum and Anthony--Bartlett~\cite{AnthonyBartlett1999}.  A fixed attention head is therefore PAC learnable.  The \emph{sharp} VC dimension and sample complexity as functions of $d$, $n_K$, and the score temperature are not pinned down by the measurability result; determining them (in particular whether the dependence on $n_K$ is logarithmic or linear) is open (\cref{op:attention-vc}).
...
infinite and VC dimension replaces $\log|\mathcal{H}|$.  Realizability does
real work here beyond easing the bookkeeping: it changes the information
structure qualitatively, which is what lets the realizable rate outrun the
agnostic one for an infinite class.
\end{remark}
...
The upper bound of \Cref{thm:vc-char-pac} gives $m = O(d/\varepsilon)$ in the
...
\]
for $\varepsilon \leq 1/8$ and $\delta \leq 1/7$.\formal{FLT_Proofs.Theorem.PAC.pac_lower_bound}
\end{thm}
...
\]
where $D_c$ is the uniform distribution on $X$ with target $c$.\formal{FLT_Proofs.Complexity.Generalization.nfl_per_sample_shattered}
\end{thm}
...
PAC learnable & Valiant (1984) & \Cref{thm:vc-char-pac} \\
...
\begin{openprob}[The $O(d)$ sample-compression conjecture, \textsc{ku}]
\label{op:compression-od}
The Fundamental Theorem's compression equivalence \ref{F:compression} is the finiteness statement: finite VC dimension yields a compression scheme of size $2^{O(d)}$ (with side information).\formal{FLT_Proofs.Theorem.PAC.fundamental_vc_compression} Prove or refute the Littlestone--Warmuth conjecture that every class of VC dimension $d$ admits a scheme of size $O(d)$ \emph{without} side information; the gap is isolated as \leanname{FLT_Proofs.Complexity.Structures.Open_NoInfoCompressionStrengthening} and recurs from \Cref{ch:objects}.
\end{openprob}

\begin{openprob}[The VC dimension of attention, \textsc{uk}]
\label{op:attention-vc}
\Cref{prop:attention-pac} reduces ``is soft attention PAC learnable?'' to ``is its VC dimension finite?'', and parameter counting answers yes for fixed architectures.  Determine the sharp VC dimension and realizable sample complexity of $\mathcal{A}_{d,n_K}$ as functions of $d$, $n_K$, and the score temperature, deciding whether the dependence on the number of keys $n_K$ is logarithmic or linear, using the chaining and covering-number machinery proved for attention (\leanname{TLT_Proofs.Capacity.Chaining.RademacherSymmetrization}).
\end{openprob}

\begin{openprob}[Optimal constants in the sample complexity, \textsc{ku}]
\label{op:sample-constant}
\Cref{thm:vc-char-pac,thm:pac-lower} bracket the realizable sample complexity as $\Theta(d/\varepsilon)$, and Hanneke~\cite{Hanneke2016} removed the logarithmic factor.  Determine the exact leading constant in both the realizable $\Theta(d/\varepsilon)$ and agnostic $\Theta(d/\varepsilon^2)$ rates, matching the verified lower bound \leanname{FLT_Proofs.Theorem.PAC.pac_lower_bound} to a constant-tight upper bound.
\end{openprob}

\begin{openprob}[A characterization of efficient PAC learnability, \textsc{ku}]
\label{op:efficient-pac}
\Cref{thm:kearns-valiant-pac} exhibits classes that are PAC learnable but not efficiently so.  The Fundamental Theorem characterizes information-theoretic learnability by a single dimension; give a combinatorial or complexity-theoretic characterization of which finite-VC classes are \emph{polynomial-time} PAC learnable, or prove (under standard assumptions) that no purely combinatorial characterization exists.
\end{openprob}

\begin{openprob}[Unifying the Rademacher face, \textsc{uk}]
\label{op:rademacher-unify}
The development proves a Rademacher-complexity face of the Fundamental Theorem: PAC learnability is equivalent to uniform vanishing of the Rademacher complexity.\formal{FLT_Proofs.Theorem.PAC.fundamental_rademacher} Integrate it into the equivalence web of \cref{fig:equivalence-web} as a tenth condition with explicit quantitative links to the VC and compression faces, and formalize the two-sided VC$\leftrightarrow$Rademacher comparison.
\end{openprob}

\begin{openprob}[The proper/improper price in PAC, \textsc{ku}]
\label{op:proper-improper-pac}
The Fundamental Theorem is representation-independent (\Cref{ch:objects}), yet \cref{thm:kearns-valiant-pac} and the computational interlude show the proper/improper distinction is computationally sharp.  Exhibit an invariant of $\mathcal{H}$ that controls the proper-versus-improper sample or time gap, and characterize the classes for which proper PAC learning is no costlier than improper.
\end{openprob}

\begin{openprob}[Distribution-dependent learnability, \textsc{ku}]
\label{op:distribution-dependent}
Exercise~2 shows finite VC dimension is not necessary for fixed-distribution PAC learning.  Characterize distribution-dependent PAC learnability by a metric-entropy condition on $\mathcal{H}$ under $L^1(D)$, and determine exactly how the distribution-free VC characterization is recovered as the supremum over $D$.
\end{openprob}

\begin{openprob}[The multiclass Fundamental Theorem, \textsc{uk}]
\label{op:multiclass-fundamental}
For multiclass and real-valued prediction the VC dimension is replaced by the DS, Natarajan, and fat-shattering dimensions (\Cref{ch:dimensions,ch:extensions}).  Determine which of the nine equivalences of \cref{thm:fundamental} survive, and in particular whether a clean five-way equivalence holds with the DS dimension in place of $\VC$.
\end{openprob}

\begin{openprob}[The agnostic price for attention, \textsc{uk}]
\label{op:attention-agnostic}
Combine \cref{thm:agnostic-gap,prop:attention-pac}: does the $\varepsilon$-vs-$\varepsilon^2$ agnostic price hold for the soft-attention class, and is the classification-noise robustness of attention learning (the analogue of \cref{thm:noise-robust} for $\mathcal{A}_{d,n_K}$) quantifiable in terms of the score temperature?
\end{openprob}

\begin{openprob}[The one-inclusion graph predictor, \textsc{ku}]
\label{op:one-inclusion}
The optimal expected-error predictor in the realizable case is the one-inclusion graph algorithm, and its analysis underlies both the Hanneke bound and the compression direction.  Formalize the one-inclusion graph predictor and prove its $\le d/(m+1)$ leave-one-out error bound, connecting it to the compression scheme of \cref{def:compression} and the open $O(d)$ conjecture (\cref{op:compression-od}).
\end{openprob}


\subsection*{Counterfactual exercises: generalizing Figure~\ref{fig:equivalence-web}}

Figure~\ref{fig:equivalence-web} is a hub with a mostly solid rim: one central object stands for the class, and the spokes that join the equivalence class are, by and large, theorems the verified kernel already certifies (solid), with a minority borrowed from the literature or genuinely one-way (dashed). Each exercise below adds, replaces, or refines a spoke. Working in that grammar, place the result in one of three roles: an \emph{instance} (a further face that joins the verified class), a \emph{boundary} (a place where the equivalence turns strict or a constant gets pinned), or a \emph{frontier} (a dashed edge where the kernel stops and the open problem begins). The interesting fact about this particular web is how few of the second and third kind there are: the hub is solid almost the whole way around, and the open work sits at three or four named edges.

\begin{enumerate}[leftmargin=*,itemsep=3pt]
\item \textbf{The Rademacher face.} Add finite Rademacher complexity as a satellite of the hub. This enters as an instance, and a solid one: PAC learnability and the uniform vanishing of Rademacher complexity are equivalent in the kernel (\leanname{fundamental_rademacher}), so the new spoke is drawn solid from the start.

\item \textbf{The compression face.} Add finite sample compression with side information as a satellite. Argue that it too joins as an instance. The deep direction, finite VC dimension yields a compression scheme, is the Moran--Yehudayoff theorem and is verified (\leanname{fundamental_vc_compression}), so the whole spoke is solid even though its proof is the hardest in the web.

\item \textbf{The quantitative ring.} Replace the qualitative hub edge by the two-sided sample complexity bound. This is a boundary, and it sharpens the hub rather than extending it: the kernel pins both an upper bound from any PAC witness and a lower bound from VC dimension (\leanname{pac_sample_complexity_sandwich}), so the spoke now carries a rate where before it carried only an equivalence.

\item \textbf{The agnostic price.} Move from realizable to agnostic PAC and ask for the sample complexity. Here the boundary lands on a dashed face: the equivalence to realizable PAC is classical and drawn dashed (\cite{ShalevShwartzBenDavid2014}, outside the kernel), and the line it draws is the $\varepsilon^{2}$ price, whose exact constant is open (\cref{op:sample-constant}).

\item \textbf{The Littlestone strict edge.} Replace the outside node's dashed edge by an attempted equivalence. This is the web's principal boundary, and it refuses to close: finite Littlestone dimension implies PAC learnability but the converse fails, witnessed by thresholds on $\R$ with $\VC=1$ and $\Ldim=\infty$ (\leanname{online_strictly_stronger_pac}, \leanname{pac_not_implies_online}), so the edge stays dashed and one-way.

\item \textbf{The growth-function rung.} Insert ``$\growth_{\mathcal H}(m)<2^{m}$ for some $m$'' as a satellite. Read this as an instance whose inner curve is dashed: the equivalence to finite VC dimension is a verified conjunct of the bundle (\leanname{fundamental_theorem}), but the quantitative Sauer--Shelah envelope $(em/d)^{d}$ it rides on is classical (\cite{Sauer1972,Shelah1972,VapnikChervonenkis1971}). The spoke is solid; only its shape annotation stays dashed.

\item \textbf{The compression size gap.} Refine the compression face from $2^{O(d)}$ to $O(d)$. Now the spoke runs out into a frontier: the kernel proves only the finiteness direction and a quadratic reverse bound, $\VC \le 2(k+1)^{2}-1$ (\leanname{compression_bounds_vcdim}), so the Littlestone--Warmuth $O(d)$ conjecture and the removal of side information remain a dashed open spoke (\cref{op:compression-od}).

\item \textbf{The ERM face.} Add ``empirical risk minimization is a PAC learner'' as a satellite. This one is a frontier on the algorithmic side: the kernel types and proves the realizable-ERM machinery (\leanname{ermLearn_in_H}, \leanname{erm_consistent_realizable}), but there is no named ERM-is-PAC equivalence theorem, so the spoke is drawn dashed and cited (\cite{ShalevShwartzBenDavid2014}) pending promotion to a bundle conjunct.

\item \textbf{The multiclass web.} Redraw the whole figure for multiclass or real-valued hypotheses with the DS dimension as hub. This is a frontier of a more violent kind, one that re-types every spoke at once: no multiclass fundamental theorem is in the kernel (\cref{op:multiclass-fundamental}), so the entire generalized star would be drawn dashed, an open analogue of the binary confluence.

\item \textbf{One representing object.} Replace the several solid spokes by a single claim that the conditions are evaluations of one representable predicate with VC dimension as its representing object. This is the most general frontier the web has, and it is open: the kernel has assembled the faces as one bundle (\leanname{fundamental_theorem}) but proves no universal property unifying them, so the meta-edge that all spokes are one functor is the figure's single dashed frontier over an otherwise solid hub.
\end{enumerate}
