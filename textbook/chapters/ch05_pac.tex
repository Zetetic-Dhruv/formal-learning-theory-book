...
Each exercise perturbs the web. Show, in the figure's grammar (one central hub that represents the class, solid spokes for equivalences the verified kernel certifies and dashed spokes for what is borrowed from the literature or genuinely one-way), that the generalized figure subsumes it as an \emph{instance} (another face that joins the equivalence class), a \emph{boundary} (where the equivalence becomes strict or a constant is fixed), or a \emph{frontier} (a dashed, open edge).

\begin{enumerate}[leftmargin=*,itemsep=3pt]
\item \textbf{The Rademacher face.} Add finite Rademacher complexity as a satellite of the hub. Show this is an instance: it is a fully verified spoke, since PAC learnability and the uniform vanishing of Rademacher complexity are equivalent in the kernel (\leanname{fundamental_rademacher}), so the face enters solid, not dashed.

\item \textbf{The compression face.} Add finite sample compression with side information as a satellite. Show this is an instance: the deep direction, finite VC dimension yields a compression scheme, is the Moran--Yehudayoff theorem and is verified (\leanname{fundamental_vc_compression}), so the whole spoke is solid even though its proof is the hardest in the web.

\item \textbf{The quantitative ring.} Replace the qualitative hub edge by the two-sided sample complexity bound. Show this is a boundary that sharpens the hub: the kernel pins both an upper bound from any PAC witness and a lower bound from VC dimension (\leanname{pac_sample_complexity_sandwich}), so the spoke carries a rate, not just an equivalence.

\item \textbf{The agnostic price.} Move from realizable to agnostic PAC and ask for the sample complexity. Show this is a boundary on a dashed face: the equivalence to realizable PAC is classical and drawn dashed (\cite{ShalevShwartzBenDavid2014}, not in the kernel), and the boundary it draws is the $\varepsilon^{2}$ price, whose exact constant is open (\cref{op:sample-constant}).

\item \textbf{The Littlestone strict edge.} Replace the outside node's dashed edge by an attempted equivalence. Show this is the web's principal boundary: finite Littlestone dimension implies PAC learnability but the converse fails, witnessed by thresholds on $\R$ with $\VC=1$ and $\Ldim=\infty$ (\leanname{online_strictly_stronger_pac}, \leanname{pac_not_implies_online}), so the edge must stay dashed and one-way.

\item \textbf{The growth-function rung.} Insert ``$\growth_{\mathcal H}(m)<2^{m}$ for some $m$'' as a satellite. Show this is an instance with a dashed inner curve: the equivalence to finite VC dimension is a verified conjunct of the bundle (\leanname{fundamental_theorem}), but the quantitative Sauer--Shelah envelope $(em/d)^{d}$ it rides on is classical (\cite{Sauer1972,Shelah1972,VapnikChervonenkis1971}), so the spoke is solid while its shape annotation is dashed.
...
\item \textbf{The ERM face.} Add ``empirical risk minimization is a PAC learner'' as a satellite. Show this is a frontier on the algorithmic side: the kernel types and proves the realizable-ERM machinery (\leanname{ermLearn_in_H}, \leanname{erm_consistent_realizable}), but there is no named ERM-is-PAC equivalence theorem, so the spoke is drawn dashed and cited (\cite{ShalevShwartzBenDavid2014}) pending promotion to a bundle conjunct.

\item \textbf{The multiclass web.} Redraw the whole figure for multiclass or real-valued hypotheses with the DS dimension as hub. Show this is a frontier that re-types every spoke: no multiclass fundamental theorem is in the kernel (\cref{op:multiclass-fundamental}), so the entire generalized star would be dashed, an open analogue of the binary confluence.

\item \textbf{One representing object.} Replace the several solid spokes by a single claim that the conditions are evaluations of one representable predicate with VC dimension as its representing object. Show this is the web's most general open frontier: the kernel has assembled the faces as one bundle (\leanname{fundamental_theorem}) but proves no universal property unifying them, so the meta-edge that all spokes are one functor is the figure's single dashed frontier over an otherwise solid hub.
\end{enumerate}
