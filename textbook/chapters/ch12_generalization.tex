...
Training error goes to zero. That is easy: memorize the data. The hard question is whether a trained hypothesis performs well on examples it has never seen. Without a quantitative answer, every reported accuracy is an artifact of the test set, and every deployed model is a bet made without odds.

\Cref{ch:pac} established \emph{that} finite VC dimension characterizes PAC learnability. \Cref{ch:compression} refined the quantitative dependence through compression. This chapter asks a sharper question: given a specific learning algorithm~$A$ running on a specific training sample~$S$, how large can the gap between the true risk of~$A(S)$ and its training error actually be?

Five distinct research traditions answer this question, all bounding the same quantity but reading different information about the learner:

\begin{enumerate}[leftmargin=*, itemsep=3pt]
\item \textbf{VC/uniform convergence} reads the combinatorial richness
  of the hypothesis class through the growth function.
\item \textbf{Rademacher complexity} refines this to a data-dependent
  measure of how well the class correlates with random noise on the actual
  training sample.
\item \textbf{PAC-Bayes bounds} measure the KL divergence of a learned
  posterior from a data-independent prior.
\item \textbf{Algorithmic stability} measures how much the output changes
  when a single training example is replaced.
\item \textbf{Margin theory} measures the confidence of predictions
  and connects to scale-sensitive dimensions.
\end{enumerate}

\noindent
A sixth thread, information-theoretic bounds, measures the mutual
information between the training sample and the learned hypothesis.

Six answers to one question. The sharpest fact about the landscape is that no single answer dominates: on a two-layer ReLU network with $m = 10^4$ training points, the VC bound exceeds~$3$ (vacuous), the Rademacher bound reaches~$0.3$ with norm control, and the PAC-Bayes bound touches~$0.05$ with a well-chosen prior, all for the identical network. Which framework gives you a number you can act on depends entirely on what you can measure.

Two results carry the weight here: (i)~the \emph{symmetrization
...
The classical approach ignores the algorithm entirely. It asks instead: does empirical risk converge to true risk \emph{uniformly over all hypotheses in the class}? If it does, then any algorithm that finds a low-training-error hypothesis automatically inherits low true risk. The algorithm does not need to be analyzed; it rides for free on the class structure. This is the content of uniform convergence, and the symmetrization argument is the mechanism that makes it computable.
...
$0.12$, a meaningful result.  For a two-layer neural network
...
\[
  ...
  \;\leq\; \sqrt{\frac{2d \log(em/d)}{m}}.\formal{FLT_Proofs.Complexity.Rademacher.vcdim_bounds_rademacher_quantitative}
\]
\end{prop}
...
Uniform convergence and Rademacher bounds hold simultaneously for every hypothesis in a class. That simultaneity is expensive: the bound must be pessimistic enough to cover the worst case. The PAC-Bayes framework trades universality for precision. It bounds not every hypothesis at once but the expected risk of a hypothesis drawn from a learned posterior~$\rho$, and it measures that posterior's distance from a data-independent prior~$\pi$. The tighter the posterior sits to the prior, the stronger the guarantee. The price is that the prior must be committed to before seeing data.
...
\]
For finite~$\mathcal{H}$, the complexity term $\KL(\rho\|\pi)$ becomes a
cross-entropy.\formal{FLT_Proofs.Theorem.PACBayes.pac_bayes_finite}
\end{thm}
...
\]
This change-of-measure argument yields the constant
$\sqrt{2(\KL(\rho\|\pi) + \log(1/\delta))/m}$.  The sharper form of
...
finite~$\mathcal{H}$, a direct union bound over hypotheses.
\end{proof}
...
prior must be calibrated: it should reflect actual uncertainty.
...
Stability bounds abandon the hypothesis class entirely. The question they ask is simpler and more direct: if you removed one training example and retrained, how different would the output be? An algorithm whose output barely moves under single-example perturbations cannot be memorizing; it must be capturing something about the distribution. That intuition has a precise quantitative form, and it yields generalization guarantees that hold for algorithms where Rademacher and VC bounds are vacuous.
...
An algorithm that learns nothing from its training data cannot overfit. The information-theoretic bound makes this precise: the generalization gap is controlled by how much the learned hypothesis $W = A(S)$ depends on~$S$, measured as the mutual information $I(S; W)$. The less information the algorithm extracts, the less it can memorize, and the better it generalizes. This is the youngest of the five traditions, but it offers the clearest conceptual picture of why noisy algorithms can outperform their deterministic counterparts.
...
A classifier that places a training point just barely on the correct side has learned less than one that places it far from the decision boundary. Margin-based bounds make this intuition precise: the confidence of correct predictions on the training set controls how badly the class can fit arbitrary labels, which in turn controls generalization. This was the original motivation for support vector machines, and it remains the primary analytic tool for neural network generalization, even where the bounds are currently loose.
...
Five frameworks, one quantity. The gap $|\risk_D(h) - \emprisk_S(h)|$ is what every bound here controls; the frameworks differ in which property of the learning problem they can see. \Cref{tab:five-frameworks} compares them along five axes: what each reads, what it bounds, where it is tight, and where it collapses to a vacuous guarantee. Reading the failure column is as instructive as reading the bound column.

\begin{table}[ht]
\centering\small
\caption[Five generalization-bound frameworks compared.]{Five
generalization-bound frameworks compared: what each measures, what it
bounds, where it is tightest, and where it fails.}
...
\Cref{fig:five-frameworks} shows which frameworks imply which others, and where the implications break. Reading it as a \emph{parallax} is the right frame: the five views are projections of the three-way interaction between data, class, and algorithm onto five different measurement axes. An implication arrow says one projection subsumes another; an obstruction arrow says two projections land on coordinates with no common ground.
...
\end{scope}
\end{tikzpicture}
\caption[Six generalization frameworks: one risk gap, many certificates.]{Six frameworks bound one quantity: the gap between true and empirical risk. They agree on what to bound and disagree on what to measure. The class-based frameworks (VC, Rademacher, PAC-Bayes) carry kernel-verified bounds; the single verified cross-framework link is that VC dimension controls Rademacher complexity via Sauer--Shelah (\leanname{vcdim_bounds_rademacher_quantitative}), which is why these two are not independent. The remaining arrows are literature implications or argued orthogonalities. The obstructions (dotted, bidirectional) mark where algorithm-based and class-based readings land on incompatible coordinates, so no single implication can cross them.}
...
\end{scope}
\end{tikzpicture}
\caption[Generalization is read from three sources: data, class, or algorithm.]{Every generalization bound reads exactly one of three information sources: the hypothesis class, the algorithm, or the channel between training data and output hypothesis. Placing those three sources at the corners of a triangle, the choice of framework becomes a choice of position: VC and Rademacher sit at the class corner; stability sits on the algorithm--data edge; PAC-Bayes on the algorithm--class edge; the information-theoretic bound at the interior, reading the $I(S;W)$ channel directly. The obstructions in \Cref{fig:five-frameworks} reappear here as frameworks placed at distant corners with no path between them. The two figures are spatial complements: the first grades the links, the second shows why links between far corners are hard to certify.}
\label{fig:triangle}
\end{figure}
...
head, the pipeline of \Cref{sec:uniform-convergence,sec:rademacher} yields a
generalization guarantee verified down to the floating-point execution of the
head.
...
ingredient is the rounding term: the bound is on the \emph{executed},
finite-precision head, so it carries an explicit envelope
$\mathrm{env}(f_\theta)$ separating the idealized map from its floating-point
realization.\formal{TLT.attnHead_certified_generalization}
\end{proof}
...
budget plus rounding only on a $\delta$-event) holds for the stack
indexed by its architectural
configuration.\formal{TLT.config_capacity_law}  That predicate is the
definition of a generalization
...
\Cref{prop:certified-attention} is the abstract theory of this chapter
instantiated for a transformer component.  It does \emph{not} resolve
...
certificate is correct but loose, exactly as the classical bounds are.  What
it adds is a bound whose constants and finite-precision corrections are
verified rather than estimated by hand.
\end{remark}
...
the chapter's five frameworks and their verified fragments open a wider
frontier.
...
bound $\mathcal{R}_m \leq \sqrt{2d\log(em/d)/m}$ are verified
...
applications) is not.  Can the full data-dependent bound, classical on
paper~\cite{BartlettMendelson2002}, be formalized end to end?
\end{openprob}
...
realistic attention stack that is also \emph{non-vacuous} on a benchmark, the
verified counterpart of \Cref{op:deep-margins}?
\end{openprob}
...
generalization-and-approximation guarantee?  The approximation invariant for
the certified stack is not yet formalized~\cite{XuRaginsky2017}.
\end{openprob}
...
The unification is asserted, but no master theorem with the three recoveries
written out exists~\cite{SteinkeZakynthinou2020}.
\end{openprob}
...
not merely sufficient?  The boundary between the two regimes is
uncharted~\cite{ShalevShwartzShamir2010}.
\end{openprob}
...
provably minimizes the bound?  The non-vacuous deep-network results opened
this question~\cite{DziugaiteRoy2017}.
\end{openprob}
...
norms~\cite{BartlettFosterTelgarsky2017}?  This complements
\Cref{prop:certified-attention}.
\end{openprob}
...
than merely bounding it?  A matching lower-bound invariant is missing for
general classes.
\end{openprob}
...
information-theoretic quantities that stays finite?
\end{openprob}
...
Five frameworks certify one quantity (the risk gap) by reading different information sources. The frameworks are recoverable from each other wherever their information sources overlap, and obstruct each other where those sources are orthogonal. Each exercise below stress-tests a specific link or gap in Figures~\ref{fig:five-frameworks} and~\ref{fig:triangle}: why the verified frameworks are exactly the class-based ones, what the Sauer--Shelah bridge actually does, why the algorithm--class boundary resists certification, and what a unifying CMI master inequality would require.

\begin{enumerate}[leftmargin=*,itemsep=3pt]
\item The three class-based frameworks have kernel-verified bounds: VC uniform convergence (\cref{thm:vc-uniform}), Rademacher complexity (\cref{def:rademacher}), and PAC-Bayes (\cref{thm:pac-bayes}).  The other three (stability, information, margin) are literature results (\cref{thm:stability-bound}, \cref{thm:xu-raginsky}, \cref{thm:margin-bound}).  Explain why the verified frameworks are exactly the ones whose complexity measure is intrinsic to the class.

\item The one verified cross-framework relationship is that VC dimension controls Rademacher complexity via Sauer--Shelah, $\mathcal{R}_m(\mathcal{H}) \le \sqrt{2\VC(\mathcal{H})\log(em/\VC(\mathcal{H}))/m}$ (\cref{prop:rad-vc}).  Show that this single inequality is what lets the VC bound be recovered from the Rademacher bound, so the two are not independent frameworks.

\item Every framework bounds the same quantity $|\risk - \emprisk|$ (or its expectation); the rows of \cref{tab:five-frameworks} differ only in the key quantity each reads (noise correlation, posterior--prior divergence, one-sample sensitivity, mutual information, margin).  Argue that generalization is one quantity admitting many certificates rather than six distinct theories.

\item Information-theoretic bounds depend on the algorithm and Rademacher bounds on the class: a deterministic ERM can carry $I(S;W)=H(W)$ on a class of small Rademacher complexity (\cref{sec:comparison}).  Show that this is an orthogonality of information sources and that reversing either direction does not produce a new bound.

\item A $\beta$-uniformly stable randomized algorithm satisfies $I(S;W)=O(m\beta^2)$ (\cref{thm:stability-bound}), so stability implies an information bound.  Show that the converse fails, and explain why an edge between two algorithm-based frameworks is harder to certify than the class-based Sauer--Shelah bridge.

\item The empirical Rademacher complexity, the Massart lemma, and the VC bound are verified, but the assembled high-probability bound of \cref{thm:rademacher-bound} (symmetrization plus the two McDiarmid steps) is not yet formalized (\cref{op:formalize-rademacher}, \cite{BartlettMendelson2002}).  Identify which step of the proof is the obstruction to a verified end-to-end Rademacher bound.

\item \Cref{ex:three-bounds} bounds one linear-classification problem three ways: a VC bound ($\VC=d+1$), a Rademacher bound ($\Rad_S\le 1/\sqrt{m}$), and a margin bound.  Compute all three for $d=1000$, $m=5000$ and explain why they differ only through which source each reads.

\item A conditional-mutual-information master inequality is conjectured to recover VC, PAC-Bayes, and stability at once (\cref{op:cmi-unification}, \cite{SteinkeZakynthinou2020}).  Sketch what such an inequality would have to say, and which of the six frameworks it would leave outside its scope.

\item Read ``one quantity, many certificates'' as a universal property: each framework is a section of a single risk-gap functor over a different fibration of the learning problem (the class for VC and Rademacher, the algorithm for stability and information, the posterior for PAC-Bayes).  Decide whether the certified attention bound (\cref{prop:certified-attention}) is a further algorithm-specific section, and whether the fibrations can be unified (\cref{op:cmi-unification}, \cref{op:tighten-certified-attention}).
\end{enumerate}
