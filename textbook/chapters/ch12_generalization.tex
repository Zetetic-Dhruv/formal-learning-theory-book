...
\end{scope}
\end{tikzpicture}
\caption[Six generalization frameworks: one risk gap, many certificates.]{The
six frameworks all bound the same quantity, the gap between true and empirical risk, but each
reads a different slice of the data, class, and algorithm interaction.  The class-based bounds
(VC, Rademacher, PAC-Bayes) are kernel-verified; the single verified cross-framework bridge is
that VC dimension controls Rademacher complexity through Sauer--Shelah
(\leanname{vcdim_bounds_rademacher_quantitative}).  The remaining links are literature
implications or conceptual orthogonalities between the algorithm-based and class-based readings.}
...
generalization bound reads one of three sources, the data, the hypothesis class, or the
algorithm, drawn here as the corners of a triangle, so the choice of framework is the choice
of which source one can measure.  VC and Rademacher sit at the class corner and PAC-Bayes on
the algorithm--class edge; stability, information, and margin read the algorithm and class
geometry from the edges and interior.  The obstructions of \Cref{fig:five-frameworks} reappear
here as projections landing on different corners, making the two figures spatial complements.}
\label{fig:triangle}
\end{figure}
...
Figures~\ref{fig:five-frameworks} and~\ref{fig:triangle} present the six frameworks as projections of one risk-gap quantity: the first grades the cross-framework links, the second the geometry of the three information sources.  The following exercises push on that structure.
