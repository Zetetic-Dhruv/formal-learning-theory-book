...
\end{tikzpicture}
\caption[The four data models, their characterizing dimensions, and the proven separations]{The four data models, each paired with the combinatorial quantity that decides its learnability: i.i.d.\ samples with the VC dimension, adversarial streams with the Littlestone dimension, enumerative texts with the mind-change ordinal, and interactive queries with query complexity. The chapter's lesson is the thresholds case, where one class has VC dimension one yet Littlestone dimension infinite: a complexity measure belongs to a class together with a data model, not to the class alone.}
...
\centering
\small
\caption{How the choice of data model determines the theory. Each row is a paradigm: the complexity measure, the characterization theorem, and the impossibility result all co-vary with the data model.}
...
This chapter has been about how data reaches a learner; it has left the learner itself implicit.  The consistent learner of \cref{thm:uc-pac} was defined inline (``output any $h \in \mathcal{H}$ agreeing with $S$'') with no $\sigma$-algebra on $\mathcal{H}$ and no requirement that the map $S \mapsto L(S)$ be measurable.  For the simplest classes that omission is harmless.  For a parametric attention model it is the structure that must be supplied, and supplying it exposes two data-axes the classical model suppresses: the \emph{measurability} of the data-to-hypothesis map, and the \emph{finite precision} in which data is actually read.
...
Each problem is anchored to a concept above and tagged by epistemic character: a \emph{known unknown} (\textsc{ku}) is an articulated open question; an \emph{unknown known} (\textsc{uk}) is a result present in the literature but not yet absorbed into a clean general statement.
...
Each exercise perturbs one data model and asks how the perturbation changes the characterizing dimension and the learnability it decides.
