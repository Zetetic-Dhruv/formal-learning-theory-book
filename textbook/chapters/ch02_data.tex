...
The inequality can be arbitrarily loose. Thresholds on $\R$ have $\VC = 1$ but $\Ldim = \infty$ (\cref{prop:threshold-vc,prop:threshold-ldim}), so finite VC dimension does not bound the Littlestone dimension---the formal content of fragility~(1) above. What the Littlestone dimension buys in return is a clean characterization and an algorithm that meets it.
...
The preceding sections developed each data model individually; the four relate as a single structure.
