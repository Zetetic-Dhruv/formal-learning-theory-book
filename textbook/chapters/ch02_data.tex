...
In Gold's model of identification in the limit~\cite{Gold1967}, data arrives as an enumeration, one element at a time, forever, and the learner must eventually converge to a correct hypothesis. There is no random sample here: the order is fixed by an adversary who may withhold any element arbitrarily long. Two variants of this process, \emph{text} and \emph{informant}, differ in a way that appears minor but produces a sharp separation.
...
The preceding sections developed each data model individually. The four stand in definite relation: each is pinned by a characterizing dimension, and the paradigms they generate are separated by machine-checked witnesses (Figure~\ref{fig:data-taxonomy}).\formal{FLT_Proofs.Theorem.Separation.online_strictly_stronger_pac}
...
Each addition in the fold commits a relative rounding error of at most the unit roundoff $2^{-24}$; propagating these through the $m$ summations and bounding by the total magnitude~$S$ gives the stated linear-in-$m$ envelope, a machine-checked instance of backward-error analysis.  What matters is that such a constant exists at all: the map from data to prediction that actually runs is the idealized one perturbed by a controlled, certified amount.
\end{proof}
...
Figure~\ref{fig:data-taxonomy} is mostly settled ground: three of its pairings (sample/VC, stream/Littlestone, text/mind-change) are exact characterizations verified end to end, and its two strict separations carry the witness classes their proofs exhibit. Only the dashed edges (the DFA query separation, the in-context measurability caveat) and one or two dashed nodes are still open. Each exercise below nudges one data model and asks where the perturbation lands on this map. Three landing zones are available. A perturbation that leaves a node's characterizing dimension untouched is an \emph{instance}: it sits on a node or edge already drawn. One that slides along an existing edge or trips a side-condition is a \emph{boundary}. One that opens a genuinely new question the diagram already marks dashed is a \emph{frontier}. Locate each in the figure's own grammar and justify the placement.

\begin{enumerate}[leftmargin=*,itemsep=3pt]
\item \textbf{Noisy oracle.} Corrupt labels at a rate below one half. This lands squarely inside the i.i.d.\ node: the characterizing VC dimension is invariant under the noise, so the perturbation is a stabilizer arrow that returns to where it started (\cref{thm:noise-robust}, Angluin--Laird 1988). Argue the invariance and identify the instance.
\item \textbf{Semi-supervised text.} Add a few labels to a positive-only text. The result interpolates the verified text-to-sample gap, so it is a boundary point on that edge, learnable exactly when the added labels supply the exclusion information the finite-support witness needs. Pin down the condition under which the boundary tips to learnable.
\item \textbf{Active versus passive.} Let the learner choose its queries. This is the passive-to-exact edge itself, an instance already drawn: the label-complexity measure exists (\leanname{LabelComplexity}), though the characterization remains class-specific (Angluin's $L^\ast$). Trace the edge and note why no single dimension labels it.
\item \textbf{Stream with bounded memory.} Cap the learner's state. The cap lives inside the online node's basin, since it can only raise the mistake bound above the Littlestone dimension, yet its own characterizing dimension is open. That makes it a frontier, one of the diagram's dashed nodes; explain why the bound can only worsen and why the dimension stays unknown.
\item \textbf{Distribution shift.} Let the data be non-stationary. The Littlestone mistake bound holds against \emph{any} sequence, so a drifting distribution is just one more adversarial order the online node already dominates: an instance, not a new region (\leanname{optimal_mistake_bound_eq_ldim}). Show the bound covers the shifted sequence verbatim.
\item \textbf{Equivalence-query budget.} Cap the number of equivalence queries. The budget places the learner at a boundary on the exact node, a point on the segment between the two verified endpoints: equivalence queries alone learn finite classes, while membership queries alone cannot certify. Locate the cap on that segment and say what it costs.
\item \textbf{Partial labels.} Let the oracle abstain. Abstention weakens the exclusion an informant would give, placing the model on the boundary between text and informant, learnable exactly when the surviving labels recover enough exclusion to defeat the Gold diagonalization (\leanname{gold_theorem}). Identify the abstention rate at which the boundary tips.
\item \textbf{Adversarial versus stochastic order.} Switch the sample order from random to adversarial. This is the diagram's spine, its primary verified separation (\leanname{online_strictly_stronger_pac}): stochastic order makes VC govern, adversarial order makes Littlestone govern, and thresholds are the one class realizing VC one and Littlestone infinite. Walk the spine from one regime to the other and name the witness that forces the gap.
\item \textbf{Privacy constraint.} Require a differentially private learner. Under pure privacy the characterizing quantity is a representation dimension distinct from VC, and the kernel carries no privacy infrastructure, so this hangs off the i.i.d.\ node as a dashed, open frontier. Argue that the governing dimension departs from VC and explain why the diagram must draw the node dashed.
\item \textbf{In-context presentation.} Let the data arrive as a prompt consumed by an attention learner. This sits on a boundary carrying a measurability side-condition: a hard attention head's failure event can be analytic while failing to be Borel (\leanname{attention_nonBorel_badEvent}), so the characterizations apply only under the well-behavedness the four classical nodes silently assume. Exhibit the side-condition and say which node it qualifies.
\end{enumerate}
