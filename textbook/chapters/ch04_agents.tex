...
Much of the vocabulary here is taxonomic: a learner is a function from data to hypotheses, and the field has named a dozen variants distinguished by which constraints they satisfy. These are best presented as a table. The chapter's genuine mathematical content lies in three places: the type mismatch between Bayesian posteriors and PAC hypotheses, resolved by the Gibbs posterior, whose optimality we prove in full; the surprising power of team learning; and the teacher models that connect to the teaching dimension and to Angluin's $L^\ast$. The learner abstraction and its constraints are typed in the verified development, and we close the opening section with a concrete modern instance, a finite-head attention learner, shown to satisfy the measurability condition that the PAC theory requires.
...
This definition is deliberately minimal. It says nothing about how $h_t$ is selected: whether the learner stores the full history or only its previous guess, whether it uses randomness, whether it may choose which $x$ to query. Each constraint yields a named variant (Figure~\ref{fig:learner-taxonomy} and Table~\ref{tab:learner-types}). One further condition is invisible at the level of types but indispensable for the statistical theory.
...
The same well-behavedness holds for the softmax-top-1 and multi-head argmax routers, and for the scaled dot-product attention of a full transformer.\formal{TLT_Proofs.Routing.ScaledDotProductScoreRouter.attentionRouting_wellBehaved} The abstract learner of \cref{def:learner} therefore covers the architectures that dominate modern practice, and covers them inside the measurable sub-class on which the entire PAC apparatus operates. We return to the measurability boundary itself (where attention routing \emph{fails} to be Borel) in \Cref{ch:separations}.
...
lattice of typed constraints, drawn so that each constraint's bite is paradigm-relative. The four
leaves (consistent, conservative, set-driven, iterative) are typed predicates on the base learner of
\cref{def:learner}, verified definitions in the development, so each is a real, solid node; their
conjunctions are meets in the lattice (a conservative, set-driven learner sits below two). The same
predicate reads two ways. In PAC the constraints are free (green, solid): every PAC-learnable class is
learnable by a consistent, proper empirical-risk minimizer (\leanname{erm_consistent_realizable}), and
because the i.i.d.\ assumption makes example order irrelevant the set-driven and iterative distinctions
collapse with it. In Gold's framework the same predicates bite (orange, dashed): some classes are
identifiable by a conservative learner from an informant but not from text, and a set-driven class can
force unbounded memory on any iterative learner. The Gold edges are dashed because they are results in
the inductive-inference literature (Gold; Angluin; K\"otzing and Palenta), not yet theorems of the
kernel, while every PAC-collapse edge is solid because the development proves it. The taxonomy has its
sharpest teeth in Gold and online, and none in PAC. The named data-access and randomness variants are
catalogued in \cref{tab:learner-types}.}
...
In the PAC setting many of the distinctions collapse, and the collapse is a theorem rather than an observation: every PAC-learnable class is learnable by a consistent, proper learner (the empirical-risk minimizer, which returns any hypothesis consistent with the sample and succeeds once uniform convergence holds (\cref{thm:uc-pac})).\formal{FLT_Proofs.Complexity.Generalization.ermLearn} Because the i.i.d.\ assumption makes example order irrelevant, the set-driven/order-sensitive distinction also vanishes. The taxonomy has its sharpest teeth in the Gold and online paradigms, where the learner's memory and update policy genuinely affect what is learnable.
...
\]
a distribution over hypotheses rather than a single hypothesis.\formal{FLT_Proofs.Learner.Bayesian.BayesianLearner}
\end{defn}
...
The PAC-Bayes bound's right-hand side is, up to the monotone square root and constants, an instance of $F_\lambda$: minimizing the complexity-penalized empirical risk over posteriors yields $Q_\lambda$. This is what makes the bound actionable: it names the posterior to use, not merely the existence of a good one, and it is the precise sense in which the Gibbs posterior converts a Bayesian learner into a PAC learner.\formal{FLT_Proofs.Theorem.PACBayes.gibbs_bound_of_pointwise}
...
\item \textbf{Teams are strictly more powerful than individuals.} In the Gold setting, two learners identify classes no single learner can (\cref{thm:team-power}); the mechanism is disjunctive success, not data partitioning or communication.
\end{enumerate}
...
\label{op:meta-char}
No analogue of the fundamental theorem of PAC learning exists for meta-learning: Baxter's bounds use covering numbers, not a dimension. Find a single complexity measure of the environment $\mathcal{E}$ that characterizes meta-learnability the way $\VC$ characterizes PAC learnability, building on the typed \leanname{FLT_Proofs.Theorem.Extended.MetaLearnerPAC}, or prove no such measure exists.
\end{openprob}
...
\subsection*{Counterfactual exercises: generalizing Figure~\ref{fig:learner-taxonomy}}

Each exercise perturbs the constraint lattice. Show, in the figure's grammar (typed learner predicates as nodes, two regime edges reading the same constraint as free-in-PAC or biting-in-Gold, solid where a kernel theorem backs the edge and dashed where it is borrowed from the inductive-inference literature or open), that the generalized figure subsumes the perturbation as an \emph{instance} (a constraint already shown free or biting), a \emph{boundary} (a constraint whose bite flips between paradigms, or a tie a paradigm must break), or a \emph{frontier} (a dashed, open edge the figure can only annotate).

\begin{enumerate}[leftmargin=*,itemsep=3pt]
\item \textbf{Consistency is free in PAC.} Impose consistency (\leanname{FLT_Proofs.Learner.Properties.IsConsistent}) on the PAC learner. Show this is the collapse instance on the green lane: every PAC-learnable class is learnable by a consistent, proper empirical-risk minimizer, so the constraint costs nothing in learnable-ness, verified with no gaps by \leanname{erm_consistent_realizable} resting on \leanname{empError_zero_iff_consistent} and closed by \leanname{fundamental_theorem}.

\item \textbf{Properness rides with consistency.} Require additionally that the learner output a hypothesis from the class. Show this stays a green instance, not a new edge: the ERM witness already lies in $\mathcal{H}$ (\leanname{ermLearn_in_H}), so proper plus consistent is the \emph{same} collapse node, and the meet of the two constraints in the lattice is free in PAC for the same reason.

\item \textbf{Conservativeness bites in Gold.} Read conservativeness (\leanname{FLT_Proofs.Learner.Properties.IsConservative}) on the Gold lane. Show this is a separation instance the figure must draw dashed: there are classes a conservative learner identifies from an informant but not from text, a result in the inductive-inference literature (\cite{Angluin1980,Gold1967,CaseSmith1983}) with no constraint-level theorem in the kernel, so the edge is orange and dashed.

\item \textbf{The same predicate, two lanes.} Take consistency alone and ask where it sits. Show this is the figure's defining boundary: consistency is free in PAC (green, solid, \leanname{erm_consistent_realizable}) yet in Gold a consistent learner can be forced into infinitely many mind changes, so the one predicate carries a green edge in one paradigm and an orange edge in another, which is exactly the paradigm-relativity the lattice exists to display.

\item \textbf{Set-driven against iterative memory.} Compare set-drivenness (\leanname{FLT_Proofs.Learner.Properties.IsSetDriven}) with iterativeness (\leanname{FLT_Proofs.Learner.Properties.IsIterative}) in the Gold and online settings. Show this is a separation instance, dashed: a set-driven class can force unbounded memory on any iterative learner (\cite{KotzingPalenta2016,Gold1967}), an edge the kernel does not yet prove, recorded open as \cref{op:order-sensitivity}.

\item \textbf{Order-sensitivity collapses under i.i.d.} Now move the set-driven versus iterative pair into PAC. Show this is the boundary's other face: because the i.i.d.\ assumption makes example order irrelevant, the distinction that bites in Gold vanishes, so the orange dashed Gold edge becomes the green collapse edge into the same ERM node, the bite flipping off as the paradigm changes.

\item \textbf{A constraint that bites only computationally.} Keep properness but ask for it under cryptographic hardness rather than information-theoretically. Show this is a frontier boundary: properness preserves the finite-VC PAC class (green on statistical grounds, \leanname{fundamental_theorem} is silent on computation) yet can cost strictly more computation, a separation on the resource axis the kernel does not carry, so the edge is dashed and cited to \cite{PittValiant1988}.

\item \textbf{The probabilistic constraint.} Add internal randomness, the probabilistic variant (\leanname{FLT_Proofs.Learner.Properties.ProbabilisticLearner}). Show this is a tie the paradigm must break: in PAC randomness can be derandomized at no asymptotic cost so the constraint is free (green), while in the Gold limit a probabilistic identification criterion is a genuinely distinct success notion; the figure places the node and lets the paradigm label decide the lane, dashed where the kernel has only the structure and no separation.

\item \textbf{Teams as a constraint that always bites.} Replace a single learner by a team (\leanname{FLT_Proofs.Learner.Properties.TeamLearner}). Show this is a separation instance robust across paradigms: two learners identify Gold classes no single learner can (\cref{thm:team-power}), the disjunctive-success mechanism, so unlike consistency the team relaxation bites in Gold and is not collapsed by PAC, and the edge is annotated by the team hierarchy of \cref{op:team-hierarchy}.

\item \textbf{Is collapse a closure operator?} Ask whether ``impose consistency and properness'' is a reflector onto a subcategory: a closure operator under which PAC learnability is invariant and idempotent, so that a constraint is free exactly when it lies below the closure's fixed points and biting otherwise. Show this is the figure's principal open frontier: it would re-type both lanes as one Galois-style law classifying which constraints collapse in which paradigm, but no such adjunction is defined in the development, so the meta-edge is dashed and recorded as a known unknown alongside \cref{op:order-sensitivity}.
\end{enumerate}
