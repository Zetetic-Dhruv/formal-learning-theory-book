...
Chapters~\ref{ch:objects} and~\ref{ch:data} fixed the objects of study and settled how data arrives. What they left open is who receives the data and acts on it. Without a typed account of agents, every result in the statistical and algorithmic chapters floats free: it refers to ``a learner'' without saying what that means as an object whose properties can be imposed, verified, and compared.

The vocabulary here is largely taxonomic: a learner is a function from data to hypotheses, and the field has named a dozen variants distinguished by which constraints they satisfy. These constraints are not equally consequential in all paradigms. The same four predicates, consistency, conservativeness, set-drivenness, and iterativeness, turn out to be free in PAC learning and costly in Gold's framework: imposing them in PAC changes nothing about what is learnable, while imposing them in Gold produces strict separations in learning power. The taxonomy matters because the paradigm determines the cost.

Three constructions go deeper than naming. First, the Bayesian learner outputs a probability measure over hypotheses, and PAC learning wants a single hypothesis: resolving this type mismatch requires not just discarding the posterior but finding the posterior that provably minimizes the complexity-penalized empirical risk. That posterior is the Gibbs posterior, and its optimality is a variational fact proved here. Second, two learners with access to the same data, communicating nothing, can identify classes that no single learner can: the team model is strictly more powerful, and the mechanism is the disjunctive success criterion alone. Third, when the source of data is itself an agent, the learner's complexity is shaped by the teacher's strategy, and the teaching dimension captures the cooperative communication cost of learning.

A finite-head attention learner satisfies the measurability condition (\cref{def:measurable-learner}) that makes the PAC success probability well-defined, placing transformer architectures inside the abstraction on which the statistical theory operates.
...
\end{tikzpicture}
\caption[Lattice of learner constraints: free in PAC, biting in Gold]{The argument of this figure:
the same four constraints on a learner cost nothing in one paradigm and separate what can be learned in
another. In PAC, consistency, conservativeness, set-drivenness, and iterativeness are all free: every
PAC-learnable class is learnable by a consistent, proper empirical-risk minimizer
(\leanname{erm_consistent_realizable}), and because examples arrive i.i.d.\ their order is irrelevant,
so set-drivenness and iterativeness collapse too. Move to Gold's framework and the same predicates bite:
conservative learners identify classes from informant that they cannot identify from text, and
set-driven learners can solve tasks that force any iterative learner into unbounded memory. The paradigm determines the cost. Named data-access and randomness variants are catalogued in
...
\section{Learners, Teams, and Teachers}
...
Figure~\ref{fig:learner-taxonomy} makes a claim: the paradigm determines the cost of each constraint. The exercises below put that claim under pressure. Each one takes a single predicate from the lattice and asks what happens when you move it from one paradigm to the other. Some collapse into equivalence; some stay strict. Tracking which do which, and why, is the concrete test of whether the taxonomy is understood.
