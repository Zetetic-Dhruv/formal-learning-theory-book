...
Figure~\ref{fig:learner-taxonomy} is built around one asymmetry, and the exercises below probe it from every side. The same typed predicate sits at a node and runs two edges out of it: a green solid edge into the PAC collapse (a kernel theorem backs it) and an orange dashed edge into Gold (the inductive-inference literature backs it, or it is open). One lane is a verified hub; the other is a frontier the figure can only annotate. Each exercise drops a new constraint onto the lattice and asks where its two edges land. Three landing zones recur: an \emph{instance} re-uses an edge already drawn (a constraint shown free in PAC or biting in Gold); a \emph{boundary} is where one predicate's bite flips between the lanes, or where a paradigm has to break a tie; a \emph{frontier} is a dashed edge with no theorem under it, present in the picture only as an open annotation. Place each perturbation in one of the three.

\begin{enumerate}[leftmargin=*,itemsep=3pt]
\item \textbf{Consistency is free in PAC.} Impose consistency (\leanname{FLT_Proofs.Learner.Properties.IsConsistent}) on the PAC learner. This lands on the green lane as the collapse instance: every PAC-learnable class is learnable by a consistent, proper empirical-risk minimizer, so the constraint costs nothing in learnable-ness. Trace the edge through its verified backing, \leanname{erm_consistent_realizable} resting on \leanname{empError_zero_iff_consistent} and closed by \leanname{fundamental_theorem}, and confirm it carries no gap.

\item \textbf{Properness rides with consistency.} Require additionally that the learner output a hypothesis from the class. No fresh edge appears; the perturbation collapses back onto the green instance already drawn, because the ERM witness already lies in $\mathcal{H}$ (\leanname{ermLearn_in_H}). Proper plus consistent is the \emph{same} collapse node, and the meet of the two constraints in the lattice is free in PAC for the same reason.

\item \textbf{Conservativeness bites in Gold.} Read conservativeness (\leanname{FLT_Proofs.Learner.Properties.IsConservative}) on the Gold lane. Here the constraint genuinely separates: some classes a conservative learner identifies from an informant escape it from text, a result in the inductive-inference literature (\cite{Angluin1980,Gold1967,CaseSmith1983}) with no constraint-level theorem in the kernel. The figure must draw this instance orange and dashed.

\item \textbf{The same predicate, two lanes.} Take consistency alone and ask where it sits. This is the boundary the whole figure is organized around: consistency is free in PAC (green, solid, \leanname{erm_consistent_realizable}) yet in Gold a consistent learner can be forced into infinitely many mind changes, so the one predicate carries a green edge in one paradigm and an orange edge in another. That split is precisely the paradigm-relativity the lattice exists to display.

\item \textbf{Set-driven against iterative memory.} Compare set-drivenness (\leanname{FLT_Proofs.Learner.Properties.IsSetDriven}) with iterativeness (\leanname{FLT_Proofs.Learner.Properties.IsIterative}) in the Gold and online settings. A set-driven class can force unbounded memory on any iterative learner (\cite{KotzingPalenta2016,Gold1967}); this is another dashed separation instance, an edge the kernel does not yet prove, recorded open as \cref{op:order-sensitivity}.

\item \textbf{Order-sensitivity collapses under i.i.d.} Now move the set-driven versus iterative pair into PAC. Watch the same edge flip lanes: because the i.i.d.\ assumption makes example order irrelevant, the distinction that bites in Gold vanishes, the orange dashed Gold edge turning into the green collapse edge that runs into the same ERM node. This is the other face of the boundary, the bite switching off as the paradigm changes.

\item \textbf{A constraint that bites only computationally.} Keep properness, but now read it under cryptographic hardness, where the resource being spent is computation time and not information. The landing is a frontier boundary: properness preserves the finite-VC PAC class (green on statistical grounds, \leanname{fundamental_theorem} is silent on computation) yet can cost strictly more computation, a separation on the resource axis the kernel does not carry, so the edge is dashed and cited to \cite{PittValiant1988}.

\item \textbf{The probabilistic constraint.} Add internal randomness, the probabilistic variant (\leanname{FLT_Proofs.Learner.Properties.ProbabilisticLearner}). This is a tie each paradigm settles its own way: in PAC randomness can be derandomized at no asymptotic cost so the constraint is free (green), while in the Gold limit a probabilistic identification criterion is a genuinely distinct success notion. The figure places the node and lets the paradigm label decide the lane, dashed where the kernel has only the structure and no separation.

\item \textbf{Teams as a constraint that always bites.} Replace a single learner by a team (\leanname{FLT_Proofs.Learner.Properties.TeamLearner}). The separation here is robust across both paradigms: two learners identify Gold classes no single learner can (\cref{thm:team-power}) through the disjunctive-success mechanism, so unlike consistency the team relaxation keeps its bite in Gold and survives the move to PAC uncollapsed. Annotate the edge with the team hierarchy of \cref{op:team-hierarchy}.

\item \textbf{Is collapse a closure operator?} Ask whether ``impose consistency and properness'' is a reflector onto a subcategory: a closure operator under which PAC learnability is invariant and idempotent, so that a constraint is free exactly when it lies below the closure's fixed points and biting otherwise. This is the deepest frontier the figure opens onto: such a closure would re-type both lanes as one Galois-style law classifying which constraints collapse in which paradigm. No such adjunction is defined in the development, so the meta-edge stays dashed and is recorded as a known unknown alongside \cref{op:order-sensitivity}.
\end{enumerate}
