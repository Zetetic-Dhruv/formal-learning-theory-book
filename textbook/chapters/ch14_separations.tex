...
If a learner could carry over the tools of one paradigm into any other,
theoretical learning would have a single axis, a uniform ordering from
weaker to stronger, and the question ``can this class be learned?'' would
have a single answer independent of how you ask it. It does not. What PAC
learnability guarantees breaks in the online adversarial setting, what
finite VC dimension buys computationally vanishes under a cryptographic
assumption, and what works for binary classification stops working once
the loss takes real values. The gap between what one paradigm promises
and what another demands is not a gap in the proofs. It is a structural
feature of the landscape, and the only way to see it clearly is through
an explicit witness: a concrete mathematical object that demonstrates,
by construction, that the missing implication cannot be recovered.
...
failure. Throughout this chapter we privilege the construction over the
claim: a witness that exists carries more information than a theorem that
merely states the failure.
...
relationship type: red arrows denote \rel{does\_not\_imply} edges, with
the label naming the witness; blue arrows denote \rel{strictly\_stronger}
edges. \emph{Line style} marks the evidence grade: solid for
machine-checked, dashed for classical.

Read the lattice and notice what it does not contain. Thirteen edges
among seventeen concepts are far fewer than the $\binom{17}{2} = 136$
pairs the concepts could in principle exhibit, yet the sparsity is not
a symptom of incomplete research. Most paradigm pairs simply operate on
different mathematical objects: incomparable data models, incomparable
success criteria, incomparable adversarial protocols. Between such
concepts a clean implication is often not the right question to ask.
Where it is the right question, this chapter either witnesses the
non-implication or witnesses the strict containment. Learning theory
is a partially ordered collection of largely independent formalisms,
with no single axis from weak to strong threading them all.
...
\caption[The separation lattice: learning theory as a sparse partial order.]{The
separation lattice: thirteen witnessed edges among seventeen concepts. Color marks the
relationship type (red = non-implication, blue = strict containment); line style marks
the evidence grade (solid = kernel-checked, dashed = classical). Reading the diagram,
notice that the order is sparse: far fewer than the $\binom{k}{2}$ relations the concepts
could in principle exhibit (\Cref{thm:sparsity}). The sparsity reflects genuine structural
incomparability: most paradigm pairs measure different mathematical objects and cannot be simply compared; where
a comparison makes sense, this chapter either witnesses the failure or verifies the containment.
The two independent codes make it possible to ask two separate questions from a single glance:
what is the relationship, and how well is it established.}
...
Each of the 9 \rel{does\_not\_imply} edges in this section fails a
plausible-sounding transfer: the kind of implication a student might
conjecture after seeing the first positive result. Each failure is
established by an explicit construction. The simplest witness is a
one-parameter family on $\R$; the most involved is the breakdown of the
fundamental theorem beyond binary classification, where construction
complexity tracks the depth of the mismatch it exposes.
...
single nine-way equivalence for binary classification. It does not
survive real-valued loss.

Shalev-Shwartz et al.~\cite{ShalevShwartzShamir2010} construct a
learning problem (arbitrary loss, multi-valued output) that is
learnable yet for which uniform convergence fails outright. In its place
they prove a different characterization: in the general learning setting,
a problem is learnable if and only if it admits a \emph{stable},
asymptotically empirical-risk-minimizing learner. Uniform convergence is
demoted from characterization to a sufficient condition.

\medskip\noindent
\emph{What the witness exploits.}
The symmetrization argument at the heart of the Fundamental Theorem's
proof requires the loss to take two values. Make the loss real-valued and
symmetrization breaks: uniform convergence stops characterizing
learnability, and the stable-AERM condition takes over as the
characterization, indifferent to the cardinality of the loss. The
Fundamental Theorem is \emph{local} to the 0-1 loss.
...
The 4 \rel{strictly\_stronger} edges carry a different burden. For a
non-implication it suffices to exhibit one counterexample. For a strict
containment the argument must close on both sides: the forward implication
must hold for all classes in the stronger paradigm, and the witness must
show a class that the weaker paradigm reaches but the stronger paradigm
cannot. Each edge therefore requires two proofs: one for the forward
implication, and one (the witness) for the strictness of the gap.
...
With all 13 edges in place, the separation lattice of
\Cref{fig:separation-lattice} says something beyond any individual
witness. Taken together, the 13 edges carry a structural claim about
what formal learning theory is: a collection of paradigms that partially
order by containment in some directions and are simply incomparable in
others. We can now state that claim explicitly.
...
that $k$ such concepts could \emph{a priori} exhibit. The witnessed edges
are scarce, and many remaining pairs are unrelated: they operate on
...
census in the chapter were settled tomorrow. PAC learning and Gold-style
identification take different data models (i.i.d.\ samples versus infinite
enumerations); online and exact learning use different interaction
protocols (adversarial instances versus query access); VC and SQ dimension
measure different properties (combinatorial shattering versus correlation
structure). Between such concepts a clean implication is often not even the
right question. Where we \emph{can} ask it, this chapter witnesses one
direction, while the reverse non-implication is in several cases a separate
open problem.

Each witness exploits a specific structural mismatch in the data model,
the success criterion, the adversarial model, or the computational model,
and these mismatches are not removable by clever proof techniques. They
...
\Cref{ch:generalization} is well-behaved\formal{TLT.Bridge.softAttention_wellBehaved}
and with norm-bounded parameters has finite capacity
(\Cref{prop:certified-attention}); it therefore sits on the PAC node, and
is PAC learnable.

Its position relative to the \emph{online} node is open. The witness for
Separation~1 was a class of finite VC dimension and infinite Littlestone
dimension (thresholds on~$\R$). Attention compares query--key scores, and
threshold behavior on a score is exactly the structure that sends the
Littlestone dimension to infinity while the VC dimension stays finite. So
the softmax-attention class may be its \emph{own} witness for
$\text{PAC} \not\Rightarrow \text{online}$, joining thresholds on that red
edge; whether it is is \Cref{op:attention-littlestone}.
...
The 13 witnesses leave two kinds of gap. The first is the missing \emph{reverse}
of a one-sided non-implication (where one direction is witnessed but the
other is not), and the second is the missing \emph{formal proof} of a separation so far
established only in the classical literature. Each open problem below is
tagged a \emph{known unknown} (KU), sharply posed but unresolved, or an
\emph{unknown known} (UK), where the right object or invariant for a
machine-checked proof is still missing.
...
The pseudo-manifold witness with $\Ndim = 1$ and $\DSdim = \infty$ is from
Brukhim et al. Can the construction, and the non-learnability it implies,
be machine-checked? An unknown
known~\cite{BrukhimCarmonDinurMoranYehudayoff2022}.
\end{openprob}
...
The lattice is a fixed object, but understanding it requires moving inside
it: removing an edge and checking what collapses, asking which edges would
need to change if a classical result were formalized, reading the entire
diagram through a single lens and discovering whether that lens is
consistent. Each exercise below does exactly this: it perturbs one feature
of \Cref{fig:separation-lattice} and asks where the perturbation lands.
This is the chapter's argument continued, now with the reader doing the
navigation. The references point to the relevant separation, open problem,
or verified result.

\begin{enumerate}[leftmargin=*,itemsep=3pt]
\item Verify that the four machine-checked edges form a single connected component touching the cluster $\{\text{PAC}, \text{Ex}, \text{online}, \text{FIN}\}$: the separations \leanname{pac_not_implies_online} and \leanname{ex_not_implies_pac} and the containments \leanname{ex_strictly_stronger_finite} and \leanname{online_strictly_stronger_pac}. Why does deleting the classical edges leave one small component rather than four isolated facts?

\item Take the PAC node and list its incident edges. Only the separation \leanname{pac_not_implies_online} and the containment \leanname{online_strictly_stronger_pac} are kernel-verified; PAC$\,\not\Rightarrow\,$exact (\cite{Angluin1988, PittValiant1988}), Natarajan$\,\not\Rightarrow\,$PAC (\cite{BrukhimCarmonDinurMoranYehudayoff2022}), and universal$\,\supsetneq\,$PAC (\cite{BousquetHannekeMoranVanHandelYehudayoff2021}) are classical. Even the most-studied paradigm sits mostly on classical edges.

\item For the containment $\DSdim \supsetneq \Ndim$, the witness is a hyperbolic pseudo-manifold with $\Ndim = 1$ and $\DSdim = \infty$ (\cite{BrukhimCarmonDinurMoranYehudayoff2022}), yet the kernel proves only the reverse inequality $\Ndim \le \DSdim$. Identify what remains to formalize (\Cref{op:formalize-ds-natarajan}).

\item For universal$\,\supsetneq\,$PAC, the forward implication (every PAC-learnable class is universally learnable) is verified, but the strictness rides the Bousquet et al.\ trichotomy (\cite{BousquetHannekeMoranVanHandelYehudayoff2021}). Show that closing this edge requires the hard strictness direction, not the easy implication (\Cref{op:formalize-trichotomy}).

\item The verified \leanname{ex_not_implies_pac} certifies that Gold identification does not imply PAC learning, witnessed by all finite subsets of $\N$ ($\VC = \infty$). Argue that PAC and Gold become genuinely incomparable only once the reverse non-implication is also established, which is open (\Cref{op:reverse-pac-ex}).

\item The two separations out of the VC node, $\VC\,\not\Rightarrow\,$tractability (\cite{KearnsValiant1994}, under a cryptographic assumption) and $\VC\,\not\Rightarrow\,$low SQ dimension (parities, \cite{BlumFurstJacksonKearnsMansourRudich1994}), separate computational learnability from the sample-complexity characterization. Explain why no $\binom{k}{2}$ count absorbs them: the hardness layer is orthogonal to the VC layer.

\item Formalizing the parity separation ($\VC = n$, $\SQdim = 2^n$) or the P\'alv\"olgyi--Tardos $\VC = 2$ compression separation would each promote one classical edge to verified status (\Cref{op:sq-vc-quantitative}, \Cref{op:formalize-compression-sep}). State what construction each formalization needs.

\item Read every edge, separation and containment alike, as a single ``$A$ reduces to $B$'' relation, so the diagram becomes the Hasse diagram of one reducibility order. Decide whether such a uniform reduction recovering all thirteen edges exists, or whether the paradigms are provably non-unifiable (\Cref{op:lattice-completeness}).
\end{enumerate}
