...
The lattice is \emph{sparse}. Thirteen edges connect concepts drawn from
six paradigms and a dozen complexity measures, and most paradigm pairs
are simply incomparable, because they operate on different mathematical
objects. Learning theory is a partially ordered collection of largely
independent formalisms, with no single axis from weak to strong threading
them all.
...
separation lattice: thirteen witnessed edges among seventeen concepts, color marking
relationship type (red for non-implication, blue for strict containment) and line style
marking evidence (solid kernel-checked, dashed classical). The order is sparse, far below the
$\binom{k}{2}$ pairs a priori possible (\Cref{thm:sparsity}), because most paradigms measure
different mathematical objects and admit no implication to witness.}
...
\emph{formal proof} of a separation so far only cited. Each is tagged a
...
Each exercise perturbs one feature of \Cref{fig:separation-lattice} and asks where the perturbation lands. The references point to the relevant separation, open problem, or verified result.
