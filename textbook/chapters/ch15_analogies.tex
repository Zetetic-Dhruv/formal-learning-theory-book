...
most of these gaps are structural rather than temporary, but this is a
judgment, not a theorem: no impossibility result rules out a bridge for
any individual pair, and at least one (information-theoretic bounds versus
PAC-Bayes) already has partial bridges~\cite{HellstromDurisi2020}. The
classification records where bridges are currently absent, not where they
...
Each exercise perturbs the obstruction map.  Read the figure as a concept
graph whose every edge is an \rel{analogy} that fails, carrying two
independent codes: a \emph{color and line style} for the obstruction type
(the book's classification of \emph{why} the analogy is not a theorem, in six
values, namely type mismatch, missing equivalence witness, one-way theorem
only, data-model mismatch, proof-method mismatch, and success-criterion
mismatch), and a \emph{filled dot} for the evidence, present on exactly the
five edges where the kernel certifies a one-way bridge or an equivalence
sorry-free.  Argue, in this grammar, that each perturbed picture subsumes the
concrete one as an \emph{instance} (an edge, color class, or dotted handle the
map already exhibits), a \emph{boundary} (one code, direction, or evidence
grade toggled, so an edge changes color, gains or loses its dot, or is
revealed as settled rather than open), or a \emph{frontier} (an undotted,
open or literature-only edge whose formalization, or whose meta-level
definition, is the next move).

\begin{enumerate}[leftmargin=*,itemsep=3pt]
\item \textbf{The distribution of colors is the lesson, not any single edge.}
Read the figure by counting colors and ignoring node identities.  Show this is
the map's defining instance: thirty-two analogies fail across only six
obstruction types, with type mismatch the plurality (twelve), so ``why does
this analogy fail?'' has a finite-codomain answer rather than a case-by-case
story.  The figure is the abstract witness that analogy-failure in learning
theory is taxonomic, and the colour histogram, twelve type mismatches
dominating, is the chapter's central claim that the field's most common
bridges break for the most basic reason, that the objects compared are not the
same kind of thing.

\item \textbf{The five dotted edges are the formalization frontier, and they
are a minority.}  Read only the edges carrying a filled dot.  Show this is the
instance that exposes where the kernel reaches into the analogy web: exactly
...
none is a theorem.}  Read only the orange strokes.  Show this is the instance
that anchors the honesty of the two-code scheme: every type-mismatch edge
(\Cref{sec:type-mismatch}) is an argument that source and target live in
different formal categories, not a proved impossibility and not a verified
bridge, so not one orange edge carries a dot.  Drawing color (a conceptual
classification) and the dot (a verified handle) as separate axes is exactly
what stops the reader from mistaking the book's analysis of \emph{why} an
...
\nde{littlestone\_dimension}.  Show this is the boundary that separates the
obstruction type from the evidence: the pair \emph{is} a verified equivalence
(\leanname{optimal_mistake_bound_eq_ldim}, hence the dot), so the ``missing
witness'' is not a missing proof of equivalence but the stand-alone-measures
distinction, that as complexity measures in their own right the two diverge
when extended to bandit or partial-monitoring feedback
(\Cref{sec:missing-equiv}).  The edge is blue because the naive ``these are
...
\nde{vc\_dimension}.  Show this is the boundary between a verified
\emph{direction} and an open \emph{equivalence}: the dot marks
...
\item \textbf{Rademacher versus VC and label versus sample are settled
non-equivalences, not conjectures.}  Read the two blue edges
\nde{rademacher\_complexity}--\nde{vc\_dimension} and
\nde{label\_complexity}--\nde{sample\_complexity}, which carry no dot.  Show
this is the boundary the chapter's caveat draws inside the blue class: these
are not open problems awaiting a proof of equivalence but \emph{established}
non-equivalences (Rademacher complexity is data-dependent and can fall far
below the VC bound; label complexity can be exponentially smaller than sample
complexity, the content of active learning, \Cref{op:active-passive}), so the
``missing witness'' is the recognition that the naive equivalence is false,
not a gap to be closed.  The blue color groups them with the genuinely open
MML edges, but the absence of a dot together with the prose distinguishes a
known separation from a live conjecture.
...
\nde{covering\_number}--\nde{vc\_dimension} and
\nde{covering\_number}--\nde{rademacher\_complexity}.  Show this is the
boundary where same color does not mean same evidence: covering numbers are
...
\nde{sample\_complexity} and \nde{vc\_dimension}.  Show this is the boundary
between a theorem and a definition: the dot marks \leanname{fundamental_theorem},
the equivalence between finite VC dimension and finite sample complexity, but
...
\emph{as a theorem}, not assumable as a definition, so the marker here guards a
two-directional result against being read as a tautology.
...
the meta-level classification.  Show this is the frontier the figure marks but
...
seventh is possible.  Show this is the figure's deepest frontier: the diagram
