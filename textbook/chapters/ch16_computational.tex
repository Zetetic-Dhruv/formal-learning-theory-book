...
The Fundamental Theorem of \Cref{ch:pac} settles a question and leaves a
second one completely open.  It settles existence: $\mathcal{H}$ is PAC
learnable if and only if $\VC(\mathcal{H}) < \infty$, a sample size
$m(\varepsilon, \delta)$ and a learner $A$ are guaranteed.  What it leaves
open, without comment, is \emph{time}.  A learner $A$ that runs for a
trillion years satisfies the theorem's conditions.  A practitioner cannot use
that learner.

Finite VC dimension guarantees a learner, but says nothing about that
learner's running time.  There exist hypothesis classes with finite VC
dimension, information-theoretically PAC learnable, that no polynomial-time
algorithm can PAC learn unless widely believed cryptographic assumptions are
false.  A fast learner for such a class would break modern public-key
cryptography; the gap is one of conditional impossibility.
...
The most natural escape route would be to ground the gap in $\mathrm{P} \neq
\mathrm{NP}$: a familiar, widely conjectured separation that already implies
hard learning problems exist.  The route is blocked.  Applebaum, Barak, and
...
The block is structural.  PAC learning is an \emph{average-case} problem: the
learner sees random examples drawn from a distribution and must generalize from
them.  $\mathrm{NP}$-hardness is worst-case: one hard instance suffices.
Connecting these two worlds requires a reduction that turns one hard
worst-case instance into a hard average-case problem over random examples, and
in many settings that reduction is provably unavailable through relativizing
techniques.
...
The information--computation gap has a subtler cousin.  Even among classes that
\emph{are} efficiently learnable, the learner's obligation to stay inside the
hypothesis class can matter: the gap between \emph{proper} and \emph{improper}
learning.
...
The distinction matters because two algorithms can compute the same set of
functions from different representations, and the representation, not the set,
governs whether efficient learning is possible.
...
The results of this chapter do not stand on their own: each one holds only if a
cryptographic assumption holds.  The concept graph makes this dependence
explicit through edges of type \rel{requires\_assumption}.  Each such edge
records that the source node's validity is conditional on an assumption that
the axioms of the theory alone cannot supply.
...
The computational layer is where formal learning theory leaves the most
unfinished: its central hardness results are conditional on assumptions that
may fall, and the basic characterization question, the one that would give VC
dimension its computational analogue, remains open.
...
conditionally on cryptographic assumptions, not efficiently learnable.  The
next chapter examines a different kind of gap: the distance between binary
classification and the richer learning settings that require new mathematical
primitives.
