...
problem.  Its source is that primitive, with all its average-case structure,
and so the reduction sidesteps the worst-case starting point one might expect.
The learner is given random examples
...
cryptographic problem on random instances.  This is why the hardness lives in
the \emph{average-case} world, over random instances rather than adversarial
ones.
\end{remark}
...
graph.  The claim that VC dimension characterizes PAC learnability is
information-theoretically valid, but no analogous computational characterization is known.
...
formula that is consistent with the data.  That hypothesis is a CNF wearing
none of the target's DNF clothing, and it classifies every example correctly
all the same.
...
learning, alive in every training run rather than shelved as a theorist's
curiosity.  A softmax-attention head induces a concept class
...
target was a DNF. The gap of this chapter is, in this sense, the precise reason that
...
\item \nde{computational\_hardness} requires \nde{cryptographic\_hardness}.
  ...
  The assumption is genuinely cryptographic: it asserts the hardness of a
  specific computational problem, a fact about factoring or LWE that sits
  entirely outside any constraint one might place on the learner's hypothesis
  space.
