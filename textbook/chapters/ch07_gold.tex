...
In 1967, seventeen years before Valiant introduced PAC learning, E.\ Mark Gold asked a question that has driven the oldest open research programme in computational learning theory: which classes of recursive functions can a machine identify from examples? The question looks deceptively simple. What makes it hard is that the learner gets no deadline, no error tolerance, no probability distribution. It receives a stream of data, produces a sequence of guesses, and must eventually guess correctly and stay there. The stream never ends. The learner never gets to announce that it is done. Whether it has converged is not a fact the learner can read off the data; it is a fact about the entire infinite future.

That structure, the infinite horizon with no convergence signal, is why this chapter's proof technique is \emph{diagonalization} rather than the concentration inequalities of PAC theory or the combinatorial game trees of online learning. The adversary in Gold's setting has infinite time and no distribution to work with. It defeats every candidate learner by constructing a stream that forces the learner to keep changing its mind, using only one tool: switching the true target whenever the learner commits. The proof method shapes every result in this chapter, and it is fundamentally unlike anything in Chapters 5 or 6.

Three features follow from this structure, each one a departure from the preceding paradigms:
...
\item \textbf{Ordinal-valued complexity.} When we ask ``how many times does the learner change its mind?'' the answer is a \emph{transfinite ordinal}: the integers run out, because some learnable classes force a number of mind changes that no fixed integer bounds. This is the first appearance of ordinals in learning theory.
...
Text presents only positive examples; informant presents both positive and negative examples. Gold showed that informant is strictly more powerful than text. We focus on text, the harder case.
...
Gold's impossibility theorem closes one door and opens two. If $\Ex$-identification cannot handle any class containing all finite languages and one infinite language, what can it handle? Two responses follow directly from the geometry of the definition: tighten the convergence requirement and learn less, or loosen it and learn more. Both directions yield strict separations.
...
$\Ex$ requires \emph{syntactic} convergence: the index stabilizes. $\BC$ requires only \emph{semantic} convergence: the computed language stabilizes, even if the learner keeps switching between programs that all compute the same language. This relaxation is not minor.
...
\end{tikzpicture}
\caption[The identification hierarchy as a chain of separations]{Each edge in this diagram records a theorem. The spine $\FIN\subsetneq\Ex\subsetneq\BC$ records what is gained by relaxing the convergence requirement: $\FIN$ demands a single irreversible guess, $\Ex$ allows revision but requires the index to stabilize, $\BC$ requires only the computed language to stabilize. The bottom rung $\FIN\subsetneq\Ex$ is verified in Lean and witnessed by the class of all finite languages (\leanname{ex_strictly_stronger_finite}): a learner cannot commit without revision to a finite language, because it cannot read off from the data when the last element has arrived. The Case--Smith separation $\Ex\subsetneq\BC$ is literature, not yet in the kernel. The branches off $\Ex$ record the effect of toggling a single clause: anomalous relaxes zero-error, monotonic adds no-retraction, vacillatory allows a finite oscillation among correct indices, trial-and-error connects to the $\Delta^0_2$ level of the arithmetical hierarchy.}
...
Each arrow is a theorem, witnessed by a class one criterion learns and the other cannot.
\end{traversal}
...
Anomalous learning relaxes $\Ex$ by replacing exact correctness with correctness up to a finite set. The class of $\Ex^*$-identifiable families strictly contains the $\Ex$-identifiable families~\cite{CaseSmith1983}.
...
Monotonic learning \emph{strengthens} $\Ex$ by adding a constraint. The class of $\mathrm{Mon}\Ex$-identifiable families is a strict subset of the $\Ex$-identifiable families.
...
Gold's impossibility theorem draws the outer boundary: certain classes cannot be identified at all. But the theorem also raises a sharper question for the classes inside that boundary. Among the classes a learner can identify, identification is not free: the learner must change its mind some number of times before settling. How many times? The answer turns out to require a number system larger than the integers.
...
Integer-valued bounds are \emph{not sufficient} to characterize the full landscape.
...
The mind-change ordinal measures $\Ex$-identifiable classes as VC dimension measures PAC-learnable ones, but where VC dimension is an integer this measure is a potentially transfinite ordinal.
\end{traversal}
...
Three learning paradigms are now in play: PAC (\Cref{ch:pac}), online (\Cref{ch:online}), and Gold. Each defines ``learnable'' by a different question, using a different proof technique and a different complexity measure. The natural question is how their notions of learnability relate.

The answer is a partial order with one strict containment and one incomparability. Online learnability sits strictly inside PAC learnability: every online-learnable class is PAC-learnable, and the converse fails. Gold-style identification is incomparable to both. It identifies classes of unbounded VC and Littlestone dimension that no statistical learner handles, because identification in the limit requires no generalization guarantee; and it fails on classes with no convergence signal that PAC and online learners handle easily, because those classes give the adversary no way to fix a target. Statistical capacity and convergence in the limit measure orthogonal things, and the partial order reflects that. A finite Littlestone dimension forces a finite VC dimension, so online sits properly inside PAC; nothing analogous connects either statistical dimension to the mind-change ordinal.

\begin{separation}
\textbf{Theorem} (PAC, online, and Gold).\formal{FLT_Proofs.Theorem.Separation.online_pac_gold_separation} \textit{Online learnability is strictly contained in PAC learnability; Gold-style identification is incomparable to both. Separation (a) below and the threshold separation are verified; separations (b)--(d) are classical:}
...
$\Ex$-identification succeeds because on any \emph{fixed} finite set the learner eventually sees all elements; PAC fails because the learner must handle \emph{all} finite sets simultaneously with bounded sample complexity, and the VC dimension is infinite.
...
\end{tikzpicture}
\caption[The three paradigms: online strictly inside PAC, Gold incomparable to both]{This figure makes an argument: the three paradigms do not form a containment chain, and two of the four possible comparison arrows do not exist. Online learnability sits strictly inside PAC learnability (\leanname{online_imp_pac}; the threshold class has $\VC{=}1$ and $\Ldim{=}\infty$, so PAC does not imply Online). The Gold region sits entirely outside PAC: the class of all finite languages is $\Ex$-identifiable from text yet has infinite VC dimension, so no statistical learner handles it (\leanname{ex_not_implies_pac}). Conversely, the threshold class is PAC-learnable but not $\Ex$-identifiable, so the Gold region does not contain the PAC region either. The incomparability is not a gap in the theory; it reflects that statistical capacity and convergence in the limit are orthogonal questions.}
...
Gold-style identification in the limit, the oldest paradigm in formal learning theory, yields five structural facts:
...
\item \textbf{The identification hierarchy} (\Cref{sec:hierarchy}): the chain $\FIN \subsetneq \Ex \subsetneq \BC$ is strict, with the Case--Smith separation (\Cref{thm:bc-strictly-stronger}) providing the witness for $\BC > \Ex$. Each rung of the hierarchy is a chain of theorems: every $\subsetneq$ carries a class one criterion learns and the next cannot.
...
\item \textbf{$\Ex$-identification of co-finite languages.}
  ...
  \item The result of~(b) is sharper than Gold's theorem applied
    ...
    diagonalization work is the learner's inability to distinguish
    ``finite set, done growing'' from ``co-finite set, still
    growing,'' a confusion that the complexity of $L_\infty$ neither
    creates nor removes.
  \end{enumerate}
...
The two figures above are not illustrations of the chapter's results; they are the results, compressed into edges and evidence grades. Each exercise below takes one edge, one node, or one structural feature of those figures and asks what happens when you read it more carefully: what the theorem actually says, what its proof actually uses, where the boundary of the argument sits. Working through them is not a drill. It is the same move the chapter made in building the figures, done at the level of a single claim at a time.

\begin{enumerate}[leftmargin=*,itemsep=3pt]
\item \textbf{The verified rung.} Read $\FIN \subsetneq \Ex$ as a theorem rather than a containment of definitions. $\FIN$-learnability implies $\Ex$-learnability by dropping the no-mind-change conjunct (\leanname{ex_strictly_stronger_finite}), and the class of all finite languages supplies the strict separation (\Cref{ex:finite-languages}).

\item \textbf{Syntactic versus semantic convergence.} $\Ex$ requires the index to satisfy $h = c$ while $\BC$ requires only $\forall x,\ h\,x = c\,x$: one clause toggled from equality of programs to equality of extensions. Show that this single toggle is exactly what the Case--Smith class exploits to separate $\Ex \subsetneq \BC$ (\cite{CaseSmith1983}, \cref{op:bc-ex-formal}).

\item \textbf{Two relaxations of $\Ex$.} Toggling the exactness clause of $\Ex$ from $h = c$ to ``$h$ errs on at most $a$ strings'' enlarges the class ($\Ex^* \supsetneq \Ex$); conjoining the no-retraction clause $h\,\mathrm{data}\,x = \mathrm{true} \Rightarrow h\,(\mathrm{data} {+}{+} [xy])\,x = \mathrm{true}$ shrinks it ($\mathrm{Mon}\Ex \subsetneq \Ex$). Verify both strictness directions (\cite{CaseSmith1983,OshersonStobWeinstein1986}).

\item \textbf{The arithmetical anchor.} Read the $\Delta^0_2$ tag on the trial-and-error node as a full characterization, an equality of classes rather than a containment. Trial-and-error learnability is pointwise convergence and equals membership in $\Delta^0_2$ by the Putnam--Gold theorem (\cite{Putnam1965}); state the equality precisely and identify which direction is the harder one.

\item \textbf{Gold's theorem as the pressure behind the chain.} Gold's impossibility (\leanname{gold_theorem}) rules out $\Ex$-identification for any class with all finite languages and one infinite language. Show that strengthening to $\FIN$ and weakening to $\BC$ are the two escape routes from this bound, so the chain exists because $\Ex$ is provably limited.

\item \textbf{Mind changes grade the bottom rung.} Refine the $\FIN$-to-$\Ex$ gap by counting retractions rather than forbidding them: $\FIN$ is zero mind changes, $\Ex$ is finitely many, and the intermediate measure is ordinal-valued (\leanname{MindChangeOrdinal}). State the transfinite strictness (\cite{FreivaldsSmith1993}, \cref{op:fs-ordinal}) that the integer counts cannot capture.

\item \textbf{An edge that does not exist.} Try to draw ``online-learnable but not PAC-learnable'', certified by ``finite Littlestone, infinite VC''. Show that $\VC \le \Ldim$ (\leanname{BranchWiseLittlestoneDim_ge_VCDim}) makes that certificate an empty predicate and \leanname{online_imp_pac} makes the class empty, so the edge is ruled out.

\item \textbf{One arrow certifies two separations.} The finite-languages class is $\Ex$-identifiable but has $\VC = \infty$ (\leanname{ex_not_implies_pac}). Show that because Online lies inside PAC, the same class is a fortiori not online-learnable, so $\Ex \not\Rightarrow \mathrm{PAC}$ alone certifies $\Ex \not\Rightarrow \text{Online}$ with no separate theorem.

\item \textbf{Is the order one graded measure?} Ask whether the VC dimension, the Littlestone dimension, and the mind-change ordinal are three readings of a single parameterized complexity measure under one data-model parameter. The kernel proves $\VC \le \Ldim$ and $\text{Online} \Rightarrow \mathrm{PAC}$ but carries the three invariants separately; decide whether they unify or are provably non-unifiable (\cref{op:meta-measure}, \cref{op:effective-three-paradigm}).
\end{enumerate}
