...
$\BC$ is strictly more powerful than $\Ex$: there exists a class of languages that is $\BC$-identifiable but not $\Ex$-identifiable from text.\formal{FLT_Proofs.Criterion.Gold.BCLearnable}
\end{thm}
...
\begin{separation}
\textbf{Theorem} (Three-Paradigm Separation).\formal{FLT_Proofs.Theorem.Separation.online_pac_gold_separation} \textit{The following four separations hold:}
...
The three-paradigm separation (\cref{sec:three-paradigm}, \leanname{FLT_Proofs.Theorem.Separation.online_pac_gold_separation}) shows PAC, online, and Gold are pairwise incomparable.  Determine whether a single parameterized complexity measure specializes to the VC dimension, the Littlestone dimension, and the mind-change ordinal under three settings of a data-model parameter, or prove that the three invariants are provably non-unifiable.
\end{openprob}
...
The three-paradigm separation produces its witnesses existentially.  Give a single parameterized family of concept classes that realizes each of the four pairwise separations of \cref{sec:three-paradigm} with explicit witnesses and explicit obstructions, turning the separation lattice into one constructive theorem rather than four independent constructions.
\end{openprob}
