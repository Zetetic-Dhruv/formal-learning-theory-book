...
\end{tikzpicture}
\caption[The identification hierarchy as a chain of separations]{The identification hierarchy:
the spine $\FIN\subsetneq\Ex\subsetneq\BC$ of success criteria, refined by single-constraint
relaxations of $\Ex$ (anomalous, monotonic, vacillatory, trial-and-error). Each spine edge is a
separation theorem witnessed by a class one criterion learns and the next cannot; the bottom rung
$\FIN\subsetneq\Ex$ is witnessed by the class of all finite languages
(\leanname{ex_strictly_stronger_finite}).}
...
The answer is a partial order, not an antichain. Online learnability is \emph{strictly contained} in PAC learnability: every online-learnable class is PAC-learnable, and the converse fails. Gold-style identification is incomparable to both: it identifies classes of unbounded VC and Littlestone dimension that no statistical learner handles, while it fails on classes with no convergence signal that PAC and online learners handle easily. Statistical capacity (PAC and online) and convergence in the limit (Gold) are orthogonal axes; a finite Littlestone dimension forces a finite VC dimension, so online sits properly inside PAC.
...
\end{tikzpicture}
\caption[The three paradigms: online strictly inside PAC, Gold incomparable to both]{The three paradigms
form a partial order, not an antichain. Online learnability is strictly contained in PAC learnability
(\leanname{online_imp_pac}; strict via the threshold class), so the Online region sits inside the PAC
region. Gold-style identification is incomparable to both: it identifies the finite languages, which
have infinite VC dimension, yet fails on classes with no convergence signal.}
...
Each exercise perturbs one of the chapter's two figures and asks what the corresponding separation, relaxation, or limit becomes.
