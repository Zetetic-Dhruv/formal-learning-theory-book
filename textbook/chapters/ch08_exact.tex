...
The previous chapters impose a constraint so familiar it is easy to overlook: the learner waits. Data arrives drawn from a distribution, or delivered by an adversary, or enumerated by nature; in every case the environment decides what the learner sees. The learner has no say. This chapter removes that constraint and asks what changes.

The answer is sharper than most results in this book. Angluin's exact learning model~\cite{Angluin1987} gives the learner two oracles: a membership oracle that answers any labelling query $\mathrm{MQ}(x) = c(x)$ on demand, and an equivalence oracle that either confirms a proposed hypothesis or returns a counterexample. The goal is not approximate generalization but \emph{exact identification}: the game ends only when the learner's hypothesis is accepted on every string, not merely on most of them. With these two instruments a learner can identify the minimal DFA for any regular language in polynomially many queries. Without them, the same task is computationally intractable under cryptographic assumptions~\cite{KearnsValiant1994}. The gap is qualitative: no quantity of random examples bridges it, and the barrier is computational rather than statistical, since DFAs have finite VC dimension and are sample-efficiently learnable in principle.
...
\end{tikzpicture}
\caption[The $L^*$ exact-learning loop]{Each cycle of the $L^*$ loop either refines the learner's model of the target or ends the game. The learner interacts with a minimally adequate teacher (\leanname{MinimallyAdequateTeacher}): the observation table $(S,E,T)$ is a finite slice of the target's Myhill--Nerode right-congruence, membership queries fill it string by string, and when a closed, consistent table licences a DFA conjecture, an equivalence query either confirms it or hands back a counterexample that splits at least one equivalence class. Because the minimal DFA has $n$ states, the congruence has index $n$; the table starts with one class and gains at least one per counterexample, so the loop terminates in at most $n-1$ equivalence queries. This bound is Angluin's theorem (\cref{thm:lstar-complexity}), literature-cited and open in the kernel (\cref{op:lstar-formal}).}
...
The $L^*$ algorithm is only half the argument. To see what it costs the learner to go passive, set it beside the hardness result it was built to contrast.
...
\end{tikzpicture}
\caption[The passive--active gap on DFAs]{DFAs prove that query access and random examples are not interchangeable. $L^*$ identifies any $n$-state DFA exactly with polynomially many queries (\cref{thm:lstar-complexity}), yet no polynomial-time algorithm PAC-learns DFAs from random examples under the cryptographic hardness of RSA or factoring (Kearns--Valiant, \cref{rmk:crypto-assumption}). The separation is conditional on the cryptographic assumption and holds in one direction only: a class that is PAC-easy but exact-hard has not been exhibited (\cref{op:reverse-separation}). The gap is computational, not information-theoretic: DFAs have finite VC dimension, so sample-efficient passive learning is possible in principle; polynomial-time passive learning is what the cryptographic assumption rules out.}
...
\begin{proof}
The membership oracle $\mathrm{MQ}(s) = f(s)$ is total and truthful: a forward pass computes $f(s)$ up to the bounded finite-precision forward error of the attention layer.\formal{TLT.Fp32AttnLit.attnLiteralForwardError} The equivalence oracle is approximate: it certifies $L(M) = f$ on a sampled test set rather than on all strings, so the conjecture is exact on the queries made and probably approximately correct off them; with an exact equivalence oracle $L^*$ returns the minimal DFA for $f$ by \cref{thm:lstar-complexity}. Either way the model is a legitimate $L^*$ teacher.
\end{proof}

This is how Weiss, Goldberg, and Yahav~\cite{WeissGoldbergYahav2018} extract interpretable automata from recurrent networks, and the construction applies to attention models verbatim. A model opaque under passive inspection becomes a queryable teacher, and $L^*$ distills its regular core into a readable DFA. Because the equivalence oracle is only approximate, extraction is sound on the queries and heuristic beyond them, and whether a transformer's string function even has a finite regular core is open (\cref{op:transformer-regular}).

\section{Query Complexity}
\label{sec:query-complexity}

The natural complexity measure for exact learning is not time or space but \emph{query complexity}: how many questions must the learner ask in the worst case to pin down a target of representation size $n$. Query complexity is the active-learning analogue of sample complexity; it measures how much the learner must interrogate, not how much data the environment supplies.
...
Each problem is anchored to a result above and tagged: a \emph{known unknown} (\textsc{ku}) is an articulated open question; an \emph{unknown known} (\textsc{uk}) is a literature or development result not yet absorbed into a clean statement.
...
The argument of this chapter rests on a specific combination of parts: an observation table that approximates the Myhill--Nerode congruence, closedness and consistency conditions that licence a quotient DFA, counterexamples that split classes, and a teacher that can answer both membership and equivalence queries. Each exercise below removes or replaces one of those parts. The question in each case is the same: what breaks, and why does it break exactly there? Work through the $L^*$ loop (Figure~\ref{fig:lstar-protocol}) and the passive--active gap (Figure~\ref{fig:passive-active-gap}) as the object being disassembled.

\begin{enumerate}[leftmargin=*,itemsep=3pt]
\item \textbf{The table is the right congruence.} Read the observation table as the object being learned. Two access strings share a row exactly when they are indistinguishable by the suffixes in $E$, a finite slice of the Myhill--Nerode relation $s_1 \sim s_2 \iff (\forall w,\ s_1 w \in L \Leftrightarrow s_2 w \in L)$ (\cref{rmk:table-intuition}), so the distinct rows are the equivalence classes discovered so far and the conjecture DFA is their quotient.

\item \textbf{Closedness and consistency are the quotient's well-definedness.} Toggle off closedness: let some $\mathrm{row}(s\cdot a)$ match no representative in $S$. Then the transition $\delta(\mathrm{row}(s),a)=\mathrm{row}(s\cdot a)$ has no target state, and without consistency it depends on the chosen representative (\cref{def:closed-consistent}). The two conditions are precisely what licenses the quotient automaton; dropping one leaves the conjecture without a legal transition function.

\item \textbf{A counterexample is a new-class witness.} Replace the teacher's \textsc{yes}/counterexample reply by a bare ``correct or not'' bit. The equivalence oracle's counterexample $z$ is a point where the conjecture and target disagree, and adding $z$ with its prefixes forces at least one new distinct row, splitting a Myhill--Nerode class. A bare bit removes the witness, and the loop can no longer discover the missing state.

\item \textbf{The $n-1$ bound is the index of the congruence.} The minimal DFA has $n$ states, so the congruence has index $n$ (\cref{rmk:table-intuition}, uniqueness by \cite{RabinScott1959}); the table starts at one class and gains at least one per counterexample, so termination after $\le n-1$ rounds is the finiteness of the index (\cref{thm:lstar-complexity}). Formalizing this bound on top of the typed teacher is \cref{op:lstar-formal}.

\item \textbf{Drop membership queries and the query count blows up.} Run the cycle with equivalence queries alone. Without membership queries the learner cannot place a string in its Myhill--Nerode class, so each equivalence query removes only one candidate from the version space, and an $N$-concept class costs $N-1$ queries in the worst case (\cref{prop:eq-lower}, \cite{Angluin1988}); membership queries collapse this to the linear $n-1$ of \cref{thm:lstar-complexity}.

\item \textbf{Counterexample length is a hidden cost.} The equivalence oracle may return an adversarially long counterexample (\cref{rmk:counterexample-length}), so the membership-query count carries a factor $m$ (\cref{thm:lstar-complexity}). Whether a learner can binary-search the counterexample to remove the linear dependence on $m$ is open (\cref{op:counterexample-length}, \cite{RivestSchapire1993}).

\item \textbf{Passive data cannot drive the loop.} Replace the active teacher with random labelled examples from a distribution. DFAs are exactly learnable by the loop (\cref{thm:lstar-complexity}) yet not PAC-learnable under cryptographic assumptions (\cref{thm:dfa-pac-hard}, \cite{KearnsValiant1994}), so the passive--active edge is qualitative. The reverse direction, a class PAC-easy yet exact-hard, is open (\cref{op:reverse-separation}).

\item \textbf{The gap is computational, not information-theoretic.} Ask whether DFAs are PAC-learnable at all, ignoring efficiency. They have finite VC dimension, so by the fundamental theorem of statistical learning they are PAC-learnable with polynomially many samples; \cref{thm:dfa-pac-hard} blocks only the polynomial-time computation, leaving the sample complexity intact. The separation is therefore computational.

\item \textbf{Any queryable function is an $L^*$ target.} Replace the unknown regular language by the binary string function of a trained attention model, answering membership by a forward pass and equivalence by sampling. The model is a legitimate minimally adequate teacher (\cref{prop:transformer-extraction}, \cite{WeissGoldbergYahav2018}), so the loop extracts a DFA exact on the queried strings. Whether the model's function has a finite Myhill--Nerode index, so that extraction is exact rather than heuristic, is open (\cref{op:transformer-regular}), as is the soundness of an unreliable oracle such as an LLM critic (\cref{op:llm-teacher}).
\end{enumerate}
