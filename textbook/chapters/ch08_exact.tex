...
Each exercise perturbs the gap. Show, in the figure's grammar (two learnability criteria joined by evidence-graded edges, where a box is drawn solid when a verified type certifies the criterion, a separation is drawn dashed when it is a literature theorem rather than a kernel one, and a direction is drawn with a question mark when the chapter flags it open), that the general object is not a symmetric antichain but an \emph{asymmetric, evidence-graded} pair: one established conditional separation and one open reverse. Classify each perturbation as an \emph{instance} (the gap read on a determined class), a \emph{boundary} (one capability or one assumption toggled and the established direction strictly changes), or a \emph{frontier} (a dashed literature-only or open edge).

\begin{enumerate}[leftmargin=*,itemsep=3pt]
\item \textbf{The boxes are types, the arrows are literature.} Read the three nodes as the objects the kernel actually owns. Show this is the figure's defining instance: the exact-learning criterion (\leanname{FLT_Proofs.Criterion.PAC.ExactLearnable}, \cref{def:exact-learning-formal}), the PAC criterion (\leanname{FLT_Proofs.Criterion.PAC.PACLearnable}), and the witness type (\leanname{FLT_Proofs.Computation.DFA'}) are all verified, sorry-free definitions, so the boxes are solid; but neither separation leg is a kernel theorem, so the arrows between them are dashed and carry citations, not handles.

\item \textbf{The reverse direction is open, not established.} Add to the figure the arrow ``PAC $\not\Rightarrow$ exact'' as if it were proven. Show this is the boundary that exposes the figure's former over-claim: the chapter flags the reverse as an open problem, ``exhibit a class efficiently PAC-learnable but not efficiently exactly learnable'' (\cref{op:reverse-separation}), so no witness class is known and the second arrow has no content; drawing it symmetrically to the first banks an open problem as a theorem. The honest object draws the reverse as a dashed question-mark edge, never a not-implies arrow.

\item \textbf{The established separation is conditional, not a fact.} Remove the cryptographic hypothesis from the hardness leg. Show this is the frontier that re-grades the established direction: the non-PAC-learnability of DFAs holds only under the hardness of RSA or factoring (\cref{thm:dfa-pac-hard}, \cref{rmk:crypto-assumption}, \cite{KearnsValiant1994}); without that assumption the PAC-hardness of DFAs is itself open, so even the surviving direction is a conditional literature edge, not an unconditional one, and the figure must say so.

\item \textbf{The gap is computational, not informational.} Ask whether DFAs are PAC-learnable at all, ignoring efficiency. Show this is the boundary that names what the gap is \emph{not}: DFAs have finite VC dimension, so by the fundamental theorem of statistical learning they are PAC-learnable with polynomially many samples (\cref{thm:dfa-pac-hard} blocks only the polynomial-time computation, not the sample complexity); the separation is therefore qualitative and computational, and an information-theoretic reading of it is a category error.

\item \textbf{The easy side is cited, not verified.} Ask the kernel for the statement ``$L^*$ exactly learns the minimal DFA.'' Show this is the figure's first principal frontier: the algorithm object exists as a verified type (\leanname{LStar}, fields only, sorry-free) but with no correctness lemma, so the ``exactly learnable'' leg of Direction~1 rests on Angluin's theorem (\cref{thm:lstar-complexity}, \cite{Angluin1987}), open in the kernel (\cref{op:lstar-formal}); it is drawn dashed.

\item \textbf{The hard side has no kernel theorem.} Search the verified library for a DFA-PAC-hardness result. Show this is the figure's second principal frontier: there is none; the only hardness-named theorem about a finite-VC class concerns the statistical-query dimension, not DFAs and not PAC intractability, so the ``not efficiently PAC-learnable'' leg is entirely literature (\cref{thm:dfa-pac-hard}, \cite{KearnsValiant1994}) and is drawn dashed, with both legs of Direction~1 unverified in the kernel.

\item \textbf{The witness is the only thing forcing the gap.} Replace DFAs by a class learnable both passively and actively, such as a class of small VC dimension with a polynomial-time PAC learner. Show this is the instance where the gap collapses: the separation is carried by the witness type (\leanname{FLT_Proofs.Computation.DFA'}), and only because that class is simultaneously exact-easy (\cref{thm:lstar-complexity}) and PAC-hard (\cref{thm:dfa-pac-hard}); change the witness and Direction~1 no longer holds, so the figure asserts a property of a class under two presentations, not of the paradigms in the abstract.

\item \textbf{The teacher is the mechanism knob, passive and active are its settings.} Read the membership and equivalence oracles as the do-structured interface and turn them off. Show this is the boundary that makes the gap a single mechanism: a minimally adequate teacher is exactly a membership oracle plus an equivalence oracle on one target (\leanname{FLT_Proofs.Learner.Active.MinimallyAdequateTeacher}, \cref{def:mat}; oracles \leanname{FLT_Proofs.Data.MembershipOracle}, \leanname{FLT_Proofs.Data.EquivalenceOracle}), and the passive setting is the same target presented by random examples; passive and active are two settings of the oracle knob, not two incomparable primitives, so the figure records what learnability does as that knob moves.

\item \textbf{Drop membership queries and the active side degrades.} Keep only equivalence queries on the active side. Show this is the boundary internal to Direction~1: with equivalence queries alone an $N$-concept class costs $N-1$ queries in the worst case (\cref{prop:eq-lower}, \cite{Angluin1988}), whereas membership queries collapse the count to the linear $n-1$ of \cref{thm:lstar-complexity}; the ``active learning succeeds'' leg depends on which oracle the teacher provides, so the active node is not atomic.
