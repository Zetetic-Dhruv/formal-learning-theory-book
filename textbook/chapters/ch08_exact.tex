...
\end{tikzpicture}
\caption[The $L^*$ exact-learning loop]{The $L^*$ protocol: a learner and a minimally adequate teacher
(\leanname{MinimallyAdequateTeacher}) cooperate to identify the minimal DFA for an unknown regular
language. The observation table $(S,E,T)$ is a finite approximation of the target's Myhill--Nerode
right-congruence; membership queries fill it cell by cell, and each equivalence query returns a
counterexample that forces a new state class. Termination in at most $n-1$ equivalence queries is the
index of the congruence (Angluin's theorem, \cref{thm:lstar-complexity}).}
...
\end{tikzpicture}
\caption[The passive--active gap on DFAs]{The passive--active gap. Exact learning with membership and
equivalence queries does not imply efficient PAC learning from random examples: DFAs witness it, being
exactly identifiable by $L^*$ in polynomially many queries yet not efficiently PAC-learnable under the
hardness of RSA or factoring (Kearns--Valiant, \cref{rmk:crypto-assumption}). The gap is computational,
not information-theoretic, since DFAs have finite VC dimension; the reverse direction is open
(\cref{op:reverse-separation}).}
...
These exercises perturb the $L^*$ loop (Figure~\ref{fig:lstar-protocol}) and the passive--active gap (Figure~\ref{fig:passive-active-gap}) one piece at a time and ask what the learnable class, or the established separation, does in response.
