...
\item \textbf{Special cases are resolved.}  Maximum classes (those achieving
  ...
  teaching dimension compress to that dimension; these are the established
  families, and the list is illustrative rather than exhaustive.
...
schemes, and the belief that this regularity was structural: a property of
finite-VC classes as such, which would make those three examples instances of
a general law rather than a lucky coincidence.  Four
...
\item \textbf{Machine-checked certificates.}  For the specific function class
  ...
  \Cref{ch:generalization} has been formalized: a proof assistant verifies
  that the executed (finite-precision) network's true risk exceeds its
  empirical risk by more than an explicit Dudley-capacity budget only on a
  sample event of probability at most~$\delta$.\formal{TLT.CertifiedGeneralization}
  The budget is discharged from the layer norms and depth of the
  stack,\formal{TLT.config_capacity_law} so the bound is correct by
  construction.  Like its hand-proved cousins, however, it grows with depth and
  norm and is not yet non-vacuous on large models: it advances
  \Cref{op:deep-gen} along the axis of \emph{trust}, supplying a
  kernel-audited guarantee, while leaving \emph{tightness} where it found it,
  the same Dudley budget now merely certified rather than shrunk.
\end{itemize}
...
is itself part of \Cref{op:deep-gen}.  The phenomenon suggests that the
relevant notion of complexity for overparameterized models is not a simple
...
for:
\begin{itemize}[leftmargin=*]
\item the architecture, with the function class alone left underdetermined,
\item the full optimization trajectory, the endpoint by itself being silent,
\item the data distribution, which worst-case analysis discards.
\end{itemize}
...
machine-checked and non-vacuous on a benchmark, thereby closing \Cref{op:deep-gen} on
the trust and tightness axes at once?  A known unknown on the transformer side.
\end{openprob}
...
universal learning.  The frontier of formal learning theory is a single,
tightly coupled surface; the problems on it are entries into one shared
geometry, and resolving any one would deform the rest.
\end{computational}
...
where that framework reaches its current limits.  They are invitations: each
section leads with one and ends by asking what argument would close it, which
is the form a frontier takes when it is read as work to be done.
