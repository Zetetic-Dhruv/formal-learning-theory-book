...
Every preceding chapter has rested on something the reader could hold onto: a definition that settled a concept, a theorem that finished an argument, a characterization that turned a class of questions into a single combinatorial parameter.  This chapter offers no such resting place.

Its subject is what the theory cannot yet say, and the right way to read it is as a map of where the methods of Chapters~1--17 run out.  Each method runs out at a specific point: Sauer--Shelah gives exponential compression, not the conjectured linear; classical complexity bounds give vacuous generalization certificates for the networks that actually work; the trichotomy applies to binary classification and not yet to the real-valued losses that practitioners use.  These are not gaps waiting to be filled by routine extension.  They are places where a fundamentally new argument is required.

Each section leads with the specific question that remains open, presents the best partial results, identifies the precise gap between what is known and what is conjectured, and asks what kind of argument might close it.

The problems chosen are those that connect most tightly to the concept graph of this book: the compression conjecture (Chapter~\ref{ch:compression}), deep network generalization (Chapter~\ref{ch:generalization}), universal learning beyond countable classes (Chapter~\ref{ch:universal}), computational--statistical tradeoffs (Chapter~\ref{ch:computational}), and the role of Kolmogorov complexity in learning.  Other important open problems (online multiclass learnability, private agnostic learning, quantum sample complexity gaps) are noted but not developed.
...
Stated by Littlestone and Warmuth in 1986, this is among the oldest major open problems in computational learning theory.  The PAC sample complexity of a class with VC dimension $d$ is $\Theta(d/\varepsilon)$.  The conjecture asks whether that bound is explained by compression: whether it suffices, always, to store only $O(d)$ of the training points and reconstruct a consistent hypothesis from those alone.  If the answer is yes, the fundamental theorem of PAC learning would gain a structural companion, revealing that a small certificate inside every training set carries the guarantee.  Four decades have not settled the question either way.
...
\item \textbf{Special cases are resolved.}  Maximum classes (those achieving
  ...
  compression of size $d$, and classes of small recursive
...
Classical generalization bounds give a clear prediction: a model with more parameters than training examples should overfit catastrophically.  Deep networks routinely falsify that prediction.  The bounds are not vacuously wrong; they are wrong in a specific, reproducible, architecturally systematic way, and the existing theories of VC dimension, Rademacher complexity, and parameter count cannot explain the pattern.  This is among the most practically urgent open problems in learning theory precisely because the phenomenon is real and the explanation is missing.
...
Chapter~\ref{ch:computational} shows that computational hardness can prevent efficient PAC learning even when the VC dimension is finite.  But every one of those results is conditional: the hardness assumption names a specific cryptographic problem, factoring or LWE or discrete cube root, and the gap disappears if that assumption fails.  The question the field has not answered is whether the computational--statistical gap is a structural feature of learning itself, or whether it is an artifact of what we cannot yet prove unconditionally.  That distinction matters: a structural gap would mean that some learnable concept classes are simply inaccessible to any efficient algorithm, regardless of how cryptography turns out.
...
The previous four sections concern classes, dimensions, and optimization algorithms: entities that a learner can, in principle, compute with.  This section concerns what happens when the ideal learner is one that cannot be computed at all.  Solomonoff's universal prior $M$ converges to any computable environment, at a rate depending only on the Kolmogorov complexity of that environment; it is the learner that wins against any computable opponent.  The cost is that $M$ is itself uncomputable.  The question is how much of that convergence survives when $M$ is approximated by something a machine can actually run.
...
The five problems above share a common property: each has a partial answer, and the gap between that partial answer and the full resolution is precisely locatable.  The problems that follow are in a somewhat different position.  Each involves a setting where the characterization theorems of Chapters~1--17 do not yet reach, and where new combinatorial objects may be required, not just new proofs of existing ones.
...
The theory of Chapters~1--17 is a rigorous, interconnected framework: characterization theorems, tight bounds, separation results, structural analogies across dimensions.  The framework's limits are not arbitrary; they form a connected subgraph, with each gap bearing on the others.  Resolving any one of these problems would reshape the rest.  That is where the work continues.
