...
\emph{necessary} for bounded-size compression, and the conjecture concerns the
labeled case specifically.
\end{remark}
...
\end{scope}
\end{tikzpicture}
\caption[Compression and learnability.]{The qualitative core: a class is PAC learnable
  if and only if its VC dimension is finite if and only if it admits a finite compression
  scheme with side information (\leanname{fundamental_vc_compression}). How short the summary
  can be is the open frontier: a bound of size $2^{O(d)}$ is known, while $O(d)$ and even
  $\mathrm{poly}(d)$ remain open. Unlabeled compression of bounded size is impossible in
  general (the red edge).}
...
Each exercise below perturbs \Cref{fig:compression-landscape} and asks where the
perturbation lands on the verified core, on a known rung, or out on the open frontier.
...
\item \textbf{The one refuted edge.} Dropping labels from the compressed subsample makes bounded-size compression impossible in general (\cref{def:unlabeled-compression}, \cref{thm:unlabeled-fails}). Show this is the lone edge settled in the negative, and explain what it tells you about where compressibility ends.
