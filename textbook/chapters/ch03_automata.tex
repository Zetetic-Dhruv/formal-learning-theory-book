...
\caption[The language hierarchy lifted to the computability hierarchy, with learning inside it]{The Chomsky language hierarchy, lifted by G\"odel numbering to the arithmetical hierarchy of computability, with learning placed inside it. A class is identifiable in the limit exactly when a learner converges after finitely many mind changes (\leanname{FLT_Proofs.Theorem.Gold.mind_change_characterization}), which fixes Gold identification at the limit-computable level $\Delta^0_2$ and no higher.}
\label{fig:chomsky}
\end{figure}
...
\end{tikzpicture}
\caption[Restricts versus extends: conservative equivalences and generative separations]{The
\rel{restricts} and \rel{extends\_grammar} edge types as one pattern, with an automata instance and a learning instance. A restricts edge (green) removes a constraint and preserves power, appearing as an equivalence (NFA to DFA, \cref{thm:nfa-dfa}); an extends edge (orange) adds a primitive and recognizes a strictly larger class, appearing as a separation (PDA over DFA, \cref{prop:anbn}). The learning column shows the same split: criteria pinned to dimensions by equivalences and the strict online-over-PAC separation (\leanname{online_strictly_stronger_pac}).}
...
Each problem is anchored to a concept above and tagged \textsc{ku} (an articulated open question) or \textsc{uk} (a result in the literature or the development not yet absorbed into a clean general statement).
...
Each exercise perturbs one rung or edge of \cref{fig:chomsky} or \cref{fig:restricts-extends}. Work out what changes and where the result sits relative to the structure the chapter has fixed.
