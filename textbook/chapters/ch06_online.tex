...
\end{tikzpicture}
\caption[The online learning game, whose value is the Littlestone dimension]{The online learning game: each
round the adversary presents an instance, the learner predicts, and the adversary reveals the truth,
charging a mistake on disagreement, subject only to realizability. The optimal mistake bound equals the
Littlestone dimension (\leanname{optimal_mistake_bound_eq_ldim}), and the adaptive adversary makes the game
strictly harder than PAC, with thresholds on the reals (VC one, Littlestone infinite) as witness.}
...
\end{tikzpicture}
\caption[A shattered mistake tree, the witness for the Littlestone dimension]{A complete mistake tree of
depth $3$ for thresholds on $\{1,\ldots,8\}$. Each internal node presents an instance, its two edges are
the labels, and every root-to-leaf path is realized by the threshold at its leaf. A tree shattered at depth
$d$ forces any learner to make at least $d$ mistakes (highlighted: one run forcing $3$), and the Littlestone
dimension is the depth of the deepest shattered tree (\leanname{LittlestoneDim}).}
...
\end{tikzpicture}
\caption[A shattered set embeds as a non-adaptive mistake tree: $\VC\le\Ldim$]{Freezing the instances at
every depth turns a shattered set of size $d$ into a non-adaptive mistake tree of depth $d$, so
$\VC(\mathcal{H})\le\Ldim(\mathcal{H})$ always (\leanname{BranchWiseLittlestoneDim_ge_VCDim}). The inclusion
is one-way: an adaptive tree need not come from any set, and thresholds on $\R$ exhibit the gap with $\VC=1$
but $\Ldim=\infty$, so the slack $\Ldim-\VC$ measures adaptivity.}
...
Each exercise changes one feature of the online game and asks how the value $\mathrm{Opt}=\Ldim$ moves with it.
