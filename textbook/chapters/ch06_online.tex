...
Each exercise perturbs the game. Show, in the figure's grammar (an adversarial sequential protocol whose value is a combinatorial invariant, solid where a kernel theorem determines that value and dashed where the value is borrowed from the literature or open), that the generalized figure subsumes it as an \emph{instance} (a setting where the value equals the Littlestone dimension), a \emph{boundary} (where realizability is dropped or the gap to PAC becomes strict), or a \emph{frontier} (a dashed, open value).

\begin{enumerate}[leftmargin=*,itemsep=3pt]
\item \textbf{A finite-Littlestone class.} Play the game on a class of finite Littlestone dimension, such as thresholds on a finite domain. Show this is an instance: the value is determined, the optimal mistake bound equals the Littlestone dimension (\leanname{optimal_mistake_bound_eq_ldim}), and finiteness makes the class online learnable (\leanname{littlestone_characterization}).

\item \textbf{The value is two-sided.} Separate the equality into its halves. Show this is an instance whose content is the conjunction: the Standard Optimal Algorithm guarantees at most $\Ldim$ mistakes (\leanname{soa_mistakes_bounded}) and the adversary forces at least $\Ldim$ (\leanname{adversary_lower_bound}), so the value is determined, not merely bounded, only because both directions are proved.

\item \textbf{Drop realizability.} Remove the constraint that some hypothesis fits every label. Show this is a boundary the figure marks but does not cross: the mistake bound becomes vacuous, the right notion is regret against the best fixed hypothesis, and although the kernel defines bounded regret (\leanname{RegretBounded}) it proves no theorem tying regret to a dimension, so the agnostic value is drawn dashed and cited (\cite{CesaBianchiLugosi2006}, \cref{op:regret-constant}).

\item \textbf{The gap to PAC.} Compare the game with the PAC framework. Show this is the figure's principal boundary: online learnability implies PAC learnability but the converse fails, witnessed by thresholds on $\R$ with $\VC=1$ and $\Ldim=\infty$ (\leanname{online_strictly_stronger_pac}, \leanname{pac_not_implies_online}), so an adaptive adversary is strictly stronger than any random distribution.

\item \textbf{The conservative leg.} Keep only the implication that online learnability gives PAC learnability. Show this is an instance on the safe side of the gap: a finite Littlestone bound forces a finite VC bound, so the criterion adds no statistical power (\leanname{online_imp_pac}), the one-way half of the strict separation.

\item \textbf{Multiclass labels.} Replace binary labels by a finite label set. Show this is a frontier that re-types the value: the natural conjecture is that a multiclass tree-dimension still pins the optimal mistake bound, but the kernel has no multiclass online value theorem (\cref{op:online-multiclass}), so the generalized value is dashed and cited (\cite{DanielyShalevShwartz2014}).

\item \textbf{Bandit feedback.} Let the learner observe only whether its prediction was correct, not the true label. Show this is a frontier: the partial-information game has its own value, governed by a bandit analogue of the Littlestone dimension that the kernel does not formalize, so the value edge is dashed and open (\cref{op:bandit-attention}).

\item \textbf{An efficient optimal learner.} Ask whether the Standard Optimal Algorithm can be run in polynomial time per round. Show this is a frontier on the algorithmic axis: the value is achieved information-theoretically, but computing the version-space Littlestone dimensions the algorithm needs may be intractable, so the efficiency of an optimal learner is an open, dashed annotation (\cref{op:soa-computable}).

\item \textbf{Online to batch.} Convert an online learner into a batch one by averaging or by the longest-survivor rule. Show this is an instance that runs the gap backward: a mistake-bounded online learner yields a PAC learner with sample complexity governed by the mistake bound, the constructive direction underlying \leanname{online_imp_pac}, so the conversion lands inside the verified web.

\item \textbf{A minimax for the value.} Ask whether the game value is a minimax that factors through a single object, the deepest shattered mistake tree, dual to a potential the Standard Optimal Algorithm descends. Show this is the figure's most general open frontier: it would present the value not as two matched bounds but as one duality, yet no such minimax or potential duality is formalized in the kernel, so the meta-edge is dashed over an otherwise solid value.
\end{enumerate}

\subsection*{Counterfactual exercises: generalizing Figure~\ref{fig:mistake-tree}}

Each exercise perturbs the tree. Show, in the figure's grammar (a shattered mistake tree whose depth is a lower bound on mistakes and whose maximum over all shattered trees is the Littlestone dimension, solid where a kernel theorem certifies the depth and dashed where the value is borrowed or open), that the generalized tree subsumes the concrete one as an \emph{instance} (another class whose deepest shattered tree gives its dimension), a \emph{boundary} (a non-adaptive tree, or a class where tree-depth strictly exceeds set-size), or a \emph{frontier} (a dashed, open tree dimension).

\begin{enumerate}[leftmargin=*,itemsep=3pt]
\item \textbf{A deeper threshold tree.} Replace $\{1,\ldots,8\}$ by $\{1,\ldots,2^d\}$ and build the depth-$d$ binary-search tree. Show this is an instance: the kernel certifies that the threshold tree on any index window $[lo,lo+2^d)$ is shattered (\leanname{thresholdTree_shattered}), so $\Ldim \ge d$, and the concrete figure is the case $d=3$.

\item \textbf{The adversary run.} Read the highlighted root-to-leaf path as an execution rather than a drawing. Show this is the instance that gives the path its meaning: against \emph{any} learner the adversary descending a shattered depth-$d$ tree realizes exactly $d$ mistakes (\leanname{adversary_core}), so the depth is a genuine lower bound and not merely a count of nodes (\leanname{adversary_lower_bound}).

\item \textbf{Erase the labels.} Strip the instances $\{4,2,6,\ldots\}$ and threshold leaves, keeping only the shape. Show this is the figure's generalizing move: the bare complete binary tree with a realizability certificate at each leaf is the abstract witness, the threshold labels are one filling, and $\Ldim$ is the deepest such shape by definition (\leanname{LittlestoneDim}).

\item \textbf{A non-adaptive tree (the VC link).} Force every node at a given depth to present the \emph{same} instance, so the tree is a shattered \emph{set}. Show this is the boundary the figure shares with Figure~\ref{fig:vc-leq-ldim}: a shattered set of size $d$ is a special mistake tree of depth $d$, hence $\VC(\mathcal{H}) \le \Ldim(\mathcal{H})$ always (\leanname{BranchWiseLittlestoneDim_ge_VCDim}), the non-adaptive corner of the adaptive object.

\item \textbf{Depth exceeds set-size.} Take thresholds on $\R$, where the adversary bisects the remaining interval forever. Show this is the boundary where adaptivity strictly wins: the tree dimension is infinite while the shattered-set dimension is one ($\VC=1$), so $\Ldim(\mathcal{H})$ can exceed $\VC(\mathcal{H})$ without bound (\leanname{ldim_threshold_top}, \leanname{online_strictly_stronger_pac}).

\item \textbf{The deepest tree is the value.} Connect the depth bracket to the mistake bound of Figure~\ref{fig:online-protocol}. Show this is an instance that names the bracket: the optimal mistake bound equals the deepest shattered-tree depth (\leanname{optimal_mistake_bound_eq_ldim}), so the annotation ``depth $=\Ldim$'' is the verified value of the game, not a label local to this picture.

\item \textbf{A finite class by counting leaves.} Let $|\mathcal{H}| < \infty$. Show this is an instance read off the tree's width: a complete depth-$d$ tree has $2^d$ leaves each needing a distinct consistent hypothesis, so $\Ldim(\mathcal{H}) \le \lfloor \log_2 |\mathcal{H}| \rfloor$ (\Cref{ex:ldim-examples}), the cardinality ceiling on how deep a shattered tree can run.

\item \textbf{The exact threshold value.} Ask for the matching upper bound that turns $\Ldim \ge \lceil\log_2(n+1)\rceil$ into an equality on $\{1,\ldots,n\}$. Show this is a frontier the kernel only half-covers: the lower half is certified (\leanname{thresholdTree_shattered}) and the infinite case is $\Ldim=\infty$ (\leanname{ldim_threshold_top}), but the exact finite value is borrowed, so the ``$=\lceil\log_2\rceil$'' edge is drawn dashed and cited (\cite{Littlestone1988}).

\item \textbf{Multiclass trees.} Give each internal node $|Y|>2$ children, one per label. Show this is a frontier that re-types the witness: the conjecture is that a multiclass tree-dimension still pins the optimal mistake bound, but the kernel formalizes only the binary $\Ldim$, so the multiclass deepest-tree value is dashed and open (\cref{op:online-multiclass}, \cite{DanielyShalevShwartz2014}).

\item \textbf{The tree as dual of the SOA potential.} Ask whether the deepest shattered tree is one object serving twice: the adversary's strategy (this figure's lower bound) and the learner's potential, which the Standard Optimal Algorithm decreases by at least one per mistake. Show this is the most general open frontier: it would present $\Ldim$ as a single minimax through the tree rather than as two matched bounds, yet no such tree-potential duality is formalized in the kernel, so the meta-edge is dashed over an otherwise solid depth.
\end{enumerate}

\subsection*{Counterfactual exercises: generalizing Figure~\ref{fig:vc-leq-ldim}}

Each exercise perturbs the embedding. Show, in the figure's grammar (a shattered set re-read as a non-adaptive mistake tree, with a solid embedding set $\hookrightarrow$ tree giving $\VC\le\Ldim$ where a kernel theorem certifies it, and a dashed edge where adaptivity escapes the image or the value is borrowed or open), that the generalized figure subsumes the depth-$2$ picture as an \emph{instance} (a class where the embedding is tight, $\VC=\Ldim$), a \emph{boundary} (where adaptivity strictly wins, $\Ldim>\VC$, or the non-adaptive constraint itself), or a \emph{frontier} (a dashed, open or interpolating dimension).

\begin{enumerate}[leftmargin=*,itemsep=3pt]
\item \textbf{The embedding is a map.} Read the left-to-right arrow not as a picture but as the construction that sends a shattered set of size $d$ to the non-adaptive tree of depth $d$. Show this is the figure's generalizing move: the map preserves size as depth (\leanname{MistakeTree.fromList_depth}) and validity as shattering (\leanname{MistakeTree.fromList_shattered}), so $\VC(\mathcal{H})\le\Ldim(\mathcal{H})$ is the statement ``this embedding exists,'' certified by \leanname{BranchWiseLittlestoneDim_ge_VCDim}.

\item \textbf{The non-adaptive constraint is the definition.} Force every node at a given depth to carry the \emph{same} instance, both children identical. Show this is an instance that re-types the object: a tree all of whose depth-$k$ nodes present $x_{k+1}$ \emph{is} the set $\{x_1,\dots,x_d\}$, so $\VC$ is exactly $\Ldim$ restricted to non-adaptive trees (\leanname{MistakeTree.fromList}), and ``set'' and ``non-adaptive tree'' name one object in two grammars.

\item \textbf{Halfspaces: the embedding is tight.} Take halfspaces in $\R^n$, where both dimensions equal the ambient dimension. Show this is the instance where the arrow is an isomorphism of values: $\VC=\Ldim=n$, so no adaptive tree runs deeper than the shattered set already forces, and the slack $\Ldim-\VC$ is zero. The tightness is classical (\cite{Littlestone1988}); the kernel certifies only the general direction $\VC\le\Ldim$ (\leanname{BranchWiseLittlestoneDim_ge_VCDim}), so the matching reverse bound for this class is cited.

\item \textbf{Thresholds on $\R$: adaptivity wins without bound.} Replace the set by the threshold class on the real line. Show this is the figure's principal boundary, the cokernel of the embedding: the adaptive adversary bisects the surviving interval forever, so $\Ldim=\infty$ while a single point already over-shatters, $\VC=1$ (\leanname{ldim_threshold_top}, \leanname{vcdim_threshold_finite}, \leanname{online_strictly_stronger_pac}). The image of the set-into-tree map misses exactly these strategies.

\item \textbf{One unit of adaptivity.} Drop the same-instance constraint at a single depth, letting that one instance depend on the previous labels. Show this is the boundary edit that creates the gap: the single relaxation is precisely the adaptive freedom, and a class admitting an adaptive depth-$2$ tree but no shattered pair has $\Ldim\ge 2>\VC$, so the slack $\Ldim-\VC$ is the measured magnitude of adaptivity, certified one unit at a time against the set bound (\leanname{BranchWiseLittlestoneDim_ge_VCDim}, with the strict side witnessed as in the threshold case, \leanname{ldim_threshold_top}).

\item \textbf{Branch-wise versus path-consistent.} Notice that the certified inequality is stated for the branch-wise dimension, while the chapter's $\Ldim$ is the path-consistent one. Show this is a boundary the figure marks honestly: on the non-adaptive image the two notions coincide (\leanname{MistakeTree.fromList_shattered} reuses one realizing witness per branch, so branch-wise freedom buys nothing), hence $\VC\le\Ldim$ holds for the path-consistent $\Ldim$ (\leanname{LittlestoneDim}); the kernel proves the bound only against \leanname{BranchWiseLittlestoneDim}, so the bridging step is the figure's lone dashed edge.

\item \textbf{The exact threshold values.} Ask for the numbers, not just finiteness: $\VC=1$ and $\Ldim=\lceil\log_2(n{+}1)\rceil$ on $\{1,\dots,n\}$. Show this is a frontier the kernel half-covers: the gap's existence is certified ($\Ldim=\infty$ in the infinite case, \leanname{ldim_threshold_top}; $\VC<\infty$, \leanname{vcdim_threshold_finite}), but the exact constants $\VC=1$ and the $\log$ are borrowed, so those numerals are cited (\cite{Littlestone1988}) over an otherwise solid gap.

\item \textbf{Online to batch through the gap.} Read the inequality as a transfer of guarantees: a finite Littlestone bound forces a finite VC bound. Show this is the instance running the arrow on the safe side: online learnability implies PAC learnability (\leanname{online_imp_pac}, the conservative half of \leanname{online_strictly_stronger_pac}), so the embedding's existence has the operational reading that the adaptive guarantee is never weaker than the statistical one.

\item \textbf{Multiclass faces.} Give each internal node $|Y|>2$ children, one per label, and ask for one dimension over the larger label set unifying both faces. Show this is a frontier that re-types the embedding: the conjecture is that a multiclass set-dimension still embeds into a multiclass tree-dimension, but the kernel formalizes only the binary $\VC$ and $\Ldim$ (\cref{op:online-multiclass}), so the multiclass arrow is dashed and cited (\cite{DanielyShalevShwartz2014}).

\item \textbf{An adaptivity budget that interpolates.} Ask for a single parameterized dimension $\mathrm{D}_b$ that allows at most $b$ depths to depend on past labels, so $\mathrm{D}_0=\VC$ (no adaptivity) and $\mathrm{D}_\infty=\Ldim$ (full adaptivity), with the gap $\mathrm{D}_\infty-\mathrm{D}_0$. Show this is the most general open frontier: it would present $\VC$ and $\Ldim$ not as two measures joined by a one-way map but as the endpoints of one budgeted family, and would read the embedding set $\hookrightarrow$ tree as a functor whose failure to be essentially surjective is the gap, yet no such interpolating dimension or functor is formalized in the kernel, so the meta-edge is dashed over an otherwise solid inequality.
\end{enumerate}
