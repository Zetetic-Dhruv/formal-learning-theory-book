...
Two combinatorial questions, whether $\mathcal{H}$ admits an infinite Littlestone tree and whether it
admits an infinite VCL tree (\cref{def:vcl-tree}), partition every class with $|\mathcal{H}|\geq 3$ into
exactly three rate regimes: exponential, linear, and arbitrarily slow. The first boundary is exactly
online learnability (\leanname{littlestone_characterization}). There is no fourth regime and no
intermediate rate.}
...
\end{tikzpicture}
\caption[The universal-rate spectrum: one cross-paradigm dimensional ladder]{The same trichotomy as a
totally ordered spectrum, with the VC dimension (\Cref{ch:pac}) gating whether a class is learnable, the
Littlestone dimension (\Cref{ch:online}) drawing the exponential/linear bar, and the VCL tree
(\cref{def:vcl-tree}) drawing the linear/arbitrarily-slow bar. The left bar is exactly online learnability
(\leanname{littlestone_characterization}). The structural surprise is that an adversarial online quantity
governs the i.i.d.\ statistical convergence rate.}
...
Each exercise routes a class to its regime, toggles a tree-condition so the regime jumps, or pushes on the boundary between a verified threshold and a literature rate.
