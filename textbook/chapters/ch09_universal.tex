...
Knowing that $\mathcal{H}$ is learnable is only the beginning of a harder
question.  The Fundamental Theorem of \Cref{ch:pac} settles learnability:
yes if and only if $\VC(\mathcal{H}) < \infty$.  But learnability is a
binary verdict, and two classes can both clear the VC gate while converging
at completely different speeds.  Once you know learning is possible, what
you need to know next is \emph{how fast}.

PAC theory offers a partial answer through the sample complexity
$m(\varepsilon, \delta)$: how many samples suffice for accuracy $\varepsilon$
with confidence $1 - \delta$?  The bound, $\Theta(d/\varepsilon)$ in
the realizable case, describes how accuracy improves as data grows, with the
VC dimension $d$ as a constant.  What it does not tell you is the
\emph{rate} at which the risk itself decreases as a function of the sample
size $n$.  Fix $n$ and ask: how small can $\risk_D(A(S))$ be forced, for
every realizable distribution simultaneously?  That is the rate question,
and the PAC bound does not answer it.

The answer is a trichotomy: every hypothesis class
with $|\mathcal{H}| \geq 3$ falls into exactly one of three regimes,
$\Theta(e^{-n})$, $\Theta(1/n)$, or arbitrarily slow, with no intermediate
rates.  The boundary between the first two regimes is drawn by the Littlestone
dimension, a combinatorial quantity from the adversarial online setting.
That it controls statistical convergence rates was not predicted by either
learning tradition; the trichotomy theorem revealed it.
...
The PAC guarantee fixes an accuracy target $\varepsilon$ and asks how many
samples are needed to clear it.  Invert that: fix the sample count $n$ and
ask what accuracy is achievable.  This shift in viewpoint produces a
different and finer measure of a class's learnability.  Define the
\emph{learning rate} of $\mathcal{H}$ as follows.
...
$R(n) = O(d/n)$, a $\Theta(1/n)$ rate.  Three questions follow at once:
is $1/n$ always achievable, or can some classes converge exponentially fast?
And are there learnable classes where no uniform rate holds at all, where the
convergence speed depends irreducibly on which distribution is in play?
All three phenomena occur, and the trichotomy theorem says they are the
\emph{only} three.
...
The classification uses a combinatorial object that generalizes the
...
realized by some $h \in \mathcal{H}$.  What the trichotomy needs is a
variant where the consistency requirement on each path is not just
``one hypothesis passes'' but ``the full shattering condition holds.''
That demand, strictly weaker globally yet decisive for the rate question,
defines the VCL tree.
...
exist even when $\Ldim(\mathcal{H}) < \infty$.\formal{FLT_Proofs.Complexity.Ordinal.OrdinalLittlestoneDim}
\end{remark}
...
The boundary case ``$\Ldim < \infty$ iff online learnable'' is verified (\cref{thm:littlestone-characterization}).  The trichotomy theorem is a literature result; we present it as a sketch following~\cite{BousquetHannekeMoranVanHandelYehudayoff2021}, and formalizing it is \cref{op:trichotomy-formal}.
\end{remark}
...
\end{tikzpicture}
\caption[The universal-rate trichotomy: two tree-conditions, three regimes]{Two questions settle every
class's fate. Does $\mathcal{H}$ admit an infinite Littlestone tree? If no, the rate is exponential
$\Theta(e^{-n})$; this boundary is exactly online learnability
(\leanname{littlestone_characterization}), verified. If yes, does $\mathcal{H}$ admit an infinite VCL
tree (\cref{def:vcl-tree})? If no, the rate is linear $\Theta(1/n)$; if yes, no uniform rate holds
at all. Every class with $|\mathcal{H}|\geq 3$ lands in exactly one of these three regimes. There is
no fourth, and no intermediate rate such as $1/\sqrt{n}$. Solid edges carry verified decisions;
dashed boxes mark the $\Theta$-rates, which are literature results not yet machine-checked; see the
...
The trichotomy is the structural capstone of this book.  To read it clearly,
track what each learning tradition expected and where that expectation breaks.

The PAC tradition (\Cref{ch:pac}) characterized learnability through the VC
dimension and established sample complexity $\Theta(d/\varepsilon)$.  The
natural continuation: the VC dimension should also determine the rate.
It does not.  Two classes with identical VC dimension can sit in different
rate regimes, one converging exponentially, the other only at $1/n$.

The online learning tradition (\Cref{ch:online}) introduced the Littlestone
dimension to characterize mistake-bounded learning against an adversary: a
worst-case combinatorial quantity, built entirely for a worst-case adversarial
game.  No one imported it into the statistical setting.  There was no apparent
reason to.

The trichotomy theorem reveals that the Littlestone dimension is exactly what
draws the boundary between exponential and polynomial convergence in the
i.i.d.\ setting.  The same object that counts the worst-case depth of an
adversarial tree controls the rate at which an i.i.d.\ learner closes in on
zero risk.  Neither tradition anticipated this.  The deepest structural divide
in statistical learning is drawn by a quantity from a different paradigm entirely.
...
The trichotomy connects three paradigms through one combinatorial ladder.
The spectrum below lays out the three rate regimes in order, each separated
by a boundary drawn by a different dimension from a different chapter.
...
\end{tikzpicture}
\caption[The universal-rate spectrum: one cross-paradigm dimensional ladder]{The trichotomy as a totally
ordered spectrum. The VC gate (\Cref{ch:pac}) decides whether learning is possible at all. Among the learnable
classes, the left bar, exactly online learnability (\leanname{littlestone_characterization}), separates
exponential from linear convergence: the Littlestone dimension (\Cref{ch:online}), invented for adversarial
mistake-counting, draws this statistical rate boundary. The right bar, defined by the VCL tree
(\cref{def:vcl-tree}), separates uniform linear convergence from classes where no rate holds. Each bar
comes from a different chapter and a different paradigm; the trichotomy shows they form one coherent ladder.
Rate values are literature results; the evidence-grade legend is inside the figure.}
\label{fig:rate-regimes}
\end{figure}

Reading the trichotomy through all three chapters at once, a clean hierarchy
emerges: three combinatorial conditions, each one finer than the last, each
...
Each row refines the previous.  The VC dimension draws the coarsest line:
learnable or not.  Within the learnable classes, the Littlestone dimension
draws a finer line: exponential or merely polynomial.  The VCL tree draws the
finest: uniformly polynomial or arbitrarily slow.  This three-rung ladder
was not predicted by any single tradition; it emerged from the trichotomy
proof, which required tools from both the PAC and online learning literatures.
The three paradigms share one combinatorial landscape; the trichotomy is its
map.
...
The trichotomy classifies every fixed-architecture attention class.
The classification turns on one question, carried forward from \Cref{ch:online}:
is the Littlestone dimension of the attention class finite?  The answer to
that question decides whether the class converges exponentially or only linearly.
The answer is open.
...
\item \textbf{The cross-paradigm bridge.}  The Littlestone dimension, born
  ...
  statistical setting.  This was resolved by
  Bousquet et al.~\cite{BousquetHannekeMoranVanHandelYehudayoff2021}, and the
  answer came from an unexpected paradigm.
\end{enumerate}
...
The trichotomy theorem hands you a procedure: two tree-conditions, three
regimes, no in-between.  The exercises below make that procedure active.
Each one places a class, shifts a tree-condition so the regime changes,
or presses on the seam between a verified boundary and a literature rate
that is cited but not yet machine-checked.  Working through them converts
the trichotomy from a result you have read into a tool you can apply.

\begin{enumerate}[leftmargin=*,itemsep=3pt]
\item ``No infinite Littlestone tree'' is exactly $\Ldim(\mathcal{H}) < \infty$, which is exactly online learnability by the Littlestone characterization (\cref{thm:littlestone-characterization}, \leanname{littlestone_characterization}). Show that this is the only one of the trichotomy's two boundaries that reduces to a single known dimensional threshold.

\item A VCL tree (\cref{def:vcl-tree}) demands that the instances along each root-to-leaf path be shattered, strictly stronger than asking for one hypothesis consistent with the path. Conclude that every Littlestone tree is a VCL tree but not conversely (\cref{rmk:vcl-vs-littlestone}), so an infinite VCL tree can exist with $\Ldim < \infty$ and the ladder $\{\Ldim<\infty\} \subsetneq \{\text{no }\infty\text{-VCL}\} \subsetneq \{\text{all}\}$ is strict at both steps.

\item Take thresholds on $\mathbb{N}$, with $\VC = 1$ (\cref{def:vc-dim}, \leanname{vcdim_threshold_finite}) yet $\Ldim = \infty$ (\leanname{ldim_threshold_top}). Show the class clears the VC gate but has $\Ldim = \infty$ and no infinite VCL tree, placing it in the linear regime $\Theta(1/n)$.

\item Show that halfspaces in $\R^d$ have finite Littlestone dimension $d$ (\cref{ex:ldim-examples}) and hence sit in the exponential regime $\Theta(e^{-n})$ despite having infinite cardinality.

\item Show that an infinite VC dimension forces an infinite VCL tree, so any class shattering arbitrarily large finite sets lands in the arbitrarily-slow regime, where for every $R(n)\to 0$ some realizable $D$ defeats every learner (\cite{BousquetHannekeMoranVanHandelYehudayoff2021}).

\item Let $\mathcal{H} = \bigcup_k \mathcal{H}_k$ be unions of finitely many intervals on $\R$ with no bound on the number. Show it has finite VC dimension, so it clears the whether-learnable gate, yet admits infinite VCL trees and so lands in the arbitrarily-slow regime: the linear/slow boundary is strictly finer than the VC gate.

\item A finite-head attention class on $\R^d$ has finite VC dimension, ruling out an infinite VCL tree (\cref{prop:attention-trichotomy}), so it is never arbitrarily slow. Show that whether it is exponential or linear is decided by its Littlestone dimension, whose finiteness for attention is open (\cref{op:attention-regime}, \cref{op:ldim-attention}).

\item The same Littlestone dimension that bounds worst-case mistakes in the adversarial online setting of \Cref{ch:online} draws the exponential/linear boundary in the i.i.d.\ setting (\cref{sec:surprise}). A single structural reason for this cross-paradigm coincidence is open (\cref{op:universal-online-bridge}).

\item Suppose an ordinal-valued measure had finiteness equivalent to ``no infinite VCL tree''. Show this would render the linear/slow boundary a clean threshold matching ``$\Ldim < \infty$'', so a single ordinal invariant with three value-ranges would order the whole spectrum. Whether such a dimension exists is open (\cref{op:vcl-characterization}).
\end{enumerate}
