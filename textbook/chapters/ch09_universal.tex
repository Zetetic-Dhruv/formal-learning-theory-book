...
Each exercise perturbs the rate spectrum.  Argue, in the figure's grammar (a totally ordered three-regime spectrum, exponential inside linear inside arbitrarily-slow, separated by two boundary bars and induced by one dimensional ladder of increasing fineness: the VC dimension gates whether a class is learnable, the Littlestone dimension draws the left bar, the VCL tree draws the right bar; where the bars are drawn solid because a verified dimensional threshold or the VCL tree definition certifies them, and the rate values that name the regimes are drawn dashed because they are Bousquet--Hanneke--Moran--van Handel--Yehudayoff's theorem, cited not proved), that the perturbed picture subsumes the concrete one as an \emph{instance} (a class placed in its regime by its dimension), a \emph{boundary} (one dimension toggled so the class crosses a bar), or a \emph{frontier} (a dashed, open or literature-only edge).

\begin{enumerate}[leftmargin=*,itemsep=3pt]
\item \textbf{The VC dimension is the gate, not a bar.} Read the spectrum bottom-up: ask first whether $\mathcal{H}$ is learnable at all. Show this is the instance that frames the whole picture: a finite VC dimension (\cref{def:vc-dim}) is what places a class in \emph{one of} the three regimes, since finite VC rules out an infinite VCL tree, and an infinite VC dimension drops the class off the spectrum entirely (not universally learnable); the VC dimension therefore sits underneath all three regimes as the whether-learnable rung of the ladder, the coarsest line drawn by \Cref{ch:pac}, while the Littlestone and VCL bars draw the two how-fast lines above it.

\item \textbf{The left bar is the only verified rate boundary.} Read the exponential/linear bar not as a regime label but as a dimensional threshold. Show this is the spectrum's defining instance: the bar is exactly $\Ldim(\mathcal{H}) < \infty$ versus $\Ldim(\mathcal{H}) = \infty$, which is exactly online learnability versus not, by the Littlestone characterization (\cref{thm:littlestone-characterization}, \leanname{littlestone_characterization}); this is the one boundary on the spectrum that the kernel certifies, so the bar is solid even though the $\Theta(e^{-n})$ and $\Theta(1/n)$ labels beside it are dashed.

\item \textbf{The threshold family straddles the left bar.} Instantiate $\mathcal{H}$ as thresholds (on $\mathbb{N}$, the verified witness for thresholds on $\R$). Show this is the boundary that lands on the middle regime: the class has VC dimension one, hence finite (\leanname{vcdim_threshold_finite}), yet Littlestone dimension infinite (\leanname{ldim_threshold_top}), so it sits to the learnable side of the VC gate but to the $\Ldim = \infty$ side of the left bar, placing it in the linear regime; the two dimensions are verified and pin the class's \emph{position}, while its $\Theta(1/n)$ \emph{rate} is the dashed literature claim (\cref{op:trichotomy-formal}).

\item \textbf{Halfspaces sit in the exponential regime.} Replace $\mathcal{H}$ by halfspaces in $\R^d$. Show this is the instance on the left regime: halfspaces have finite Littlestone dimension $d$ (\cref{ex:ldim-examples}), so the class answers ``no'' at the left bar and lands in the exponential regime despite infinite cardinality; its membership to the left of the bar is certified by the Littlestone characterization (\leanname{littlestone_characterization}), and only the $\Theta(e^{-n})$ rate that names the regime is the dashed value (\cref{op:trichotomy-formal}).

\item \textbf{An infinite VC dimension drops the class off the spectrum.} Take a class that shatters arbitrarily large finite sets. Show this is the boundary that exits the picture: infinite VC dimension forces an infinite VCL tree (every path shatters its instances), so the class crosses the right bar into the arbitrarily-slow regime and is not universally learnable at any uniform rate; the regime box is solid as a typed object (\leanname{UniversalLearnable}), but the lower bound ``for every $R(n)\to 0$, some realizable $D$ defeats every learner'' is the theorem, dashed and not kernel-proved (\cref{op:trichotomy-formal}).

\item \textbf{Unbounded unions reach the right regime with finite VC.} Instantiate $\mathcal{H}$ as $\bigcup_k \mathcal{H}_k$, unions of finitely many intervals on $\R$ with no bound on the number. Show this is the instance that distinguishes the right bar from the VC gate: the class has finite VC dimension, so it clears the whether-learnable gate, yet admits infinite VCL trees, so it still lands in the arbitrarily-slow regime; this is why the right bar is a genuinely finer line than the VC gate, and why no single finite-VC argument places a class on the spectrum without consulting its VCL trees (\cref{op:vcl-characterization}).

\item \textbf{The rates are typed but unproved.} Ask the kernel for the statement ``this class learns at rate $\Theta(e^{-n})$''. Show this is the spectrum's principal frontier: the three regimes are typed (the universal-rate criterion \cref{def:universal-rate} is \leanname{FLT_Proofs.Criterion.Extended.UniversalLearnable}), so the regime boxes are well-typed objects, but the rate \emph{values} and the ``exactly one of three'' partition are not machine-checked; the trichotomy theorem and its middle branch are pending and the universal-to-PAC direction rests on an unfinished boosting lemma, which is exactly \cref{op:trichotomy-formal} (\cite{BousquetHannekeMoranVanHandelYehudayoff2021}), so every rate label on the spectrum is dashed.

\item \textbf{An online quantity draws a statistical line.} Read the left bar across paradigms. Show this is the frontier that makes the spectrum surprising: the same Littlestone dimension that bounds worst-case mistakes against an adversary in \Cref{ch:online} draws the exponential/linear boundary in the i.i.d.\ statistical setting (\cref{sec:surprise}), so the spectrum's left bar is imported wholesale from a different paradigm; a single structural reason for this cross-paradigm coincidence, beyond the trichotomy proof, is open (\cref{op:universal-online-bridge}).

\item \textbf{Collapse the right bar to a single dimension.} Suppose someone exhibits an ordinal-valued measure whose finiteness equals ``no infinite VCL tree''. Show this would make the spectrum symmetric: the linear/arbitrarily-slow bar would read ``$\dim < \infty$'' the way the left bar reads ``$\Ldim < \infty$'', so a single ordinal invariant with three value-ranges would order the entire spectrum and both bars would become solid dimensional thresholds; whether such a dimension exists is open (\cref{op:vcl-characterization}, building on \leanname{FLT_Proofs.Complexity.Ordinal.OrdinalLittlestoneDim}).

\item \textbf{Leave the realizable case and the spectrum may grow a regime.} Relabel the regimes for agnostic or bounded-noise data. Show this is the frontier that may add a bar: the three-regime spectrum is sharp only in the realizable setting, and whether an intermediate rate such as $1/\sqrt{n}$ appears, splitting a regime or inserting a fourth, is open (\cref{op:intermediate-rates}); the figure's three-regime totality is therefore a realizable-case claim, and the multiclass version with the fat-shattering tree in place of the VCL tree may refine it further (\cref{op:universal-multiclass}, \Cref{ch:extensions}).
\end{enumerate}
