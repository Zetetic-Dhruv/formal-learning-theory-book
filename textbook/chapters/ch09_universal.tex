...
\item \textbf{Expected risk in place of high-probability.}  We use
...
\centering
\begin{tikzpicture}[
    regime/.style={draw, thick, ellipse, minimum width=3.5cm, minimum height=2.0cm,
      font=\small, align=center},
    rate/.style={draw, densely dashed, rounded corners, font=\small\bfseries, inner sep=2pt},
    example/.style={font=\scriptsize, align=center, text=notegray},
    hdl/.style={font=\tiny, text=defblue!55!black, align=center},
    srctag/.style={font=\scriptsize\itshape, text=defblue, align=center},
    solidbar/.style={very thick, {Stealth[length=2mm]}-{Stealth[length=2mm]}},
    gate/.style={draw, semithick, rounded corners, fill=black!4, draw=black!45,
      font=\scriptsize, align=center},
    conn/.style={thin, densely dashed, notegray, -{Stealth[length=1.5mm]}},
]
% --- three regimes: typed objects (SOLID) ---
\node[regime, fill=sepgreen!10, draw=sepgreen!60!black] (exp) at (-3.6,0) {};
\node[regime, fill=defblue!10, draw=defblue!60!black] (lin) at (0,0) {};
\node[regime, fill=thmred!10, draw=thmred!60!black] (slow) at (3.6,0) {};
% --- rate VALUES: BHMR literature/pending -> DASHED tags inside the top ---
\node[rate, draw=sepgreen!65!black, text=sepgreen!40!black] at (-3.6,0.55) {$\Theta(e^{-n})$};
\node[rate, draw=defblue!65!black, text=defblue!45!black] at (0,0.55) {$\Theta(1/n)$};
\node[rate, draw=thmred!65!black, text=thmred!55!black] at (3.6,0.55) {arb.\ slow};
% --- example classes ---
\node[example] at (-3.6,-0.45) {finite classes,\\halfspaces, conjunctions};
\node[example] at (0,-0.4) {thresholds,\\intervals on $\R$};
\node[example] at (3.6,-0.45) {$\bigcup_k\mathcal{H}_k$\\unbounded unions};
\node[hdl, anchor=north] at (0,-1.15) {VC$=1$, $\Ldim=\infty$\\\leanname{vcdim_threshold_finite}\\\leanname{ldim_threshold_top}};
% --- boundary bars: left VERIFIED, right typed-object (both SOLID); rate consequence dashed ---
\draw[solidbar, sepgreen!70!black] (-1.8,-1.5) -- (-1.8,1.7)
   node[above, font=\scriptsize, align=center, text=sepgreen!70!black]
   {$\Ldim<\infty$ vs.\ $\Ldim=\infty$\\{\tiny\leanname{littlestone_characterization} (verified)}};
\draw[solidbar, thmred!70!black] (1.8,-1.5) -- (1.8,1.7)
   node[above, font=\scriptsize, align=center, text=thmred!70!black]
   {no $\infty$-VCL vs.\ $\infty$-VCL\\{\tiny\cref{def:vcl-tree} (\leanname{VCLTree})}};
% --- cross-chapter dimensional sources ---
\node[srctag] at (-3.6,-2.25) {Ch.~6 Online\\(Littlestone dim)};
\node[srctag] at (0,-2.25) {Ch.~5 PAC\\(VC dim)};
\node[srctag] at (3.6,-2.25) {Ch.~9 Universal\\(VCL trees)};
\draw[conn] (-3.6,-1.85) to[out=75,in=-100] (-1.8,-1.55);
\draw[conn] (3.6,-1.85) to[out=105,in=-80] (1.8,-1.55);
\draw[conn] (0,-2.7) -- (0,-3.05);
% --- VC gate: the whether-learnable rung (ch5), bottom of the ladder ---
\node[gate, text width=11.6cm] (gate) at (0,-3.45)
   {VC gate (\Cref{ch:pac}): $\VC(\mathcal{H})<\infty\iff\mathcal{H}$ lies in one of the three regimes; $\VC=\infty\Rightarrow$ not universally learnable.};
% --- evidence-grade legend ---
\node[font=\scriptsize\itshape, text=black!65, align=center, text width=12cm, anchor=north] at (0,-4.15)
   {Solid bars: verified or typed boundaries. Dashed rate tags: the values $\Theta(e^{-n})$, $\Theta(1/n)$, arbitrarily slow are \cite{BousquetHannekeMoranVanHandelYehudayoff2021}, cited not yet machine-checked (\cref{op:trichotomy-formal}).};
\end{tikzpicture}
\caption[The universal-rate spectrum: one cross-paradigm dimensional ladder]{The universal learning-rate
landscape as a totally ordered spectrum of three regimes, partitioned by two boundary bars and induced by
a single ladder of combinatorial dimensions drawn from three paradigms. The VC dimension (\Cref{ch:pac})
is the coarsest rung, gating \emph{whether} a class is learnable at all; within the learnable classes the
Littlestone dimension (\Cref{ch:online}, an online and adversarial quantity) draws the left boundary
between the exponential and linear regimes, and this boundary is the one rate threshold the development
verifies, since a finite Littlestone dimension is exactly online learnability
(\leanname{littlestone_characterization}). The VCL tree (\cref{def:vcl-tree}), a hybrid of VC shattering
and Littlestone trees, draws the right boundary between the linear and arbitrarily-slow regimes. The bars
and the regime boxes are typed objects, drawn solid; the rate \emph{values} that name the regimes, and the
claim that exactly one holds, are the theorem of Bousquet--Hanneke--Moran--van Handel--Yehudayoff, cited
and not yet machine-checked (\cref{op:trichotomy-formal}), so the rate tags are dashed. Each example class
is placed by its dimension: the threshold family, with VC dimension one yet infinite Littlestone dimension,
straddles the left bar into the linear regime. The structural surprise is that an adversarial, online
quantity governs the i.i.d.\ statistical convergence rate.}
...
\item \textbf{Universal learning rates.}  The optimal learning rate asks a
  harder question than PAC sample complexity.  PAC fixes an accuracy and
  counts samples; the rate fixes the sample count $n$ and asks how fast the
  risk decreases, uniformly over all realizable distributions.
...
\item \textbf{The cross-paradigm bridge.}  The Littlestone dimension, born
  ...
  Bousquet et al.~\cite{BousquetHannekeMoranVanHandelYehudayoff2021}, which
  settled both what the three rate regimes are and what combinatorial objects
  govern them; and the answer came from an unexpected paradigm.
\end{enumerate}
...
The decision tree of Figure~\ref{fig:trichotomy-tree} is two solid rungs feeding three dashed leaves: a ladder of tree-conditions of increasing permissiveness (finite Littlestone dimension sitting inside no-infinite-VCL-tree sitting inside all classes), with the two decision nodes drawn solid because a verified ordinal dimension or the VCL tree definition certifies each threshold, and the three rate leaves drawn dashed because the rate value and the ``exactly one of three'' partition are the Bousquet--Hanneke--Moran--van Handel--Yehudayoff theorem, cited here rather than machine-checked.  The ten perturbations below each push on one rung or one leaf.  Each one lands in one of three places: an \emph{instance} routes a particular class to its regime, a \emph{boundary} toggles a single tree-condition so the regime jumps, and a \emph{frontier} touches a dashed edge where the picture passes from the kernel to the literature or to an open question.  Where a perturbation lands tells you how much of that part of the figure is actually proved.

\begin{enumerate}[leftmargin=*,itemsep=3pt]
\item \textbf{The first diamond is the online-learnable boundary.} The yes/no question on Littlestone trees is, underneath, a verified dimensional threshold, and reading it that way exposes the figure's defining instance on the exponential branch. ``No infinite Littlestone tree'' is exactly $\Ldim(\mathcal{H}) < \infty$, which is exactly online learnability by the Littlestone characterization (\cref{thm:littlestone-characterization}, \leanname{littlestone_characterization}), so the \textsc{no} edge into the $\Theta(e^{-n})$ leaf is the only rate-regime boundary the kernel certifies; the ordinal refinement used for the universal setting is \leanname{FLT_Proofs.Complexity.Ordinal.OrdinalLittlestoneDim}.

\item \textbf{The VCL tree generalizes the Littlestone tree by shattering.} Replace the second diamond's ``per-path consistency'' by ``per-path shattering'' and the instance that names the second object appears. A VCL tree (\cref{def:vcl-tree}) demands that the instances along each root-to-leaf path be shattered, a strictly stronger demand than asking for one hypothesis consistent with the path, so every Littlestone tree is a VCL tree and the converse fails (\cref{rmk:vcl-vs-littlestone}); this is why an infinite VCL tree can exist with $\Ldim < \infty$, making decision two strictly weaker than decision one and the ladder $\{\Ldim<\infty\} \subsetneq \{\text{no }\infty\text{-VCL}\} \subsetneq \{\text{all}\}$ strict at both steps.

\item \textbf{The leaves are typed regimes, the rates are not theorems yet.} Ask the kernel for the statement ``this class learns at rate $\Theta(e^{-n})$'' and you reach the figure's principal frontier. The three regimes are typed (the universal-rate criterion \cref{def:universal-rate} is \leanname{FLT_Proofs.Criterion.Extended.UniversalLearnable}, the ordinal dimensions exist), so the leaf \emph{boxes} are well-typed objects, yet the rate \emph{values} $\Theta(e^{-n})$, $\Theta(1/n)$, and arbitrarily-slow, together with the ``exactly one of three'' claim, are not machine-checked; closing them, by formalizing the middle-branch doubling construction and completing the boosting lemma in the universal-to-PAC direction, is exactly \cref{op:trichotomy-formal} (\cite{BousquetHannekeMoranVanHandelYehudayoff2021}), so those edges are dashed.

\item \textbf{Collapse the second diamond to a single dimension.} Suppose someone exhibits an ordinal-valued measure whose finiteness equals ``no infinite VCL tree''. Such a measure would redraw decision two as a clean threshold matching decision one: the linear/arbitrarily-slow boundary would read ``$\dim < \infty$'' the way the exponential/linear boundary reads ``$\Ldim < \infty$'', unifying both diamonds into one ordinal invariant with three value-ranges. This single-invariant question is open (\cref{op:vcl-characterization}, building on \leanname{FLT_Proofs.Complexity.Ordinal.OrdinalLittlestoneDim}).

\item \textbf{Shrink the class to a single threshold family.} Instantiate $\mathcal{H}$ as thresholds on $\mathbb{N}$, the boundary case that lands squarely on the middle leaf. Thresholds have $\VC = 1 < \infty$ (\cref{def:vc-dim}) yet $\Ldim = \infty$, so decision one answers \textsc{yes} and decision two answers \textsc{no} (no infinite VCL tree), placing the class in the linear regime $\Theta(1/n)$; the witness is the threshold class of \Cref{ch:online}, whose $\Ldim=\infty$ is constructive while its $\Theta(1/n)$ rate is the dashed literature claim (\cref{op:trichotomy-formal}).

\item \textbf{Force the bottom leaf with an infinite VC dimension.} Take a class that shatters arbitrarily large finite sets and you cross the boundary into the arbitrarily-slow regime. Infinite VC dimension forces an infinite VCL tree (a tree whose every path shatters its instances), so decision two answers \textsc{yes} and no learner achieves any uniform rate; the leaf is dashed because the lower bound ``for every $R(n)\to 0$, some realizable $D$ defeats every learner'' is the cited theorem and remains kernel-pending (\cref{op:trichotomy-formal}, \cite{BousquetHannekeMoranVanHandelYehudayoff2021}).

\item \textbf{A fixed attention head is never arbitrarily slow.} Replace $\mathcal{H}$ by a finite-head attention class on $\mathbb{R}^d$, an instance that prunes the bottom branch off the tree. Finite VC dimension rules out an infinite VCL tree (\cref{prop:attention-trichotomy}, \leanname{FLT_Proofs.Criterion.Extended.UniversalLearnable}), so decision two answers \textsc{no} and the head is universally learnable; whether it sits in the exponential or the linear leaf is decided by its Littlestone dimension, whose finiteness for attention is itself open (\cref{op:attention-regime}, \cref{op:ldim-attention}).

\item \textbf{The Littlestone dimension is an online quantity drawing a statistical line.} Read decision one across paradigms and you are standing at the frontier that makes the tree surprising. The same invariant that bounds worst-case mistakes in the adversarial online setting of \Cref{ch:online} draws the exponential/linear boundary in the i.i.d.\ statistical setting (\cref{sec:surprise}), and a single structural reason for this cross-paradigm coincidence, beyond the trichotomy proof, is open (\cref{op:universal-online-bridge}).

\item \textbf{Leave the realizable case and the partition may refine.} Relabel the leaves for agnostic or bounded-noise data and you reach a frontier that may grow new regimes. The trichotomy is sharp only in the realizable setting, and whether intermediate rates such as $1/\sqrt{n}$ appear, splitting a leaf or introducing a fourth, is open (\cref{op:intermediate-rates}); the figure's ``no fourth regime'' annotation holds for the realizable case alone and carries no further.

\item \textbf{Swap the VCL tree for a multiclass tree.} Replace binary labels by a finite or real-valued label set and the VCL tree by the DS or fat-shattering tree, a swap that tests the ladder's stability at its frontier. Whether the three-regime structure persists with the new combinatorial object in the second diamond, or splits further, is open (\cref{op:universal-multiclass}, \Cref{ch:extensions}); decision two's object is paradigm-relative, while decision one's $\Ldim$ threshold is the fixed verified rung.
\end{enumerate}

\subsection*{Counterfactual exercises: generalizing Figure~\ref{fig:rate-regimes}}

Where Figure~\ref{fig:trichotomy-tree} is a branching ladder, Figure~\ref{fig:rate-regimes} lays the same content out as a line: a totally ordered spectrum running exponential, then linear, then arbitrarily-slow, cut by two boundary bars and induced by one dimensional ladder of increasing fineness.  The VC dimension gates whether a class is learnable at all; the Littlestone dimension draws the left bar; the VCL tree draws the right bar.  The bars are solid because a verified dimensional threshold or the VCL tree definition certifies each, while the rate values that name the regimes are dashed because they are the Bousquet--Hanneke--Moran--van Handel--Yehudayoff theorem, cited rather than proved.  Each perturbation below either places a class somewhere on the line by its dimension (an \emph{instance}), toggles a dimension so the class crosses a bar (a \emph{boundary}), or pushes on a dashed edge that runs into the literature or an open problem (a \emph{frontier}).  The left half of the spectrum is the verified half; the right bar and every rate label are where the proof obligations still sit.

\begin{enumerate}[leftmargin=*,itemsep=3pt]
\item \textbf{The VC dimension is the gate beneath the bars.} Read the spectrum bottom-up: ask first whether $\mathcal{H}$ is learnable at all. This is the instance that frames the whole picture. A finite VC dimension (\cref{def:vc-dim}) is what places a class in \emph{one of} the three regimes, since finite VC rules out an infinite VCL tree, and an infinite VC dimension drops the class off the spectrum entirely (it is then not universally learnable); the VC dimension therefore sits underneath all three regimes as the whether-learnable rung of the ladder, the coarsest line drawn by \Cref{ch:pac}, while the Littlestone and VCL bars draw the two how-fast lines above it.

\item \textbf{The left bar is the only verified rate boundary.} The exponential/linear bar is a dimensional threshold wearing a regime label, and reading the threshold underneath gives the spectrum's defining instance. The bar is exactly $\Ldim(\mathcal{H}) < \infty$ versus $\Ldim(\mathcal{H}) = \infty$, which is exactly online learnability versus not, by the Littlestone characterization (\cref{thm:littlestone-characterization}, \leanname{littlestone_characterization}); this is the one boundary on the spectrum that the kernel certifies, so the bar is solid even though the $\Theta(e^{-n})$ and $\Theta(1/n)$ labels beside it are dashed.

\item \textbf{The threshold family straddles the left bar.} Instantiate $\mathcal{H}$ as thresholds (on $\mathbb{N}$, the verified witness for thresholds on $\R$), the boundary case that comes to rest on the middle regime. The class has VC dimension one, hence finite (\leanname{vcdim_threshold_finite}), yet Littlestone dimension infinite (\leanname{ldim_threshold_top}), so it sits to the learnable side of the VC gate but to the $\Ldim = \infty$ side of the left bar, placing it in the linear regime; the two dimensions are verified and pin the class's \emph{position}, while its $\Theta(1/n)$ \emph{rate} is the dashed literature claim (\cref{op:trichotomy-formal}).

\item \textbf{Halfspaces sit in the exponential regime.} Replace $\mathcal{H}$ by halfspaces in $\R^d$ for an instance squarely in the left regime. Halfspaces have finite Littlestone dimension $d$ (\cref{ex:ldim-examples}), so the class answers ``no'' at the left bar and lands in the exponential regime despite infinite cardinality; its membership to the left of the bar is certified by the Littlestone characterization (\leanname{littlestone_characterization}), and only the $\Theta(e^{-n})$ rate that names the regime is the dashed value (\cref{op:trichotomy-formal}).

\item \textbf{An infinite VC dimension drops the class off the spectrum.} Take a class that shatters arbitrarily large finite sets, the boundary case that exits the picture. Infinite VC dimension forces an infinite VCL tree (every path shatters its instances), so the class crosses the right bar into the arbitrarily-slow regime and admits no uniform rate at all; the regime box is solid as a typed object (\leanname{UniversalLearnable}), while the lower bound ``for every $R(n)\to 0$, some realizable $D$ defeats every learner'' stays dashed as a cited, kernel-pending theorem (\cref{op:trichotomy-formal}).

\item \textbf{Unbounded unions reach the right regime with finite VC.} Instantiate $\mathcal{H}$ as $\bigcup_k \mathcal{H}_k$, unions of finitely many intervals on $\R$ with no bound on the number: the instance that separates the right bar from the VC gate. The class has finite VC dimension, so it clears the whether-learnable gate, yet admits infinite VCL trees, so it still lands in the arbitrarily-slow regime; this is why the right bar is a genuinely finer line than the VC gate, and why no single finite-VC argument places a class on the spectrum without consulting its VCL trees (\cref{op:vcl-characterization}).

\item \textbf{The rates are typed but unproved.} Ask the kernel for the statement ``this class learns at rate $\Theta(e^{-n})$'' and you arrive at the spectrum's principal frontier. The three regimes are typed (the universal-rate criterion \cref{def:universal-rate} is \leanname{FLT_Proofs.Criterion.Extended.UniversalLearnable}), so the regime boxes are well-typed objects, yet the rate \emph{values} and the ``exactly one of three'' partition are not machine-checked; the trichotomy theorem and its middle branch are pending and the universal-to-PAC direction rests on an unfinished boosting lemma, which is exactly \cref{op:trichotomy-formal} (\cite{BousquetHannekeMoranVanHandelYehudayoff2021}), so every rate label on the spectrum is dashed.

\item \textbf{An online quantity draws a statistical line.} Read the left bar across paradigms and you reach the frontier that makes the spectrum surprising. The same Littlestone dimension that bounds worst-case mistakes against an adversary in \Cref{ch:online} draws the exponential/linear boundary in the i.i.d.\ statistical setting (\cref{sec:surprise}), so the spectrum's left bar is imported wholesale from a different paradigm; a single structural reason for this cross-paradigm coincidence, beyond the trichotomy proof, is open (\cref{op:universal-online-bridge}).

\item \textbf{Collapse the right bar to a single dimension.} Suppose someone exhibits an ordinal-valued measure whose finiteness equals ``no infinite VCL tree''. Such a measure would make the spectrum symmetric: the linear/arbitrarily-slow bar would read ``$\dim < \infty$'' the way the left bar reads ``$\Ldim < \infty$'', so a single ordinal invariant with three value-ranges would order the entire spectrum and both bars would become solid dimensional thresholds. Whether such a dimension exists is open (\cref{op:vcl-characterization}, building on \leanname{FLT_Proofs.Complexity.Ordinal.OrdinalLittlestoneDim}).

\item \textbf{Leave the realizable case and the spectrum may grow a regime.} Relabel the regimes for agnostic or bounded-noise data and you reach a frontier that may add a bar. The three-regime spectrum is sharp only in the realizable setting, and whether an intermediate rate such as $1/\sqrt{n}$ appears, splitting a regime or inserting a fourth, is open (\cref{op:intermediate-rates}); the figure's three-regime totality is therefore a realizable-case claim, and the multiclass version with the fat-shattering tree in place of the VCL tree may refine it further (\cref{op:universal-multiclass}, \Cref{ch:extensions}).
\end{enumerate}
