...
Every typed object in this chapter has a counterpart in a machine-checked Lean~4 development, and every theorem whose proof we give is mirrored there. Where a result has a verified companion we name it in a footnote; where one does not exist, we cite the literature instead.
...
\end{tikzpicture}
\caption[The concept class as the vertex of an equivalence]{\textbf{The concept class as the vertex of an equivalence.} Several apparently different demands one might place on a class of concepts (learnability from few examples; finite shattering; reconstructibility of every consistent labelling from a bounded subsample with a little side information; vanishing correlation with random noise; sub-exponential growth of its labellings; a finite sufficient sample) are not separate properties but a single property wearing several faces, and the class is the one object on which they coincide. The figure draws that coincidence as a wheel whose hub is the dimension counting shatterable points and whose spokes are two-way bridges, each checked in full by a proof assistant; the wheel as a whole is one assembled theorem, so the picture is the abstract shape those machine-checked equivalences instantiate. A tighter wheel sits inside, online learning with mistakes counted as they occur, joined by a single one-way bridge, since the online demand implies the statistical one but not conversely. The wheel rests on a measurability gate that a smoothed attention head passes but a hard, argmax-routed one does not; and a dashed frontier marks the same coincidence expected but not yet verified here (many-valued labels, agnostic learning, compression without side information), with one edge known to break for partial concepts, where finite dimension and learnability come apart.}
...
The reason the concept class is the hub, rather than any one paradigm or measure, is a single theorem. To state it we need the first of the measures.
...
\item \textbf{The characterization is representation-independent.} \Cref{thm:vc-char} holds regardless of whether learning is proper or improper. What decides learnability is $\VC(\mathcal{C})$, the VC dimension of the \emph{target} class, not any property of the learner's hypothesis space $\mathcal{H}$. The learner's freedom to range outside $\mathcal{C}$ affects cost, not possibility.
\end{enumerate}
...
\]
the set of hypotheses consistent with all observed data.\formal{FLT_Proofs.Complexity.GameInfra.versionSpace} The verified development carries both this operational form and a bundled structure $\leanname{VersionSpace}$ packaging $\mathcal{H}$, the data, and the consistency proof obligations together.\formal{FLT_Proofs.Basic.VersionSpace}
\end{defn}

Version-space elimination (returning any element of $\mathrm{VS}_{\mathcal{H},S}$) is perhaps the most natural learning algorithm imaginable. Its basic behavior is captured by four propositions, each a one-line consequence of the definition, and each the prototype for the convergence arguments of \Cref{ch:gold}.
...
Inductive bias is not a design choice but a \emph{mathematical necessity}, and the reason is the No-Free-Lunch theorem. We prove it through its combinatorial engine, then read off the classical statement and the necessity of bias as corollaries.
...
\end{tikzpicture}
\caption[The compression core of model selection]{\textbf{The compression core of model selection.} Three principles for preferring a simpler hypothesis (Bayesian inference, minimum description length, minimum message length) are usually treated as one idea. This figure separates them around the single structural fact the chapter actually proves: a class can be learned exactly when its data admit a bounded summary that reconstructs every consistent labelling from a short list of the examples together with a finite tag. That equivalence between learnability and bounded compression is the spine, drawn as a machine-checked two-way relationship. The three principles hang off it as approximations, each a way of scoring simplicity. Only two of the surrounding links are theorems rather than analogies, and the figure draws them solid: the bound tying a prior-weighted complexity to generalization, and the quantitative limit a tag-free summary already forces on a class. Every link among the three principles is dashed, because the verified library contains no proof of their equivalence and the figure does not pretend otherwise. One qualification stays visible: the proven equivalence permits a finite auxiliary tag, and whether that tag can always be removed while keeping the summary short is open, shown as a dashed frontier.}
...
\begin{enumerate}[leftmargin=*,itemsep=3pt]
\item The concept class is a hub node: the same object is measured by 13 inequivalent complexity measures, and one of them, the VC dimension, decides learnability outright (\cref{thm:vc-char}). The multiplicity is not redundancy; it reflects 13 genuinely different aspects of a single object.
...
The vocabulary of this chapter is not confined to the classical examples. A single softmax-attention head, read as a binary predictor, is a concept in the exact sense of \cref{def:domain-label-concept}, a function $X \to \{0,1\}$, and letting its parameters vary produces a concept class in the sense of \cref{def:concept-class}. The question the chapter has asked of every object now returns with a sharp answer for the central object of modern machine learning: \emph{is the attention class a legitimate concept class, one to which the VC characterization applies?} Answering it requires exactly the measurability that \cref{thm:vc-char} deferred to \Cref{ch:generalization}, and that is where this section earns its place: it is the first point in the book where those ``mild measurability conditions'' are made concrete, and shown to be non-vacuous.
...
So far this is only a particular concept class among many.  Its first non-trivial property is that it is a \emph{well-behaved} object: the measurability hypothesis that \cref{thm:vc-char} quietly assumes is satisfied, machine-checked.
...
The proof used continuity of the softmax in an essential way.  Removing it is not a cosmetic change: it carries the attention class across a boundary in descriptive set theory.
...
The following thirteen problems are research-grade. Each is anchored to a concept introduced above and tagged by its epistemic character: a \emph{known unknown} (\textsc{ku}) is an articulated question awaiting an answer; an \emph{unknown known} (\textsc{uk}) is a regularity present in the verified development or the literature that the surrounding theory has not yet absorbed into a clean statement. Where a verified handle exists, it is named.
...
\Cref{cor:bias-mandatory} makes bias mandatory but offers no measure of how much bias a class encodes. Construct a complexity measure $b(\mathcal{C})$, an ``inductive-bias dimension'', that is monotone under inclusion, well-behaved under products, and lower-bounds achievable sample complexity uniformly over distributions, so that the No-Free-Lunch barrier becomes quantitative rather than merely qualitative. The typed bias of \cref{def:inductive-bias-typed} (a preference score with a monotonicity constraint) is the natural carrier for such a measure.
\end{openprob}
...
For each edge of the triangle in \cref{fig:model-selection-triangle}, either supply the missing equivalence witness or prove non-equivalence as a theorem rather than an obstruction. Concretely: characterize the parametric model families on which two-part MML coincides with Bayesian MAP beyond the constant-Fisher-information case, and decide whether the residual discrepancy can be absorbed into the side information of a compression scheme.
\end{openprob}
...
\Cref{prop:soft-hard-borel} shows that argmax (hard) attention can fall outside the measurable concept classes, with an analytic-not-Borel bad event (\leanname{soft_vs_hard_attention_measurabilitySeparation}).  Identify a refined class of objects, an o-minimal or otherwise tame family of hypothesis classes, that contains hard attention and to which a uniform-convergence theory still applies, recovering hard attention as a legitimate learning object rather than a measurability pathology.
\end{openprob}
...
\subsection*{Counterfactual exercises: generalizing Figure~\ref{fig:concept-class-hub}}

Each exercise perturbs the claim of \Cref{fig:concept-class-hub} that the listed conditions coincide. Show, in the figure's own grammar, that the generalized wheel subsumes the perturbation as an \emph{instance} (it lands on a verified face or spoke), a \emph{boundary} (it lands on the measurability gate), or a \emph{frontier} (it lands on a dashed, unverified edge), naming the relevant verified handle or the literature result.

\begin{enumerate}[leftmargin=*,itemsep=3pt]
\item \textbf{Infinite label space.} Let the labels range over many classes, so the hub measure becomes the DS dimension and the criterion becomes multiclass PAC learnability. Show this is a frontier item: the kernel carries the Natarajan and strong-shattering definitions and the inequality \leanname{strongShatteringDim_le_NatarajanDim}, but not the Brukhim et al.\ (2022) equivalence, so the edge is dashed rather than solid.
\item \textbf{Real-valued labels.} Replace shattering by $\gamma$-shattering at a scale. Show the wheel's shape survives with the shattering relation re-parametrized by scale while its proof status does not: pseudodimension and fat-shattering are defined in the kernel yet carry no equivalence theorem, so they sit on the dashed frontier (Alon--Ben-David--Cesa-Bianchi--Haussler 1997).
\item \textbf{Improper learner.} Let the learner output hypotheses outside the class. Show the wheel is invariant: the hub is the VC dimension of the \emph{target} class, not of the learner's hypothesis space, so the perturbation moves a quantity the loop does not depend on (it only shifts cost, a satellite).
\item \textbf{Learnable without uniform convergence.} Show that inside the verified wheel this is impossible, since uniform convergence and learnability are one face (\leanname{uc_imp_pac}, \leanname{vcdim_finite_imp_uc'}); the counterfactual is therefore the act of stepping off the loop, into the partial-concept or improper-agnostic region.
\item \textbf{Online learnability.} Show this is the nested inner wheel (\leanname{littlestone_characterization}), joined to the outer one by the single one-way bridge \leanname{online_strictly_stronger_pac}: the online demand implies the statistical one but not conversely, with threshold rules standing at the gap.
\item \textbf{Label noise.} Drop realizability and measure success against the best rule in the class. Show this is a frontier face: \leanname{AgnosticPACLearnable} is defined in the kernel but carries no equivalence theorem, so the classical agnostic characterization (Vapnik 1998) is cited, not verified.
\item \textbf{Partial concepts.} Allow partially-defined concepts. Show this is the one frontier edge that \emph{breaks}: there are partial classes learnable with infinite VC dimension (Alon--Hanneke--Holzman--Moran 2021), so the VC-to-learnability face fails and the figure must mark its own central edge as false here.
\item \textbf{Drop measurability.} Remove the well-behavedness side-condition. Show this opens the gate: there is a class, a hard argmax-routed attention head, whose failure event is not measurable (\leanname{soft_vs_hard_attention_measurabilitySeparation}), so uniform convergence cannot even be stated and the wheel dissolves.
\item \textbf{Compression without side information.} Demand a scheme of size linear in the dimension with no side bits. Show this is the open dashed edge parallel to the verified with-information compression face: the Littlestone--Warmuth conjecture, isolated as \leanname{FLT_Proofs.Complexity.Structures.Open_NoInfoCompressionStrengthening} but unproven.
\item \textbf{Constructive necessity.} Ask the necessity direction to exhibit an explicit hard distribution. Show this perturbs an edge's \emph{character}, not the node set: the sufficiency spokes are constructive (they build an empirical risk minimizer), the necessity spoke is existential, and whether it can be made constructive is open.
\end{enumerate}

\subsection*{Counterfactual exercises: generalizing Figure~\ref{fig:model-selection-triangle}}

Each exercise perturbs the model-selection setup. Show, in the figure's grammar, that the generalized diagram (the verified compression-to-VC spine, the three principles as approximating instances, side information as an explicit axis) subsumes it as an \emph{instance} (it lands on a node or solid edge), a \emph{boundary} (it lands on the size bound or the realizability guard), or a \emph{frontier} (it lands on a dashed or open edge).

\begin{enumerate}[leftmargin=*,itemsep=3pt]
\item \textbf{Unbounded description length.} Remove the size bound on the summary. Show this is a boundary: the spine's left node is \emph{bounded} compression, and releasing the bound makes every class trivially compressible, so the equivalence fails exactly there (the guard is the size field of \leanname{CompressionScheme}).
\item \textbf{Two-part versus one-part codes.} Compare a transmit-then-correct code with a single mixture code. Show both are instances of the side-information node: the side tag is precisely the part-selector, so the part count is invisible to the spine (\leanname{CompressionSchemeWithInfo}).
\item \textbf{Non-computable prior.} Replace the prior by one that is not lower-semicomputable. Show this breaks only the Bayesian instance edge (\leanname{bayesianToBatch}) while leaving the spine intact, since the spine never mentions a prior; it is a frontier perturbation on one approximation, not on the verified core.
\item \textbf{PAC-Bayes as the bridge.} Show this is the one solid cross-edge: \leanname{pac_bayes_finite} bounds the Gibbs risk by empirical risk plus a prior-weighted complexity term, so the shared quantity the three principles confuse is itself a theorem about generalization.
\item \textbf{Kolmogorov and Solomonoff limit.} Send description length to Kolmogorov complexity and the prior to the universal distribution. Show this is the frontier: both objects are defined (\leanname{KolmogorovComplexity}, \leanname{AlgorithmicProbability}) but the coding theorem fusing them is unproven, so the ideal-MDL-equals-ideal-Bayes identity is a dashed limit the instance edges approach but do not reach.
\item \textbf{Improper learner.} Let the reconstructed hypothesis lie outside the class. Show the spine is invariant: the scheme reconstructs an arbitrary labelling function, not a member of the class, so improperness changes nothing in the equivalence.
\item \textbf{No-side-information $O(d)$ compression.} Demand a summary of size linear in the dimension with no tag. Show this is the open frontier edge: the reverse direction (a tag-free summary already forces finite VC) is verified (\leanname{compression_bounds_vcdim}), while the forward strengthening is the Littlestone--Warmuth conjecture (\leanname{FLT_Proofs.Complexity.Structures.Open_NoInfoCompressionStrengthening}); this asymmetry is why the side-information axis is drawn.
\item \textbf{Agnostic model selection.} Drop realizability. Show this is the boundary where the scheme's realizability guard is released, moving the picture from the verified realizable spine to agnostic theory.
\item \textbf{Structural risk minimization.} Choose a nested hierarchy and let the data pick the level. Show this is the operational instance that realizes the spine's finite-VC restriction constructively (\leanname{SRM}), though no SRM-optimality theorem is yet formalized.
\item \textbf{Bias and variance.} Re-read the whole figure as a bias-variance tradeoff. Show this is a relabeling, not a new node: the spine's complexity side is the variance-controlling quantity, and PAC-Bayes exhibits the tradeoff explicitly as fit plus complexity.
\end{enumerate}
