...
The wheel of \Cref{fig:concept-class-hub} claims that the listed conditions coincide, and most of its spokes are machine-checked. Each exercise below nudges the claim sideways and asks where the nudged version lands. Three landing zones recur: a perturbation that stays on a verified face or spoke is an \emph{instance}; one that comes to rest on the measurability gate is a \emph{boundary}; one that ends on a dashed, still-unverified edge is a \emph{frontier}. For each, identify the zone and name the verified handle or literature result that pins it there. The interest lies in how few of these land back on solid ground.

\begin{enumerate}[leftmargin=*,itemsep=3pt]
\item \textbf{Infinite label space.} Let the labels range over many classes, so the hub measure becomes the DS dimension and the criterion becomes multiclass PAC learnability. This lands on the frontier: the kernel carries the Natarajan and strong-shattering definitions and the inequality \leanname{strongShatteringDim_le_NatarajanDim}, but it stops short of the Brukhim et al.\ (2022) equivalence, leaving the edge dashed.
\item \textbf{Real-valued labels.} Replace shattering by $\gamma$-shattering at a scale. The wheel keeps its shape once the shattering relation is re-parametrized by scale, while the proof status lags behind: pseudodimension and fat-shattering are defined in the kernel yet carry no equivalence theorem, so they sit on the dashed frontier (Alon--Ben-David--Cesa-Bianchi--Haussler 1997).
\item \textbf{Improper learner.} Let the learner output hypotheses outside the class. The wheel is invariant here: the hub reads the VC dimension of the \emph{target} class alone, so the learner's enlarged hypothesis space is a quantity the loop never consults; the perturbation only shifts cost, a satellite of the diagram.
\item \textbf{Learnable without uniform convergence.} Inside the verified wheel this cannot happen, since uniform convergence and learnability are one face (\leanname{uc_imp_pac}, \leanname{vcdim_finite_imp_uc'}). The counterfactual is the very act of stepping off the loop, out into the partial-concept or improper-agnostic region where the two come apart.
\item \textbf{Online learnability.} This is the nested inner wheel (\leanname{littlestone_characterization}), joined to the outer one by the single one-way bridge \leanname{online_strictly_stronger_pac}: the online demand implies the statistical one, and the implication runs in that direction only, with threshold rules standing at the gap.
\item \textbf{Label noise.} Drop realizability and measure success against the best rule in the class. This sits on a frontier face: \leanname{AgnosticPACLearnable} is defined in the kernel but carries no equivalence theorem, so the classical agnostic characterization (Vapnik 1998) enters by citation alone and awaits a verified companion.
\item \textbf{Partial concepts.} Allow partially-defined concepts. This is the one frontier edge that actually \emph{breaks}: there are partial classes learnable with infinite VC dimension (Alon--Hanneke--Holzman--Moran 2021), so the VC-to-learnability face fails outright and the figure must mark its own central edge as false here.
\item \textbf{Drop measurability.} Remove the well-behavedness side-condition. This swings the gate open: there is a class, a hard argmax-routed attention head, whose failure event escapes measurability (\leanname{soft_vs_hard_attention_measurabilitySeparation}), so uniform convergence cannot even be stated and the wheel dissolves.
\item \textbf{Compression without side information.} Demand a scheme of size linear in the dimension with no side bits. This traces the open dashed edge running parallel to the verified with-information compression face: the Littlestone--Warmuth conjecture, isolated as \leanname{FLT_Proofs.Complexity.Structures.Open_NoInfoCompressionStrengthening} and still unproven.
\item \textbf{Constructive necessity.} Ask the necessity direction to exhibit an explicit hard distribution. This perturbs an edge's \emph{character} while leaving the node set fixed: the sufficiency spokes are constructive (they build an empirical risk minimizer), the necessity spoke is existential, and whether it can be made constructive is open.
\end{enumerate}

\subsection*{Counterfactual exercises: generalizing Figure~\ref{fig:model-selection-triangle}}

\Cref{fig:model-selection-triangle} has a single load-bearing element: the compression-to-VC spine, the one part of the picture that is actually proved. The three model-selection principles hang off it as approximations, and side information runs as an explicit axis beside it. Each exercise below disturbs some part of that arrangement; the task is to say where the disturbance settles. A perturbation that comes to rest on a node or a solid edge is an \emph{instance}; one caught by the size bound or the realizability guard is a \emph{boundary}; one that ends on a dashed or open edge is a \emph{frontier}. Note throughout how robust the spine is and how fragile the three principles around it remain.

\begin{enumerate}[leftmargin=*,itemsep=3pt]
\item \textbf{Unbounded description length.} Remove the size bound on the summary. This is a boundary case: the spine's left node is \emph{bounded} compression, and releasing the bound makes every class trivially compressible, so the equivalence fails exactly there. The guard that catches it is the size field of \leanname{CompressionScheme}.
\item \textbf{Two-part versus one-part codes.} Compare a transmit-then-correct code with a single mixture code. Both reduce to the same instance of the side-information node: the side tag is precisely the part-selector, so the part count stays invisible to the spine (\leanname{CompressionSchemeWithInfo}).
\item \textbf{Non-computable prior.} Replace the prior by one that is not lower-semicomputable. This breaks the Bayesian instance edge (\leanname{bayesianToBatch}) alone while leaving the spine intact, since the spine never mentions a prior; it is a frontier perturbation confined to one approximation, with the verified core untouched.
\item \textbf{PAC-Bayes as the bridge.} This is the one solid cross-edge in the figure: \leanname{pac_bayes_finite} bounds the Gibbs risk by empirical risk plus a prior-weighted complexity term, so the shared quantity the three principles confuse is itself a theorem about generalization.
\item \textbf{Kolmogorov and Solomonoff limit.} Send description length to Kolmogorov complexity and the prior to the universal distribution. This belongs to the frontier: both objects are defined (\leanname{KolmogorovComplexity}, \leanname{AlgorithmicProbability}) but the coding theorem fusing them is unproven, so the ideal-MDL-equals-ideal-Bayes identity stays a dashed limit the instance edges approach without reaching.
\item \textbf{Improper learner.} Let the reconstructed hypothesis lie outside the class. The spine is invariant: the scheme reconstructs an arbitrary labelling function, with no membership requirement on the output, so improperness changes nothing in the equivalence.
\item \textbf{No-side-information $O(d)$ compression.} Demand a summary of size linear in the dimension with no tag. This is the open frontier edge: the reverse direction (a tag-free summary already forces finite VC) is verified (\leanname{compression_bounds_vcdim}), while the forward strengthening is the Littlestone--Warmuth conjecture (\leanname{FLT_Proofs.Complexity.Structures.Open_NoInfoCompressionStrengthening}); this asymmetry is the very reason the side-information axis is drawn.
\item \textbf{Agnostic model selection.} Drop realizability. This is the boundary at which the scheme's realizability guard is released, carrying the picture off the verified realizable spine and into agnostic theory.
\item \textbf{Structural risk minimization.} Choose a nested hierarchy and let the data pick the level. This is the operational instance that realizes the spine's finite-VC restriction constructively (\leanname{SRM}), though no SRM-optimality theorem is yet formalized.
\item \textbf{Bias and variance.} Re-read the whole figure as a bias-variance tradeoff. This is a relabeling of the existing nodes with no node added: the spine's complexity side is the variance-controlling quantity, and PAC-Bayes exhibits the tradeoff explicitly as fit plus complexity.
\end{enumerate}
