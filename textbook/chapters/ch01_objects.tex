...
A learner facing data from an unknown source needs to know what it is even asking. Without the right vocabulary, the question ``find a rule that predicts well'' remains unanswerable: not hard, unanswerable. No algorithm, however clever, generalizes without a prior commitment to which functions are candidates. Without that vocabulary the central results of learning theory cannot even be stated, let alone proved.

The vocabulary divides into three groups of unequal importance. The first group, domain, label, concept, hypothesis, is atomic: these are the sets and functions from which everything else is built. They require only brief definitions. The second group, concept class, hypothesis space, version space, inductive bias, carries the genuine mathematical weight, because the relationships among these objects determine what is learnable; we prove the load-bearing facts in full. The third group, Bayesian inference, minimum description length, minimum message length, looks like vocabulary but is actually a network of near-miss analogies, each encoding a different answer to the same question about model selection. Treating them as interchangeable is the standard mistake; the obstructions are visible only when each is stated precisely.
...
The following objects are the type-level substrate of learning theory. None of them, individually, contains any mathematical surprise; the surprise arrives only when we ask how they compose.
...
These definitions are unremarkable individually. The mathematical content begins when they are placed together and asked to answer for learnability.
...
This definition is simple. Its importance is not. The concept class is the single object on which the chapter's deepest theorem turns: a family of apparently different demands one might place on it, learnability from few examples, finite shattering, bounded compression, vanishing correlation with noise, sub-exponential growth, finite sample size, all reduce to the same condition. The class is the vertex at which they coincide (Figure~\ref{fig:concept-class-hub}).\formal{FLT_Proofs.Theorem.PAC.fundamental_theorem}
...
\end{tikzpicture}
\caption[The concept class as the hub of five equivalent characterizations]{\textbf{Five conditions, one concept class.} PAC learnability, finite shattering dimension, bounded sample compression, vanishing Rademacher correlation, and sub-exponential growth are the same condition: each holds if and only if the others do. The figure makes that equivalence visible by placing the concept class at the center and each characterization at a spoke. A tighter wheel for online learning sits inside, connected to the outer ring by a one-way bridge: the online demand implies the statistical one, but not conversely, and the threshold class witnesses that gap. The measurability gate is the condition the theory assumes; the dashed frontier marks extensions that are expected but not yet verified; and the edge for partial concepts is drawn in red because the equivalence there is known to break.}
...
The proper--improper distinction carries real weight. A learner that can only return answers from the target class faces harder constraints than one free to use a richer representation. The computational consequences of this difference are among the sharpest in the theory:
...
Inductive bias is a \emph{mathematical necessity}, forced on every learner without exception. The No-Free-Lunch theorem is the proof: any learner that tries to generalize without committing to a class can be made to fail, regardless of how many examples it sees, on a target drawn from the class it refused to commit to. We prove this through the combinatorial engine that makes the failure explicit, then read off the classical statement and the necessity of bias as corollaries.
...
Bayesian inference, minimum description length, and minimum message length are often described informally as ``the same idea.'' They are not. Each answers the question ``which hypothesis should I prefer?'' and each gives a different answer for a different reason. The differences between them are more instructive than any individual definition. Each is, in the sense of \cref{def:inductive-bias-typed}, a choice of preference score; the analogies among them are analogies among scores; and the obstructions are precisely where the scores fail to agree. What passes for a unified principle in many treatments is actually three methods with three distinct obstruction types between them, and the table of obstructions is the content.
...
\end{tikzpicture}
\caption[Three model-selection principles organized around a proven compression equivalence]{\textbf{What ``prefer the simpler hypothesis'' actually means.} The chapter proves one thing cleanly: a class is learnable exactly when its data admit a bounded summary that reconstructs every consistent labelling, the machine-checked Moran--Yehudayoff equivalence. That equivalence is the spine of the figure. Bayesian inference, MDL, and MML each approach the same compression intuition from a different direction, and every direct link among the three is blocked: the Bayesian-MML link has no equivalence theorem beyond the constant-Fisher-information case; the MML-MDL link lacks a finite-sample witness; the MDL-Bayesian link fails when the prior is improper. One genuine bridge exists: the PAC-Bayes bound connects prior-weighted complexity to generalization. Whether the side-information tag on the compression spine can be removed without losing the summary size bound is the Littlestone--Warmuth conjecture, still open (\cref{op:compression-objects}).}
...
Three different formalizations of ``prefer the simpler hypothesis'' are \emph{not} equivalent, and the reasons for non-equivalence differ pair by pair: missing witness, missing witness, and type mismatch. That each obstruction is distinct is a structural feature of learning theory that no single definition can convey. It requires placing all three side by side.
...
The MDL idea, prefer the hypothesis that compresses the data, connects directly to \cref{thm:vc-char}, and the connection is an equivalence, not an analogy. A \emph{sample compression scheme} of size $k$ reconstructs any consistent labeling from a subsequence of at most $k$ of its own examples. Compression to a bounded summary and finite VC dimension are the same condition.
...
A single softmax-attention head, read as a binary predictor, is a concept in the sense of \cref{def:domain-label-concept}, a function $X \to \{0,1\}$, and letting its parameters vary produces a concept class in the sense of \cref{def:concept-class}. The question of the chapter applies directly to the central object of modern machine learning: \emph{is the attention class a legitimate concept class, one to which the VC characterization applies?} The answer depends entirely on whether the ``mild measurability conditions'' of \cref{thm:vc-char}, stated precisely in \Cref{ch:generalization}, are satisfied. This section makes those conditions concrete. They are satisfied for softmax attention and violated for hard attention, and the line between them is a boundary in descriptive set theory.
...
A single object, the concept class, carries five equivalent characterizations of learnability; inductive bias is forced on every learner; three standard model-selection principles are distinct, with different obstructions pair by pair; and the measurability conditions of the fundamental theorem are not vacuous. The following thirteen problems extend these observations. Each is anchored to a concept introduced above and tagged by its epistemic character: a \emph{known unknown} (\textsc{ku}) is an articulated question awaiting an answer; an \emph{unknown known} (\textsc{uk}) is a regularity in the development or literature not yet absorbed into a clean statement. Resolving any one of them would tighten a claim made in the body of this chapter.
...
The hub diagram of \Cref{fig:concept-class-hub} and the compression spine of \Cref{fig:model-selection-triangle} rest on specific conditions. Each exercise removes or replaces exactly one condition and asks what survives. In some cases the equivalence persists under a different characterizing object; in others the edge breaks outright. For each, state which conditions still coincide and name the result that settles it.
