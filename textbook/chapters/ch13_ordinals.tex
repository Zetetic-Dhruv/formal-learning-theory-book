...
A learner that always converges is still not fully pinned down: it may change
its mind ten times, or a hundred, or a number that grows without bound as the
target function changes.  In \Cref{ch:gold} we saw that $\Ex$-learners are
permitted finitely many mind changes, but no single integer $n$ need bound
that count across all targets in a class.  The natural response is to measure
the mind-change complexity of a class and ask whether the measure stratifies
learning power into a strict hierarchy.

The integer counter fails this task.  A class can be $\Ex$-learnable and yet
require the learner to revise its hypothesis a number of times that grows with
the target, outrunning every fixed $k$.  What the integer lacks, and what
Freivalds and Smith~\cite{FreivaldsSmith1993} found, is a counter that can be
set to $\omega$ and then \emph{spent down adaptively}: a constructive ordinal.
That counter carries the learner past every finite threshold because its first
move can land anywhere in $\N$, the landing point chosen from the data.  The
resulting mind-change ordinal assigns each learnable class a position in a
transfinite hierarchy that fills the gap between finite learning ($\FIN$) and
identification in the limit ($\Ex$).  The hierarchy is strict at every level,
and even its entire union falls short of $\Ex$.
...
The transfinite counter a learner carries must be something a Turing machine
can actually manipulate: a constructive ordinal, one with an explicit
computable description.  Kleene's system~$\mathcal{O}$ carves out
...
Calling $\Ex_\omega$ a ``catch-all'' for classes learnable with finitely
many but unbounded mind changes merely relabels the gap; it does not expose
the structure inside it.  That gap is not flat: some classes require mind
changes that depend on prior mind changes, a second-order revision.
The question is whether any hierarchy tracks this.

\subsection*{The Freivalds--Smith construction}

In 1993, Freivalds and Smith~\cite{FreivaldsSmith1993} answered this
question with a construction that still reads as a surprise: the
\emph{ordinal arithmetic of mind changes} is the correct framework.
...
The passage from $\bigcup_n \Ex_n$ to $\Ex_\omega$ buys computational
power: the learner can defer its commitment to a finite bound until it
has seen enough data to determine what the bound should be.  The class
$\{f_n : n \in \N\}$ lives in the gap, and no relabeling of integer
counters reaches it.  This deferred commitment is the operational content
of transfinite mind changes.
\end{remark}
...
The hierarchy the ordinal index produces is strict at every level, and
its union still does not reach $\Ex$.
...
decrements the counter from $\alpha + 1$ to $\alpha$.  The
$\alpha$-bound on $M^*$'s residual revisions follows by reconstructing
the diagonal construction from the data via the recursion theorem; see
Freivalds and Smith~\cite{FreivaldsSmith1993} for the priority argument.
\end{proof}
...
The gap between program and function widens the criterion.  In
$\Ex$-learning, convergence is \emph{syntactic}: the learner must eventually
stabilize on the same hypothesis \emph{index} forever.  $\BC$-learning asks
only for \emph{semantic} convergence: every hypothesis output past some
point must compute the right function, but the index may keep changing.
That relaxation opens real space: there are classes a $\BC$-learner can
handle that no $\Ex$-learner can touch.
...
The theorem below makes this precise: $\BC \setminus \Ex \neq \emptyset$
establishes a limit of $\Ex$, and the weight of the proof falls on
constructing the witness class.
...
The operator recursion theorem~\cite{CaseSmith1983} supplies a total
recursive~$g$ together with two indices $e \neq e'$ with
$\varphi_e = \varphi_{e'} = g$ on which $M$ produces the \emph{same}
limiting hypothesis~$h^*$.  But $\varphi_{h^*}(0)$ cannot equal both
$e$ and~$e'$, so $M$ misidentifies at least one of $f_{g,e}, f_{g,e'}$,
contradicting the assumption that $M$ $\Ex$-identifies $\mathcal{C}$.
\end{proof}

\begin{separation}
A $\BC$-learner may output different indices for the same function,
exploiting the many-to-one nature of G\"odel numbering; an $\Ex$-learner
must converge to a single index.  When $\mathcal{C}$ holds infinitely many
functions differing only in their self-referential label at argument~$0$,
every candidate index is extensionally correct for the current function
but wrong for the others, so the $\Ex$-learner cannot commit.  The
obstruction is the gap between extensional and intensional identity:
...
\end{tikzpicture}
\caption[The ordinal complexity spine of limit identification.]{Two strict towers,
one ordinal-valued measure each.  Left column: the mind-change ordinal of a limit
learner stratifies $\Ex$ into levels $\Ex_\alpha$ that rise through every constructive
ordinal without ever reaching $\Ex$ itself; successive rungs are strict by the
Freivalds--Smith and Ambainis--Jain--Sharma constructions, and the integer fragment
$\FIN = \Ex_0 \subsetneq \Ex_1 \subsetneq \cdots$ (Velauthapillai) is this tower's
finite base.  Right column: an orthogonal axis relaxes the final hypothesis rather than
the revision count, producing the anomaly levels $\Ex_a^*$ that continue through $\BC_0$
to $\BC$ (Case--Smith).  Every arrow is a classical literature result, none yet
machine-checked.  Lower box: a separate, verified measure on the dimension side.  The
embedding \leanname{withTopNatToOrdinal} is machine-checked; it fixes each finite VC
dimension and sends the infinite value to $\omega$, placing softmax attention and all
other finite-capacity classes at the bottom of the ordinal picture.  The transfinite
levels above $\omega$ arise only when the objects learned are programs and the criterion
is exact identification, a recursion-theoretic regime that no finite-capacity class
can reach.}
...
The mind-change ordinal extends the grammar of the plain
\nde{mind\_change\_count} by adding one new primitive that the integer
count lacks: constructive ordinals.  The characterization of which
ordinal a class carries is supplied by \nde{mind\_change\_characterization}
(Ambainis--Jain--Sharma 1999).  The measure itself is due to
Freivalds--Smith 1993.
\end{graphnode}

\begin{graphnode}[title={\nde{bc\_learning}}]
Behaviorally correct learning restricts \nde{ex\_learning} by removing one
of its constraints: it drops the requirement of syntactic convergence and
asks only for eventual semantic correctness.  Due to Case--Smith 1983.
\end{graphnode}
...
finite-capacity concept class can reach.
\end{remark}
...
The classical lines of the hierarchy are settled, but three fronts remain
genuinely open: formalizing what is classically proved, connecting the
mind-change ordinal to the verified dimension-side rank, and mapping the
two-dimensional mind-change-by-anomaly lattice.
...
languages characterize $\omega^n$?  Left open by Sharma, Stephan, and
Ventsov~\cite{SharmaStephanVentsov2004}.
\end{openprob}
...
including the priority constructions?
\end{openprob}
...
strictness theorem survives the quotient by notation~\cite{MartinSharmaStephan2006}.
\end{openprob}
...
ordinal rank?  (\leanname{softAttention_wellBehaved},
\leanname{optimal_mistake_bound_eq_ldim}.)
\end{openprob}
...
instances of one well-founded-recursion principle?
\end{openprob}
...
$\alpha \oplus \beta$ and no smaller?
\end{openprob}
...
does naturalness break down past $\omega^\omega$?~\cite{AmbainisJainSharma1999}
\end{openprob}
...
ordinals), and does it interleave with the mind-change ordinals?
\end{openprob}
...
related: is one always a bound on the other?~\cite{MartinSharmaStephan2006}
\end{openprob}
...
The figure in \Cref{fig:ordinal-hierarchy} encodes an argument.
Each exercise below isolates one rung or one architectural choice and asks
what the picture would look like if that choice had been made differently,
or if one component were taken away.  Working through them tests
whether you can reconstruct the hierarchy's reasoning.

\begin{enumerate}[leftmargin=*,itemsep=3pt]
\item Restrict the left column to the finite fragment $\FIN = \Ex_0 \subsetneq \Ex_1 \subsetneq \cdots \subsetneq \Ex_n$. Show that this is the integer instance of the ordinal spine: a learner bounded to $n$ revisions sits at the finite ordinal $n$, the strict inclusions hold (\cref{thm:summary}~(i), \cite{Velauthapillai1989}), and the transfinite picture is the closure of this fragment under limits.

\item Read \cref{prop:online-ordinal-descent} as the inset's mechanism. The optimal online mistake bound equals the Littlestone dimension (\leanname{optimal_mistake_bound_eq_ldim}), and each mistake strictly lowers the surviving version space's Littlestone dimension, a well-founded descent that stays below $\omega$. Lifted through \leanname{withTopNatToOrdinal} this is a finite ordinal counted down by ordinary induction, the finite-rank shadow of the transfinite mind-change counter.

\item The left column is the recursion-theoretic mind-change ordinal (\cref{def:mco-class}); the inset is the dimension-side ordinal VC rank (\leanname{vcdim_to_ordinal_vcdim}, \leanname{OrdinalVCDim}) of Martin--Sharma--Stephan \cite{MartinSharmaStephan2006}. These are two ordinal-valued measures on overlapping domains, and whether one bounds the other is open (\cref{op:mco-vs-ordinalvc}).

\item Consider the rung $\bigcup_{n<\omega}\Ex_n \subsetneq \Ex_\omega$, where the spine becomes genuinely transfinite. A learner may need finitely many mind changes on each input with no uniform integer bound, so the supremum is $\omega$ and no $\Ex_n$ suffices (\cref{rem:omega-to-n}, \cref{thm:summary}~(ii), \cite{FreivaldsSmith1993}). Show that the integer hierarchy of Exercise~1 cannot reach this jump, which is what forces the ordinal index.

\item Turn from the mind-change column to the right column $\Ex \subsetneq \Ex_1^* \subsetneq \cdots \subsetneq \Ex^* \subsetneq \BC_0 \subsetneq \BC$. This is an orthogonal axis of relaxation: the mind-change column bounds the number of hypothesis revisions, while the anomaly column tolerates errors in the final hypothesis and drops syntactic for semantic convergence (\cref{thm:anomaly-hierarchy}, \cref{thm:bc-separation}, \cite{CaseSmith1983}). Argue that the two axes are independent.

\item The Freivalds--Smith hierarchy, including its priority constructions, has no verified counterpart (\cref{op:formalize-ordinal-hierarchy}), and the Case--Smith separation rests on an operator recursion theorem that is sketched rather than proved (\cref{op:formalize-bc}, \cite{CaseSmith1983}). The tower is classically complete but its formalization is open. Identify what a proof-assistant development of constructive ordinal notations, $\alpha$-bounded learners, and the strict inclusions of \cref{thm:ordinal-hierarchy} would have to supply.

\item Read the left column and the inset as two instances of one principle. The online descent lowers a finite Littlestone rank on each mistake and stays below $\omega$, while the mind-change counter descends a constructive ordinal that may exceed every $\omega^n$. Whether both are instances of a single well-founded-recursion principle, a version-space descent functor whose ordinal rank is the mind-change ordinal and whose finite-rank restriction is the certified online descent, is open (\cref{op:online-transfinite}), as is whether every point of the two-dimensional mind-change-by-anomaly lattice admits such a rank (\cref{op:two-dim-lattice}, \cite{SharmaStephanVentsov2004}).
\end{enumerate}
