...
the correct measure is a \emph{constructive ordinal}.  An integer cannot do
the job: the class $\{f_n : n \in \N\}$ below is $\Ex$-learnable yet its
mind-change count outruns every fixed $k$, so a counter that can be set to
$\omega$ and then spent down is exactly what the integer counter lacks.
...
Allowing $\Ex_\omega$ as a ``catch-all'' for classes learnable with
finitely many but unbounded mind changes merely relabels the gap; it does
not give a \emph{refined} hierarchy.  The gap between $\bigcup_n \Ex_n$ and
$\Ex$ contains structure: some classes require mind changes that depend on
mind changes, a kind of second-order revision.  Is there a hierarchy that
...
\end{tikzpicture}
\caption[The ordinal complexity spine of limit identification.]{The mind-change count
of a limit learner (left column) or its anomaly tolerance (right column) is a constructive
ordinal that stratifies the identification criteria into a strict tower, from $\FIN = \Ex_0$
through the transfinite levels $\Ex_\alpha$ into $\Ex$, and on through the anomaly levels
$\Ex_a^*$ into $\BC$.  The lower box records a separate measure on the dimension side: the
embedding \leanname{withTopNatToOrdinal} sends an infinite VC dimension to $\omega$ and fixes
every finite dimension below it, so finite-capacity classes such as softmax attention sit at
the bottom and the transfinite tower becomes necessary only for the exact identification of
programs.}
...
its formalization, its interaction with the verified online rank, and its
two-dimensional refinement remain open.
...
Each exercise perturbs one part of \Cref{fig:ordinal-hierarchy} and asks where the change lands in the tower.
