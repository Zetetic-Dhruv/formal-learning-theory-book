...
Green:~\rel{restricts}.  Brown:~\rel{lower\_bounds}.  Each arrow represents an
edge in the concept graph.  The \rel{characterizes} edges target the
appropriate variant of PAC learning for each source: VC dimension for binary
(and agnostic binary) PAC, the DS dimension for \emph{multiclass} PAC, the
fat-shattering dimension for \emph{agnostic real-valued} PAC.  The Natarajan
dimension does \emph{not} characterize multiclass PAC---it only extends the
grammar and is dominated by DS (\Cref{prop:ds-vs-nat}, with the $\Ndim=1$,
$\DSdim=\infty$ obstruction).}
...
multiclass next-token head---whether the DS dimension of an attention class
...
real DS relationship $\Ndim \le \DSdim$.  Can the true DS dimension---default plus single-coordinate
...
learning---an information-theoretic, not merely computational, separation?  A
known unknown.
\end{openprob}
