...
Binary PAC theory is as clean as learning theory gets: one condition, finite
VC dimension, characterizes learnability, and the sample complexity is tight.
That cleanliness has a price.  The theory is built for a label set of size two,
and the moment you leave that setting, the language breaks.  Not bends: breaks.
Multiclass labels, real-valued targets, corrupted observations, absent
realizability, restricted hypothesis outputs: in each case, simply substituting
richer structure into the binary definitions produces a condition that is
necessary but not sufficient, or sufficient but not necessary, or both.

Why?  Because each extension forces a new \emph{mathematical primitive}: a
concept the binary language has no word for.  You cannot describe a
``threshold vector'' in a theory whose outputs are already $\{0,1\}$.  You
cannot define a ``noise rate'' in a world with no noise.  You cannot name the
``one-inclusion witness'' with a single bit flip.  The binary theory generalizes
by \emph{grammar growth}: each extension is irreducible because each setting
adds vocabulary to the theory.

The irreducibility is the structure of the problem.  The multiclass case is
the sharpest illustration.  Natarajan's 1989 extension seemed right for thirty
years; the counterexample showing it was wrong for infinite label sets arrived
in 2022.  The gap closed only with the DS dimension, a combinatorial measure
encoding global hypergraph structure that the binary definitions, and
Natarajan's two-function primitive, simply could not express.  The sections
below trace that trajectory and the analogous ones in the real-valued,
agnostic, noisy, and computational settings.
...
\caption[Concept map: binary PAC generalizes by grammar growth]{Color encodes
relationship type; line style encodes evidence grade.  One edge is solid: VC
dimension characterizing binary PAC
learning (\leanname{fundamental_theorem}), machine-checked.  Every other arrow
is dashed or dotted, a literature result, because each setting extends the
binary theory by adding a primitive the binary language cannot name.  The
graph's structure is the chapter's thesis: grammar growth, not label
substitution, is how binary PAC generalizes.}
...
For $k = 2$, the answer is the VC dimension.  For $k > 2$, the field tried
the natural extension first: Natarajan's 1989 two-function shattering
primitive.  It worked for finite label sets.  For three decades it appeared
to be the right answer.  Then Brukhim et al.\ showed in 2022 that a class
with $\Ndim = 1$ is not PAC learnable.  The right answer requires a global
hypergraph measure, the DS dimension, whose finiteness Natarajan's local
two-function condition cannot imply.  That thirty-year trajectory, and its
2022 resolution, is the backbone of this section.
...
The same object, drawn twice.  Left: a witness table for DS-shattering with
$d = 3$.  A default labeling $f$ assigns $a, b, c$ to $x_1, x_2, x_3$; each
hypothesis $h_i$ agrees with $f$ everywhere except at coordinate $i$, where it
deviates.  Right: that table \emph{is} the star joining $f$ to each $h_i$
along edge $i$ in the one-inclusion hypergraph (\Cref{def:oig}).  The
equivalence is not cosmetic: the finiteness of the largest DS-shattered set
characterizes multiclass PAC learnability (\Cref{thm:multiclass-char}, Brukhim
et al.\ 2022), while Natarajan's two-function shattering, a strictly stronger
condition, fails to characterize it when the label set is infinite.  The
global connectivity of the hypergraph is what the binary language cannot
express, and the star is where that global structure sits.}
...
The problem with VC dimension here is immediate.  Outputs are real numbers,
not bits; shattering requires a binary condition; so the theory needs a way
to convert continuous outputs into a binary combinatorial question.  That
conversion requires a new primitive: a threshold.  Two dimensions emerge from
it, one for the realizable case and one for the agnostic.
...
thresholds]{What the margin adds that the threshold alone cannot provide.
The set $\{x_1, x_2, x_3\}$ is $\gamma$-fat-shattered when shared thresholds
$t_1, t_2, t_3$ admit, for every binary labeling, a function that clears
the margin: at least $t_i + \gamma$ where the label is $1$, at most
$t_i - \gamma$ where it is $0$.  The forbidden band of width $2\gamma$ is the
scale primitive: it has no binary analogue, makes the notion scale-sensitive,
and forces the characterization of agnostic real-valued learnability to be a
whole \emph{family} of finiteness conditions, one per $\gamma$, rather than a
single number.  The two curves realize labelings $101$ and $010$ against the
shared threshold vector; their existence for all eight labelings is exactly the
definition.  Finiteness of the fat-shattering dimension at every $\gamma >0$
(\leanname{FatShatteringDim}) characterizes agnostic real-valued PAC
learnability (Alon et al.\ 1997, \Cref{thm:real-valued-char}).}
...
Every result so far has assumed \emph{realizability}: some $h^* \in
\mathcal{H}$ perfectly fits the target.  That assumption is almost always
false in practice.  Removing it feels like it should change the characterization.
It does not: the condition for agnostic PAC learnability is still finite VC
dimension.  What changes is the cost.  The sample complexity goes from
$\Theta(d/\varepsilon)$ to $\Theta(d/\varepsilon^2)$; the $1/\varepsilon^2$ dependence
is tight, forced by a matching lower bound, and no sharper analysis can remove
it.  The agnostic setting, introduced by Haussler~\cite{Haussler1992}, is the
extension where the grammar stays the same and the price doubles.
...
All PAC definitions so far permit the learner to output \emph{any}
efficiently evaluatable hypothesis, including hypotheses outside the target
class $\mathcal{H}$.  This freedom is easy to overlook, but it is the source of the
most striking computational separation in the chapter: a hypothesis class that
is information-theoretically indistinguishable in the proper and improper
settings, needing $\Theta(d/\varepsilon)$ samples either way, is NP-hard to
learn when the learner must stay inside $\mathcal{H}$ and polynomial-time
tractable the moment it can step outside.  When the learner is restricted to
output a hypothesis from $\mathcal{H}$ itself, we call it a \emph{proper}
learner.
...
gap]{Two layers, one separation.  The statistical layer (top row) shows no
gap: proper and improper learning both need exactly $\Theta(d/\varepsilon)$
samples, because both are characterized by finite VC dimension
(\leanname{fundamental_theorem}, machine-checked, solid border).  Move to the
computational layer (bottom row, dashed border, literature) and the same class
splits: $3$-term DNF is polynomial-time improperly via $3$-CNF, yet NP-hard
to learn properly, by a direct reduction from graph $3$-coloring (Pitt and
Valiant 1988, \Cref{thm:pitt-valiant}).  The lesson is that the restriction
arrow in the top row, ``proper $\subseteq$ improper,'' costs nothing
statistically and can cost everything computationally.}
...
\end{tikzpicture}
\caption[Grammar growth from binary PAC learning]{What you cannot say in the
binary language.  Binary PAC learning, the machine-checked hub
(\leanname{fundamental_theorem}), has a complete characterization in a single
condition.  Each wavy arrow adds one primitive the binary language cannot
name: a one-inclusion witness for the multiclass setting; thresholds then a
margin for real-valued; a relative benchmark for the agnostic setting; and a
noise rate $\eta$ for classification noise.  The extensions are irreducible: no
reduction to the binary theory recovers them (\Cref{rmk:non-reduction}).  In
the multiclass case, irreducibility is a theorem: the class with $\Ndim = 1$
and $\DSdim = \infty$ (\Cref{prop:ds-vs-nat}) proves that no two-function
primitive can replace the global hypergraph condition.  For the remaining
settings, no simpler characterization is known.}
...
Each extension of binary PAC theory requires a new primitive that the binary
language cannot express, and no reduction to the binary theory eliminates that
requirement.  The figures show individual instances; the exercises below ask
you to stress-test and complete them.  Each one takes a concrete picture and
asks: does the argument generalize, what does it depend on, and where does it
fail?
...
Extending binary PAC learning to multiclass, real-valued, agnostic,
noise-tolerant, and computationally constrained settings requires grammar
growth: each setting introduces irreducible new primitives and yields its own
characterization theorem or separation result.  The next chapter turns to
what lies beyond these characterizations: application-layer concepts, the
remaining open problems, and the scope boundaries of the theory.
